...
static boolean templateMatch(const char *tmpl, const char *src, char **captures, int *captureCount, int maxCaptures) {
    ...
    while (*t) {
        ...
        while (s < matchPos) {
            if ((unsigned char)*s == COLOR_ESCAPE) s += 4; else *dest++ = *s++;
        }
        *dest = '\0';
        count++;
        s = matchPos;
...
int main() {
    char captures[32][1024]; char *capPtrs[32]; int capCount = 0;
    char src[200];
    sprintf(src, "you zap your %c%c%c%c毒素 法杖 at %c%c%c%c老鼠.", 25, 50, 50, 50, 25, 50, 50, 50);
    
    boolean res1 = templateMatch("you zap your %s at %s.", src, capPtrs, &capCount, 32);
    printf("Result 1: %d\n", res1);
    
    capCount = 0;
    sprintf(src, "Zapping your %c%c%c%c毒素 法杖:", 25, 50, 50, 50);
    boolean res2 = templateMatch("Zapping your %s:", src, capPtrs, &capCount, 32);
    printf("Result 2: %d\n", res2);
    
    capCount = 0;
    sprintf(src, "(Your %c%c%c%c桉木法杖 must be %c%c%c%c毒素 法杖.)", 25,50,50,50,25,50,50,50);
    boolean res3 = templateMatch("(Your %s must be %s.)", src, capPtrs, &capCount, 32);
    printf("Result 3: %d\n", res3);
