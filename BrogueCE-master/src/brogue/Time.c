...
void exposeCreatureToFire(creature *monst) {
  ...
  if ((monst->bookkeepingFlags & MB_IS_DYING) ||
      ...
      ((!monst->status[STATUS_LEVITATING]) &&
       cellHasTMFlag(monst->loc, TM_EXTINGUISHES_FIRE))) {
    return;
  }
  if (monst->status[STATUS_BURNING] == 0) {
    if (monst == &player) {
      ...
      // updateVision(); // this screws up the firebolt visual effect by erasing
      // it while a message is displayed
      combatMessage(tr("you catch fire"), &badMessageColor);
  ...
  monst->status[STATUS_BURNING] = monst->maxStatus[STATUS_BURNING] =
      max(monst->status[STATUS_BURNING], 7);
}
...
void updateFlavorText() {
  ...
  if (rogue.disturbed && !rogue.gameHasEnded) {
    if (rogue.armor && (rogue.armor->flags & ITEM_RUNIC) &&
        ...
        tileCatalog[pmapAt(player.loc)
                        ->layers[highestPriorityLayer(player.loc.x,
                                                      player.loc.y, false)]]
                .flags &
...
void updatePlayerUnderwaterness() {
  if (rogue.inWater) {
    if (!cellHasTerrainFlag(player.loc, T_IS_DEEP_WATER) ||
        ...
        cellHasTerrainFlag(player.loc,
                           (T_ENTANGLES | T_OBSTRUCTS_PASSABILITY))) {
...
void applyInstantTileEffectsToCreature(creature *monst) {
  ...
  if (monst == &player) {
    if (!player.status[STATUS_LEVITATING]) {
      pmap[*x][*y].flags |= KNOWN_TO_BE_TRAP_FREE;
             ...
             !monst->status[STATUS_LEVITATING] && canSeeMonster(monst) &&
             !(cellHasTerrainFlag((pos){*x, *y}, T_IS_DF_TRAP))) {
    pmap[*x][*y].flags |= KNOWN_TO_BE_TRAP_FREE;
  }

  // You will discover the secrets of any tile you stand on.
  if (monst == &player && !(monst->status[STATUS_LEVITATING]) &&
      cellHasTMFlag((pos){*x, *y}, TM_IS_SECRET) && playerCanSee(*x, *y)) {

    discover(*x, *y);
  }

  // Submerged monsters in terrain that doesn't permit submersion should
  // immediately surface.
  ...
  if (!(monst->status[STATUS_LEVITATING]) &&
    ...
    if (monst == &player) {
      sprintf(
          buf, "you plunge into %s!",
          tileCatalog[pmap[*x][*y]
                          .layers[layerWithFlag(*x, *y, T_LAVA_INSTA_DEATH)]]
              .description);
      ...
      spawnDungeonFeature(*x, *y, &(dungeonFeatureCatalog[DF_CREATURE_FIRE]),
                          true, false);
  ...
  if (cellHasTMFlag((pos){*x, *y}, TM_EXTINGUISHES_FIRE) &&
      monst->status[STATUS_BURNING] && !monst->status[STATUS_LEVITATING] &&
      !(monst->info.flags & MONST_ATTACKABLE_THRU_WALLS) &&
  ...
  if (!(monst->status[STATUS_LEVITATING]) &&
    ...
    for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++) {
      if (tileCatalog[pmap[*x][*y].layers[layer]].flags & T_IS_DF_TRAP) {
        spawnDungeonFeature(
            *x, *y,
            &(dungeonFeatureCatalog[tileCatalog[pmap[*x][*y].layers[layer]]
                                        .fireType]),
            true, false);
  ...
  if (cellHasTMFlag((pos){*x, *y},
                    TM_PROMOTES_ON_CREATURE)) { // flying creatures activate too
    // Because this uses no pressure plate to keep track of whether it's already
    // depressed, it will trigger every time this function is called while the
    // monster or player is on the tile. Because this function can be called
    // several times per turn, multiple promotions can happen unpredictably if
    // the tile does not promote to a tile without the T_PROMOTES_ON_STEP
    ...
    for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++) {
      if (tileCatalog[pmap[*x][*y].layers[layer]].mechFlags &
          TM_PROMOTES_ON_CREATURE) {
        promoteTile(*x, *y, layer, false);
      }
    }
  }

  if (cellHasTMFlag((pos){*x, *y}, TM_PROMOTES_ON_PLAYER_ENTRY) &&
      monst == &player) {
  ...
  // spiderwebs
  if (cellHasTerrainFlag((pos){*x, *y}, T_ENTANGLES) &&
      !monst->status[STATUS_STUCK] &&
      !(monst->info.flags & (MONST_IMMUNE_TO_WEBS | MONST_INVULNERABLE)) &&
      !(monst->bookkeepingFlags & MB_SUBMERGED)) {

    monst->status[STATUS_STUCK] = monst->maxStatus[STATUS_STUCK] =
        rand_range(3, 7);
    if (monst == &player) {
      if (!rogue.automationActive) {
        // Don't interrupt exploration with this message.
        sprintf(
            buf2, "you are stuck fast in %s!",
            tileCatalog[pmap[*x][*y].layers[layerWithFlag(*x, *y, T_ENTANGLES)]]
                .description);
        message(buf2, 0);
      }
  ...
  // explosions
  if (cellHasTerrainFlag((pos){*x, *y}, T_CAUSES_EXPLOSIVE_DAMAGE) &&
    ...
    if (monst == &player) {
      ...
      for (layer = 0; layer < NUMBER_TERRAIN_LAYERS &&
                      !(tileCatalog[pmap[*x][*y].layers[layer]].flags &
                        T_CAUSES_EXPLOSIVE_DAMAGE);
           layer++)
        ;
      message(tileCatalog[pmap[*x][*y].layers[layer]].flavorText, 0);
      if (rogue.armor && (rogue.armor->flags & ITEM_RUNIC) &&
          rogue.armor->enchant2 == A_DAMPENING) {
        ...
        strcpy(buf2, tileCatalog[pmap[*x][*y].layers[layerWithFlag(
                                     *x, *y, T_CAUSES_EXPLOSIVE_DAMAGE)]]
                         .description);
      ...
      // Get explosive layer before damage in case a death DF replaces the
      // explosion
      strcpy(buf3, tileCatalog[pmap[*x][*y].layers[layerWithFlag(
                                   *x, *y, T_CAUSES_EXPLOSIVE_DAMAGE)]]
                       .description);
      if (inflictDamage(NULL, monst, damage, &yellow, false)) {
        // if killed
        sprintf(buf2, "%s %s %s.", buf,
                (monst->info.flags & MONST_INANIMATE) ? "is destroyed by"
                                                      : "dies in",
                buf3);
        messageWithColor(buf2, messageColorFromVictim(monst), 0);
        killCreature(monst, false);
        ...
        sprintf(buf2, "%s engulfs %s.",
                tileCatalog[pmap[*x][*y].layers[layerWithFlag(
                                *x, *y, T_CAUSES_EXPLOSIVE_DAMAGE)]]
                    .description,
                buf);
        messageWithColor(buf2, messageColorFromVictim(monst), 0);
  ...
  // Toxic gases!
  // If it's the player, and he's wearing armor of respiration, then no effect
  // from toxic gases.
  if (monst == &player &&
    ...
    if (!(rogue.armor->flags & ITEM_RUNIC_IDENTIFIED)) {
      message(
          "Your armor trembles and a pocket of clean air swirls around you.",
          0);
      autoIdentify(rogue.armor);
    }
  } else {
    ...
    if (cellHasTerrainFlag((pos){*x, *y}, T_CAUSES_NAUSEA) &&
      ...
      if (canDirectlySeeMonster(monst) && !(monst->status[STATUS_NAUSEOUS])) {
        ...
        monsterName(buf, monst, true);
        sprintf(
            buf2, "%s choke%s and gag%s on the overpowering stench of decay.",
            buf, (monst == &player ? "" : "s"), (monst == &player ? "" : "s"));
        message(buf2, 0);
      }
      monst->status[STATUS_NAUSEOUS] = monst->maxStatus[STATUS_NAUSEOUS] =
          max(monst->status[STATUS_NAUSEOUS], 20);
    }

    // confusion gas
    if (cellHasTerrainFlag((pos){*x, *y}, T_CAUSES_CONFUSION) &&
        !(monst->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE))) {
      if (monst == &player) {
        rogue.disturbed = true;
      }
        ...
        monsterName(buf, monst, true);
        {
          ...
          if (chineseUi) {
            sprintf(buf2, "%s%s！", buf,
                    (monst == &player ? "感到非常困惑" : "看起来非常困惑"));
          } else {
            sprintf(buf2, "%s %s very confused!", buf,
      ...
      monst->status[STATUS_CONFUSED] = monst->maxStatus[STATUS_CONFUSED] =
          max(monst->status[STATUS_CONFUSED], 25);
    }
    ...
    if (cellHasTerrainFlag((pos){*x, *y}, T_CAUSES_PARALYSIS) &&
      ...
      if (canDirectlySeeMonster(monst) && !monst->status[STATUS_PARALYZED]) {
        ...
        monsterName(buf, monst, true);
        {
          ...
          if (chineseUi) {
            ...
            sprintf(buf2, "%s %s paralyzed!", buf,
                    (monst == &player ? "are" : "is"));
          }
      ...
      monst->status[STATUS_PARALYZED] = monst->maxStatus[STATUS_PARALYZED] =
          max(monst->status[STATUS_PARALYZED], 20);
      if (monst == &player) {
        rogue.disturbed = true;
      }
    }
  ...
  if (cellHasTerrainFlag((pos){*x, *y}, T_CAUSES_POISON) &&
    ...
    addPoison(monst, damage,
              0); // Lichen doesn't increase poison concentration above 1.
  }
  ...
  if (cellHasTerrainFlag((pos){*x, *y}, T_IS_FIRE)) {
    exposeCreatureToFire(monst);
             ...
             // We should only expose to fire if it is flammable and not on
             // fire. However, when gas burns, it only sets the volume to 0 and
             // doesn't clear the layer (for visual reasons). This can cause
             // crashes if the fire tile fails to spawn, so we also exclude it.
  ...
  // keys
  if (cellHasTMFlag((pos){*x, *y}, TM_PROMOTES_WITH_KEY) &&
      (theItem = keyOnTileAt((pos){*x, *y}))) {
...
static void applyGradualTileEffectsToCreature(creature *monst, short ticks) {
  ...
  if (!(monst->status[STATUS_LEVITATING]) &&
    ...
    if (monst == &player) {
      if (!(pmap[x][y].flags & HAS_ITEM) && rand_percent(ticks * 50 / 100)) {
        itemCandidates =
            numberOfMatchingPackItems(ALL_ITEMS, 0, (ITEM_EQUIPPED), false);
        if (itemCandidates) {
          ...
          if (theItem) {
            ...
            sprintf(buf, "%s float%s away in the current!", buf2,
                    (theItem->quantity == 1 ? "s" : ""));
            messageWithColor(buf, &itemMessageColor, 0);
          }
        }
      }
    } else if (monst->carriedItem && !(pmap[x][y].flags & HAS_ITEM) &&
               rand_percent(ticks * 50 / 100)) { // it's a monster with an item
  ...
  if (cellHasTerrainFlag((pos){x, y}, T_CAUSES_DAMAGE) &&
    ...
    damage = max(1, damage);
    for (layer = 0;
         layer < NUMBER_TERRAIN_LAYERS &&
         !(tileCatalog[pmap[x][y].layers[layer]].flags & T_CAUSES_DAMAGE);
         layer++)
      ;
    if (monst == &player) {
      if (rogue.armor && (rogue.armor->flags & ITEM_RUNIC) &&
          rogue.armor->enchant2 == A_RESPIRATION) {
        if (!(rogue.armor->flags & ITEM_RUNIC_IDENTIFIED)) {
          message("Your armor trembles and a pocket of clean air swirls around "
                  "you.",
                  0);
          autoIdentify(rogue.armor);
        }
        ...
        rogue.disturbed = true;
        messageWithColor(tileCatalog[pmap[x][y].layers[layer]].flavorText,
                         &badMessageColor, 0);
        if (inflictDamage(NULL, &player, damage,
          ...
          sprintf(buf, "Killed by %s",
                  tileCatalog[pmap[x][y].layers[layer]].description);
          gameOver(buf, true);
          return;
        }
      }
...
void updateClairvoyance() {
  ...
  for (i = max(0, player.loc.x - clairvoyanceRadius);
       i < min(DCOLS, player.loc.x + clairvoyanceRadius + 1); i++) {
    for (j = max(0, player.loc.y - clairvoyanceRadius);
         j < min(DROWS, player.loc.y + clairvoyanceRadius + 1); j++) {

      dx = (player.loc.x - i);
      dy = (player.loc.y - j);

      if (dx * dx + dy * dy <
              clairvoyanceRadius * clairvoyanceRadius + clairvoyanceRadius &&
...
static void updateTelepathy() {
  ...
  for (creatureIterator it = iterateCreatures(monsters); hasNextCreature(it);) {
    ...
    if (monsterRevealed(monst)) {
      getFOVMask(grid, monst->loc.x, monst->loc.y, 2 * FP_FACTOR,
                 T_OBSTRUCTS_VISION, 0, false);
      pmapAt(monst->loc)->flags |= TELEPATHIC_VISIBLE;
      discoverCell(monst->loc.x, monst->loc.y);
    }
  }
  for (creatureIterator it = iterateCreatures(dormantMonsters);
       hasNextCreature(it);) {
    creature *monst = nextCreature(&it);
    if (monsterRevealed(monst)) {
      getFOVMask(grid, monst->loc.x, monst->loc.y, 2 * FP_FACTOR,
...
static void updateScent() {
  ...
  getFOVMask(grid, player.loc.x, player.loc.y, DCOLS * FP_FACTOR,
             T_OBSTRUCTS_SCENT, 0, false);
...
short currentStealthRange() {
  ...
  if (player.status[STATUS_INVISIBLE]) {
    ...
    if (pmapAt(player.loc)->flags & IS_IN_SHADOW) {
      // When not standing in a lit area, halve, rounded down (stacks with
      // darkness halving).
      stealthRange = stealthRange / 2;
    }

    // Add 1 for each point of your armor's natural (unenchanted) strength
    // requirement above 12.
...
void updateVision(boolean refreshDisplay) {
  ...
  // Calculate player's field of view (distinct from what is visible, as
  // lighting hasn't been done yet).
  zeroOutGrid(grid);
  getFOVMask(grid, player.loc.x, player.loc.y, (DCOLS + DROWS) * FP_FACTOR,
             (T_OBSTRUCTS_VISION), 0, false);
  for (i = 0; i < DCOLS; i++) {
    for (j = 0; j < DROWS; j++) {
      if (grid[i][j]) {
  ...
  //      for (j=0; j<DROWS; j++) {
  //          if (pmap[i][j].flags & VISIBLE) {
  //              plotCharWithColor(' ', mapToWindow((pos){ i, j }), &yellow,
  //              &yellow);
  //          } else if (pmap[i][j].flags & IN_FIELD_OF_VIEW) {
  //              plotCharWithColor(' ', mapToWindow((pos){ i, j }), &blue,
  //              &blue);
  //          }
  //      }
  //  }
  ...
  if (player.status[STATUS_HALLUCINATING] > 0) {
    for (theItem = floorItems->nextItem; theItem != NULL;
         theItem = theItem->nextItem) {
...
static void checkNutrition() {
  ...
  if (player.status[STATUS_NUTRITION] ==
      1) { // Didn't manage to eat any food above.
    player.status[STATUS_NUTRITION] =
        0; // So the status bar changes in time for the message:
...
void burnItem(item *theItem) {
  ...
  spawnDungeonFeature(x, y, &(dungeonFeatureCatalog[DF_ITEM_FIRE]), true,
                      false);
}

static void flashCreatureAlert(creature *monst, char msg[200],
  ...
  flashMessage(msg, x, y, (rogue.playbackMode ? 100 : 1000), foreColor,
               backColor);
  rogue.disturbed = true;
...
static void handleHealthAlerts() {
  short i, currentPercent, previousPercent, thresholds[] = {5, 10, 25, 40},
                                            pThresholds[] = {100, 90, 50};
  char buf[DCOLS];

  const short healthThresholdsCount = 4, poisonThresholdsCount = 3;
  ...
  if (!rogue.gameHasEnded) {
    currentPercent = player.status[STATUS_POISONED] * player.poisonAmount *
                     100 / player.currentHP;

    if (currentPercent > rogue.previousPoisonPercent && !rogue.gameHasEnded) {
      for (i = 0; i < poisonThresholdsCount; i++) {
        if (currentPercent > pThresholds[i] &&
            rogue.previousPoisonPercent <= pThresholds[i]) {
...
/// @brief Add experience to the given monster. Allies gain experience when the
/// player discovers new pathable tiles.
/// @param monst The ally that gains experience
static void addXPXPToAlly(creature *monst) {
  if (!(monst->creatureState == MONSTER_ALLY) ||
      monst->info.flags & (MONST_INANIMATE | MONST_IMMOBILE)) {
    return;
  }
  ...
  if (!(monst->bookkeepingFlags & MB_TELEPATHICALLY_REVEALED) &&
    ...
    sprintf(buf, "you have developed a telepathic bond with your %s.",
            theMonsterName);
    messageWithColor(buf, &advancementMessageColor, 0);
  }

  // Companion feat
  if (!(rogue.featRecord[FEAT_COMPANION]) &&
      monst->xpxp >= gameConst->companionFeatRequiredXP) {
...
static void handleXPXP() {
  ...
  if (rogue.depthLevel > 1) {
    for (creatureIterator it =
             iterateCreatures(&levels[rogue.depthLevel - 2].monsters);
         hasNextCreature(it);) {
      creature *monst = nextCreature(&it);
      addXPXPToAlly(monst);
    }
  ...
  if (rogue.depthLevel < gameConst->deepestLevel) {
    for (creatureIterator it =
             iterateCreatures(&levels[rogue.depthLevel].monsters);
         hasNextCreature(it);) {
      creature *monst = nextCreature(&it);
      addXPXPToAlly(monst);
    }
...
static void playerFalls() {
  ...
  monstersFall();     // Monsters must fall with the player rather than getting
                      // suspended on the previous level.
  updateFloorItems(); // Likewise, items should fall with the player rather than
                      // getting suspended above.

  layer = layerWithFlag(player.loc.x, player.loc.y, T_AUTO_DESCENT);
  if (layer >= 0) {
    message(tileCatalog[pmapAt(player.loc)->layers[layer]].flavorText,
  ...
  if (rogue.depthLevel < gameConst->deepestLevel) {
    ...
    damage =
        randClumpedRange(gameConst->fallDamageMin, gameConst->fallDamageMax, 2);
    boolean killed = false;
    if (terrainFlags(player.loc) & T_IS_DEEP_WATER) {
      messageWithColor("You fall into deep water, unharmed.", &badMessageColor,
                       0);
    } else {
      if (cellHasTMFlag(player.loc, TM_ALLOWS_SUBMERGING)) {
        damage /= 2; // falling into liquid (shallow water, bog, etc.) hurts
                     // less than hitting hard floor
      }
...
void activateMachine(short machineNumber) {
  short i, j, x, y, layer, sRows[DROWS], sCols[DCOLS], monsterCount,
      maxMonsters;
    ...
    if (monst->machineHome == machineNumber &&
      ...
      if (monsterCount > maxMonsters) {
        ...
        activatedMonsterList =
            realloc(activatedMonsterList, sizeof(creature *) * maxMonsters);
      }
...
void promoteTile(short x, short y, enum dungeonLayers layer,
                 boolean useFireDF) {
  short i, j;
  ...
  if ((tile->mechFlags & TM_VANISHES_UPON_PROMOTION)) {
    ...
    pmap[x][y].layers[layer] =
        (layer == DUNGEON ? FLOOR
                          : NOTHING); // even the dungeon layer implicitly has
                                      // floor underneath it
    if (layer == GAS) {
      pmap[x][y].volume = 0;
    }
...
boolean exposeTileToFire(short x, short y, boolean alwaysIgnite) {
  ...
  if (!cellHasTerrainFlag((pos){x, y}, T_IS_FLAMMABLE) ||
      pmap[x][y].exposedToFire >= 12) {
    return false;
  }
  ...
  for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++) {
    if ((tileCatalog[pmap[x][y].layers[layer]].mechFlags &
        ...
        tileCatalog[pmap[x][y].layers[layer]].drawPriority <
  ...
  // Pick the fire type of the most flammable layer that is either gas or
  // equal-or-better priority than the best extinguishing layer.
  for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++) {
    if ((tileCatalog[pmap[x][y].layers[layer]].flags & T_IS_FLAMMABLE) &&
        (layer == GAS || tileCatalog[pmap[x][y].layers[layer]].drawPriority <=
                             bestExtinguishingPriority) &&
  ...
  if (alwaysIgnite ||
      (ignitionChance && rand_percent(ignitionChance))) { // If it ignites...
    fireIgnited = true;
      ...
      for (dir = 0, explosiveNeighborCount = 0; dir < DIRECTION_COUNT; dir++) {
        ...
        if (coordinatesAreInMap(newX, newY) &&
            (cellHasTerrainFlag((pos){newX, newY},
                                T_IS_FIRE | T_OBSTRUCTS_GAS) ||
             cellHasTMFlag((pos){newX, newY}, TM_EXPLOSIVE_PROMOTE))) {
    ...
    for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++) {
      if (tileCatalog[pmap[x][y].layers[layer]].flags & T_IS_FLAMMABLE) {
        ...
        // We preserve the layer anyways because this results in nicer gas
        // burning behavior.
        if (layer == GAS) {
    ...
    for (j = 0; j < DROWS; j++) {
      if (!cellHasTerrainFlag((pos){i, j}, T_OBSTRUCTS_GAS)) {
        ...
        if (cellHasTerrainFlag(
                (pos){i, j},
                T_AUTO_DESCENT)) { // if it's a chasm tile or trap door,
        ...
        if (pmap[i][j].layers[GAS] != gasType && newGasVolume[i][j] > 3) {
          if (pmap[i][j].layers[GAS] != NOTHING) {
            newGasVolume[i][j] =
                min(3, newGasVolume[i][j]); // otherwise interactions between
                                            // gases are crazy
          }
        ...
        if (pmap[i][j].volume > 0) {
          if (tileCatalog[pmap[i][j].layers[GAS]].mechFlags &
              TM_GAS_DISSIPATES_QUICKLY) {
            newGasVolume[i][j] -= (rand_percent(50) ? 1 : 0);
          } else if (tileCatalog[pmap[i][j].layers[GAS]].mechFlags &
                     TM_GAS_DISSIPATES) {
...
static void updateYendorWardenTracking() {
  ...
  if (!(rogue.yendorWarden->bookkeepingFlags & MB_PREPLACED)) {
    levels[rogue.yendorWarden->depth - 1]
        .mapStorage[rogue.yendorWarden->loc.x][rogue.yendorWarden->loc.y]
        .flags &= ~HAS_MONSTER;
  }
  ...
  prependCreature(&levels[rogue.yendorWarden->depth - 1].monsters,
                  rogue.yendorWarden);
  rogue.yendorWarden->bookkeepingFlags |= MB_PREPLACED;
  rogue.yendorWarden->status[STATUS_ENTERS_LEVEL_IN] = 50;
}

// Monsters who are over chasms or other descent tiles won't fall until this is
// called. This is to avoid having the monster chain change unpredictably in the
// middle of a turn.
void monstersFall() {
  short x, y;
  ...
  for (creatureIterator it = iterateCreatures(monsters); hasNextCreature(it);) {
    ...
    if ((monst->bookkeepingFlags & MB_IS_FALLING) || monsterShouldFall(monst)) {
      ...
      if (monst->info.flags & MONST_GETS_TURN_ON_ACTIVATION) {
        // Guardians and mirrored totems never survive the fall. If they did,
        // they might block the level below.
        killCreature(monst, false);
...
void updateEnvironment() {
  ...
  // First pass, make a note of each terrain layer at each coordinate that is
  // going to promote:
  for (i = 0; i < DCOLS; i++) {
    for (j = 0; j < DROWS; j++) {
      promotions[i][j] = 0;
      for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++) {
        ...
        if (tile->promoteChance < 0) {
          ...
          for (direction = 0; direction < 4; direction++) {
            if (coordinatesAreInMap(i + nbDirs[direction][0],
                                    j + nbDirs[direction][1]) &&
                !cellHasTerrainFlag(
                    (pos){i + nbDirs[direction][0], j + nbDirs[direction][1]},
                    T_OBSTRUCTS_PASSABILITY) &&
                pmap[i + nbDirs[direction][0]][j + nbDirs[direction][1]]
                        .layers[layer] != pmap[i][j].layers[layer] &&
        ...
        if (promoteChance && !(pmap[i][j].flags & CAUGHT_FIRE_THIS_TURN) &&
  ...
  for (i = 0; i < DCOLS; i++) {
    for (j = 0; j < DROWS; j++) {
      ...
      if (cellHasTMFlag((pos){i, j}, TM_PROMOTES_WITHOUT_KEY) &&
          !keyOnTileAt((pos){i, j})) {
        for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++) {
          if (tileCatalog[pmap[i][j].layers[layer]].mechFlags &
  ...
  for (i = 0; i < DCOLS; i++) {
    for (j = 0; j < DROWS; j++) {
      if (cellHasTerrainFlag((pos){i, j}, T_IS_FIRE) &&
          !(pmap[i][j].flags & CAUGHT_FIRE_THIS_TURN)) {
...
void updateAllySafetyMap() {
  ...
  for (i = 0; i < DCOLS; i++) {
    for (j = 0; j < DROWS; j++) {
      ...
      } else if ((pmap[i][j].flags & HAS_MONSTER) &&
                 monstersAreEnemies(&player, monsterAtLoc((pos){i, j}))) {
        playerCostMap[i][j] = 1;
        monsterCostMap[i][j] = PDS_FORBIDDEN;
        allySafetyMap[i][j] = 0;
      }
  ...
  for (i = 0; i < DCOLS; i++) {
    for (j = 0; j < DROWS; j++) {
      ...
      allySafetyMap[i][j] =
          50 * allySafetyMap[i][j] / (50 + allySafetyMap[i][j]);
  ...
  for (i = 0; i < DCOLS; i++) {
    for (j = 0; j < DROWS; j++) {
      ...
      if (cellHasTerrainFlag((pos){i, j}, T_OBSTRUCTS_PASSABILITY) &&
        ...
        } else if (cellHasTerrainFlag((pos){i, j}, (T_IS_DEEP_WATER |
  ...
  for (i = 0; i < DCOLS; i++) {
    for (j = 0; j < DROWS; j++) {
      if (cellHasTerrainFlag((pos){i, j}, T_OBSTRUCTS_PASSABILITY) &&
        ...
        // Secret doors that the player can't see are not particularly safe
        // themselves; the areas behind them are.
...
void updateSafeTerrainMap() {
  ...
  for (i = 0; i < DCOLS; i++) {
    for (j = 0; j < DROWS; j++) {
      ...
      } else if (cellHasTerrainFlag((pos){i, j}, T_HARMFUL_TERRAIN) ||
                 pmap[i][j].layers[DUNGEON] == DOOR) {
        // The door thing is an aesthetically offensive but necessary hack to
        // make sure that monsters trying to find their way out of caustic gas
        // do not sprint for the doors. Doors are superficially free of gas, but
        // as soon as they are opened, gas will fill their tile, so they are not
        // actually safe. Without this fix, allies will fidget back and forth in
        // a doorway while they asphyxiate. This will have to do. It's a
        // difficult problem to solve elegantly.
        costMap[i][j] = 1;
        rogue.mapToSafeTerrain[i][j] = 30000;
      } else {
        costMap[i][j] = 1;
...
static void processIncrementalAutoID() {
  item *theItem,
      *autoIdentifyItems[3] = {rogue.armor, rogue.ringLeft, rogue.ringRight};
  char buf[DCOLS * 3], theItemName[DCOLS * 3];
  short i;

  for (i = 0; i < 3; i++) {
    theItem = autoIdentifyItems[i];
    if (theItem && theItem->charges > 0 &&
        (!(theItem->flags & ITEM_IDENTIFIED) ||
         ((theItem->category & RING) &&
          !ringTable[theItem->kind].identified))) {

      theItem->charges--;
      if (theItem->charges <= 0) {
        itemName(theItem, theItemName, false, false, NULL);
        sprintf(buf, "you are now familiar enough with your %s to identify it.",
                theItemName);
        messageWithColor(buf, &itemMessageColor, 0);

        if (theItem->category & ARMOR) {
          // Don't necessarily reveal the armor's runic specifically, just that
          // it has one.
          theItem->flags |= ITEM_IDENTIFIED;
        ...
        sprintf(buf, "%s %s.", (theItem->quantity > 1 ? "they are" : "it is"),
                theItemName);
        messageWithColor(buf, &itemMessageColor, 0);
      }
    }
...
// Multiplier can be negative, in which case staffs and charms will be drained
...
static void monsterEntersLevel(creature *monst, short n) {
  ...
  if (!pit && (pmapAt(monst->loc)->flags & (HAS_PLAYER | HAS_MONSTER)) &&
      !(terrainFlags(monst->loc) & avoidedFlagsForMonster(&(monst->info)))) {
    // Monsters using the stairs will displace any creatures already located
    // there, to thwart stair-dancing.
    creature *prevMonst = monsterAtLoc(monst->loc);
    brogueAssert(prevMonst);
    prevMonst->loc = getQualifyingPathLocNear(
        monst->loc, true,
        T_DIVIDES_LEVEL & avoidedFlagsForMonster(&(prevMonst->info)), 0,
        avoidedFlagsForMonster(&(prevMonst->info)),
        (HAS_MONSTER | HAS_PLAYER | HAS_STAIRS), false);
    pmapAt(monst->loc)->flags &= ~(HAS_PLAYER | HAS_MONSTER);
    pmapAt(prevMonst->loc)->flags |=
        (prevMonst == &player ? HAS_PLAYER : HAS_MONSTER);
    refreshDungeonCell(prevMonst->loc);
    // DEBUG printf("\nBumped a creature (%s) from (%i, %i) to (%i, %i).",
    // prevMonst->info.monsterName, monst->loc.x, monst->loc.y,
    // prevMonst->loc.x, prevMonst->loc.y);
  }
  ...
  // DEBUG printf("\nPlaced a creature (%s) at (%i, %i).",
  // monst->info.monsterName, monst->loc.x, monst->loc.y);
  ...
  if (pit) {
    ...
    if (!monst->status[STATUS_LEVITATING]) {
      if (inflictDamage(NULL, monst, randClumpedRange(6, 12, 2), &red, false)) {
        if (canSeeMonster(monst)) {
          sprintf(buf,
                  "%s plummets from above and splatters against the ground!",
                  monstName);
        ...
        if (canSeeMonster(monst)) {
          sprintf(buf, "%s falls from above and crashes to the ground!",
                  monstName);
          message(buf, 0);
        }
      }
...
static void monstersApproachStairs() {
  ...
  for (n = rogue.depthLevel - 2; n <= rogue.depthLevel;
       n += 2) { // cycle through previous and next level
    if (n >= 0 && n < gameConst->deepestLevel && levels[n].visited) {
      for (creatureIterator it = iterateCreatures(&levels[n].monsters);
           hasNextCreature(it);) {
        creature *monst = nextCreature(&it);
...
static void decrementPlayerStatus() {
  ...
  if (player.status[STATUS_LEVITATING] > 0 &&
      !--player.status[STATUS_LEVITATING]) {
  ...
  if (player.status[STATUS_WEAKENED] > 0 && !--player.status[STATUS_WEAKENED]) {
    ...
    message(
        "strength returns to your muscles as the weakening toxin wears off.",
        0);
    updateEncumbrance();
  }
  ...
  if (player.status[STATUS_IMMUNE_TO_FIRE] > 0 &&
      !--player.status[STATUS_IMMUNE_TO_FIRE]) {
    message("you no longer feel immune to fire.", 0);
  }

  if (player.status[STATUS_STUCK] &&
      !cellHasTerrainFlag(player.loc, T_ENTANGLES)) {
  ...
  if (player.status[STATUS_INVISIBLE] > 0 &&
      !--player.status[STATUS_INVISIBLE]) {
...
void autoRest() {
  ...
  const boolean initiallyEmbedded =
      cellHasTerrainFlag(player.loc, T_OBSTRUCTS_PASSABILITY);

  if ((player.currentHP < player.info.maxHP ||
       player.status[STATUS_HALLUCINATING] || player.status[STATUS_CONFUSED] ||
       player.status[STATUS_NAUSEOUS] || player.status[STATUS_POISONED] ||
       player.status[STATUS_DARKNESS] || initiallyEmbedded) &&
      !rogue.disturbed) {
    ...
    while (i++ < TURNS_FOR_FULL_REGEN &&
           (player.currentHP < player.info.maxHP ||
            player.status[STATUS_HALLUCINATING] ||
            player.status[STATUS_CONFUSED] || player.status[STATUS_NAUSEOUS] ||
            player.status[STATUS_POISONED] || player.status[STATUS_DARKNESS] ||
            cellHasTerrainFlag(player.loc, T_OBSTRUCTS_PASSABILITY)) &&
           !rogue.disturbed &&
           (!initiallyEmbedded ||
            cellHasTerrainFlag(player.loc, T_OBSTRUCTS_PASSABILITY))) {
...
// Call this periodically (when haste/slow wears off and when moving between
// depths) to keep environmental updates in sync with player turns.
...
void playerRecoversFromAttacking(boolean anAttackHit) {
  if (player.ticksUntilTurn >= 0) {
    ...
    if (rogue.weapon && (rogue.weapon->flags & ITEM_ATTACKS_STAGGER) &&
        anAttackHit) {
...
static void recordCurrentCreatureHealths() {
  ...
  for (creatureIterator it = iterateCreatures(monsters);
       !handledPlayer || hasNextCreature(it);) {
...
// This is the dungeon schedule manager, called every time the player's turn
// comes to an end. It hands control over to monsters until they've all expended
// their accumulated ticks, updating the environment (gas spreading, flames
// spreading and burning out, etc.) every 100 ticks.
void playerTurnEnded() {
  ...
  do {
    ...
    if (!player.status[STATUS_PARALYZED]) {
      rogue.playerTurnNumber++; // So recordings don't register more turns than
                                // you actually have.
    }
    ...
    if (player.status[STATUS_INVISIBLE]) {
      rogue.scentTurnNumber +=
          10; // Your scent fades very quickly while you are invisible.
    } else {
    ...
    if (player.status[STATUS_NUTRITION] <= 0) {
      ...
      if ((player.turnsUntilRegen -= 1000) <= 0) {
        ...
        if (player.previousHealthPoints < player.currentHP) {
          player.previousHealthPoints++; // Regeneration doesn't display on the
                                         // status bar.
        }
      ...
      if (player.regenPerTurn) {
        ...
        if (player.previousHealthPoints < player.currentHP) {
          player.previousHealthPoints =
              min(player.currentHP,
                  player.previousHealthPoints + player.regenPerTurn);
        }
      }
    }
    ...
    for (creatureIterator it = iterateCreatures(monsters);
      ...
      if ((monst->bookkeepingFlags & MB_BOUND_TO_LEADER) &&
        ...
        if (canSeeMonster(monst)) {
          ...
          {
            boolean chineseUi2 = strcmp(tr("Health"), "Health") != 0;
            if (chineseUi2) {
              sprintf(buf, "%s消散于无形", buf2);
            } else {
    ...
    if (player.status[STATUS_BURNING] > 0) {
      ...
      if (!(player.status[STATUS_IMMUNE_TO_FIRE]) &&
          inflictDamage(NULL, &player, damage, &orange, true)) {
        killCreature(&player, false);
    ...
    for (creatureIterator it = iterateCreatures(monsters);
         hasNextCreature(it);) {
      creature *monst = nextCreature(&it);
      if (D_SAFETY_VISION || monst->creatureState == MONSTER_FLEEING &&
                                 pmapAt(monst->loc)->flags & IN_FIELD_OF_VIEW) {
        updateSafetyMap(); // only if there is a fleeing monster who can see the
                           // player
        break;
      }
    }
    ...
    while (player.ticksUntilTurn > 0) {
      ...
      if (rogue.ticksTillUpdateEnvironment <= 0) {
        ...
        // stuff that happens periodically according to an objective time
        // measurement goes here:
        rechargeItemsIncrementally(1); // staffs recharge every so often
        processIncrementalAutoID(); // become more familiar with worn armor and
                                    // rings
        ...
        for (creatureIterator it = iterateCreatures(monsters);
          ...
          if (monst->info.DFChance &&
            ...
            spawnDungeonFeature(monst->loc.x, monst->loc.y,
                                &dungeonFeatureCatalog[monst->info.DFType],
                                true, false);
          }
        }

        updateEnvironment(); // Update fire and gas, items floating around in
                             // water, monsters falling into chasms, etc.
      ...
      for (creatureIterator it = iterateCreatures(monsters);
           hasNextCreature(it) && rogue.gameHasEnded == false;) {
        creature *monst = nextCreature(&it);
        if (monst->ticksUntilTurn <= 0) {
          ...
          for (creatureIterator it2 = iterateCreatures(monsters);
               hasNextCreature(it2);) {
    ...
    for (creatureIterator it = iterateCreatures(monsters);
      ...
      creature *monst = nextCreature(&it);
      if (canSeeMonster(monst) &&
          !(monst->bookkeepingFlags & (MB_WAS_VISIBLE | MB_ALREADY_SEEN))) {
        if (monst->creatureState != MONSTER_ALLY) {
          rogue.disturbed = true;
          if (rogue.cautiousMode || rogue.automationActive) {
            ...
            if (strcmp(tr("Health"), "Health") != 0) {
              sprintf(buf, "你%s了%s",
                      playerCanDirectlySee(monst->loc.x, monst->loc.y)
                          ? "看到"
                          : "感知到",
                      buf2);
            } else {
            ...
            if (rogue.cautiousMode) {
              strcat(buf, strcmp(tr("Health"), "Health") != 0 ? "。" : ".");
              message(buf, REQUIRE_ACKNOWLEDGMENT);
            } else {
              combatMessage(buf, 0);
            }
      ...
      if (!canSeeMonster(monst) && (monst->bookkeepingFlags & MB_WAS_VISIBLE) &&
          !(monst->bookkeepingFlags & MB_CAPTIVE)) {
        // For captives we never unset MB_WAS_VISIBLE because captives are not
        // moving, so we don't want to get "You see a ..." every time they come
        // back into view.
  ...
  if ((player.status[STATUS_LEVITATING] &&
       cellHasTerrainFlag(player.loc, T_LAVA_INSTA_DEATH | T_IS_DEEP_WATER |
                                          T_AUTO_DESCENT)) ||
      (player.status[STATUS_IMMUNE_TO_FIRE] &&
       cellHasTerrainFlag(player.loc, T_LAVA_INSTA_DEATH))) {
    if (!rogue.receivedLevitationWarning) {
      turnsRequiredToShore = rogue.mapToShore[player.loc.x][player.loc.y] *
                             player.movementSpeed / 100;
      if (cellHasTerrainFlag(player.loc, T_LAVA_INSTA_DEATH)) {
        turnsToShore = max(player.status[STATUS_LEVITATING],
                           player.status[STATUS_IMMUNE_TO_FIRE]) *
                       100 / player.movementSpeed;
      } else {
        turnsToShore =
            player.status[STATUS_LEVITATING] * 100 / player.movementSpeed;
      }
      if (turnsRequiredToShore == turnsToShore ||
          turnsRequiredToShore + 1 == turnsToShore) {
...
void resetScentTurnNumber() { // don't want player.scentTurnNumber to roll over
                              // the short maxint!
  short i, j, d;
