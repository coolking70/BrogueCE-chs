...
static void initializeMenuButtons(buttonState *state, brogueButton buttons[5]) {
    ...
    if (rogue.playbackMode) {
        if (KEYBOARD_LABELS) {
            sprintf(buttons[buttonCount].text,  tr(" Unpause (%sspace%s) "), goldTextEscape, whiteTextEscape);
        } else {
            strcpy(buttons[buttonCount].text,   tr("     Unpause     "));
        }
        buttons[buttonCount].hotkey[0] = ACKNOWLEDGE_KEY;
        buttonCount++;

        if (KEYBOARD_LABELS) {
            sprintf(buttons[buttonCount].text,  tr("Omniscience (%stab%s)"), goldTextEscape, whiteTextEscape);
        } else {
            strcpy(buttons[buttonCount].text,   tr("   Omniscience   "));
        }
        buttons[buttonCount].hotkey[0] = TAB_KEY;
        buttonCount++;

        if (KEYBOARD_LABELS) {
            sprintf(buttons[buttonCount].text,  tr(" Next Turn (%sl%s) "), goldTextEscape, whiteTextEscape);
        } else {
            strcpy(buttons[buttonCount].text,   tr("   Next Turn   "));
        }
        buttons[buttonCount].hotkey[0] = RIGHT_KEY;
        buttons[buttonCount].hotkey[1] = RIGHT_ARROW;
        buttonCount++;

        strcpy(buttons[buttonCount].text,       tr("  Menu  "));
        buttonCount++;
    } else {
        sprintf(buttons[buttonCount].text,  tr("   E%sx%splore   "), goldTextEscape, whiteTextEscape);
        buttons[buttonCount].hotkey[0] = EXPLORE_KEY;
        buttons[buttonCount].hotkey[1] = 'X';
        buttonCount++;

        if (KEYBOARD_LABELS) {
            sprintf(buttons[buttonCount].text,  tr("   Rest (%sz%s)   "), goldTextEscape, whiteTextEscape);
        } else {
            strcpy(buttons[buttonCount].text,   tr("     Rest     "));
        }
        buttons[buttonCount].hotkey[0] = REST_KEY;
        buttonCount++;

        if (KEYBOARD_LABELS) {
            sprintf(buttons[buttonCount].text,  tr("  Search (%ss%s)  "), goldTextEscape, whiteTextEscape);
        } else {
            strcpy(buttons[buttonCount].text,   tr("    Search    "));
        }
    ...
    sprintf(buttons[4].text,    tr("   %sI%snventory   "), goldTextEscape, whiteTextEscape);
...
void messageWithColor(const char *msg, const color *theColor, unsigned long flags) {
    char buf[COLS*2] = "";
    short i;
    const char *translated = tr(msg);

    i=0;
    i = encodeMessageColor(buf, i, theColor);
    strcpy(&(buf[i]), translated);
    message(buf, flags);
}
...
void flavorMessage(const char *msg) {
    ...
    for (i=0; i < COLS*2 && translated[i] != '\0'; i++) {
        text[i] = translated[i];
    }
    text[i] = '\0';
...
void message(const char *msg, unsigned long flags) {
    ...
    for (i = 1; i < MESSAGE_ARCHIVE_ENTRIES; i++) {
        ...
        if (strcmp(archiveEntry->message, translatedMsg) == 0) {
    ...
    if (newMessage) {
        ...
        strcpy(archiveEntry->message, translatedMsg);
...
void updateMessageDisplay() {
    ...
    for (i=0; i<MESSAGE_LINES; i++) {
        ...
        for (j = m = 0; displayedMessage[i][m] && j < DCOLS; ) {
            ...
            if (!displayedMessage[i][m]) {
                break;
            }

            cp = decodeUtf8Codepoint(&displayedMessage[i][m], &size);
            width = unicodeDisplayWidth(cp);
            if (width <= 0) {
                m += size;
                continue;
            }
            ...
            plotCharWithColor((enum displayGlyph) cp, (windowpos){ mapToWindowX(j), MESSAGE_LINES - i - 1 },
                              &messageColor,
                              &black);
            if (cp >= 128 && width == 2 && j + 1 < DCOLS) {
                plotCharWithColor(' ', (windowpos){ mapToWindowX(j + 1), MESSAGE_LINES - i - 1 }, &messageColor, &black);
            }
            j += width;
            m += size;
        }
...
void refreshSideBar(short focusX, short focusY, boolean focusedEntityMustGoFirst) {
    ...
    if (gotFocusedEntityOnScreen) {
        ...
        sprintf(buf, tr("  -- Depth: %i --%s   "), rogue.depthLevel, (rogue.depthLevel < 10 ? " " : ""));
...
static short utf8CodepointSize(const char *s) {
    ...
    if (s[1] == '\0') return 1;
    if ((c & 0xE0) == 0xC0 && (s[1] & 0xC0) == 0x80) return 2;
    if (s[2] == '\0') return 1;
    if ((c & 0xF0) == 0xE0 && (s[1] & 0xC0) == 0x80 && (s[2] & 0xC0) == 0x80) return 3;
    if (s[3] == '\0') return 1;
    if ((c & 0xF8) == 0xF0 && (s[1] & 0xC0) == 0x80 && (s[2] & 0xC0) == 0x80 && (s[3] & 0xC0) == 0x80) return 4;
    return 1;
}
...
static void plotCodepointToBuffer(unsigned int cp, windowpos loc, short width, const color *foreColor, const color *backColor, screenDisplayBuffer *dbuf) {
    plotCharToBuffer((enum displayGlyph) cp, loc, foreColor, backColor, dbuf);
    if (cp >= 128 && width == 2 && loc.window_x + 1 < COLS) {
        // Reserve the second column for full-width glyphs.
        plotCharToBuffer(' ', (windowpos){ loc.window_x + 1, loc.window_y }, foreColor, backColor, dbuf);
    }
}
...
void printString(const char *theString, short x, short y, const color *foreColor, const color *backColor, screenDisplayBuffer *dbuf) {
    short i = 0, size, width;
    unsigned int cp;
    color fColor = *foreColor;
    const char *rendered = tr(theString);

    while (rendered[i] != '\0' && x < COLS) {
        while (rendered[i] == COLOR_ESCAPE) {
            i = decodeMessageColor(rendered, i, &fColor);
            if (!rendered[i]) {
                return;
            }
        }

        cp = decodeUtf8Codepoint(&rendered[i], &size);
        width = unicodeDisplayWidth(cp);
        if (width <= 0) {
            i += size;
            continue;
        ...
        plotCodepointToBuffer(cp, (windowpos){ x, y }, width, &fColor, backColor, dbuf);
        x += width;
        i += size;
    }
}
...
static void breakUpLongWordsIn(char *sourceText, short width, boolean useHyphens) {
    ...
    short i, m, nextChar, size, wordWidth, charWidth;
    unsigned int cp;
    ...
    for (i=0, m=0; sourceText[i] != 0;) {
        if (sourceText[i] == COLOR_ESCAPE) {
            ...
            cp = decodeUtf8Codepoint(&sourceText[i], &size);
            charWidth = unicodeDisplayWidth(cp);
            if (!charWidth) {
                charWidth = 1;
            }

            if (!useHyphens && wordWidth + charWidth > width) {
                buf[m++] = '\n';
                wordWidth = 0;
            } else if (useHyphens && wordWidth + charWidth > width) {
                nextChar = i + size;
            ...
            memcpy(&(buf[m]), &(sourceText[i]), size);
            m += size;
            i += size;
            wordWidth += charWidth;
        }
...
short wrapText(char *to, const char *sourceText, short width) {
    short i, w, textLength, lineCount, size;
    ...
    while (i < textLength) {
        ...
        for (w = i + 1; w < textLength && printString[w] != ' ' && printString[w] != '\n';) {
            if (printString[w] == COLOR_ESCAPE) {
                ...
                cp = decodeUtf8Codepoint(&printString[w], &size);
                wordWidth += unicodeDisplayWidth(cp);
                w += size;
            }
        }
...
short printStringWithWrapping(const char *theString, short x, short y, short width, const color *foreColor,
    ...
    color fColor;
    char printString[TEXT_MAX_LENGTH];
    const char *translated = tr(theString);
    short i, px, py, size, glyphWidth;
    unsigned int cp;

    wrapText(printString, translated, width); // inserts newlines as necessary
    ...
    for (i=0; printString[i] != '\0'; i++) {
        ...
        cp = decodeUtf8Codepoint(&printString[i], &size);
        glyphWidth = unicodeDisplayWidth(cp);
        if (!glyphWidth) {
            i += size - 1;
            continue;
        }

        if (locIsInWindow((windowpos){ px, py }) && px + glyphWidth <= COLS) {
            plotCodepointToBuffer(cp, (windowpos){ px, py }, glyphWidth, &fColor, backColor, dbuf);
        }
...
void printProgressBar(short x, short y, const char barLabel[COLS], long amtFilled, long amtMax, const color *fillColor, boolean dim) {
    short i, labelOffset, src, cursor, cpSize, cpWidth, labelWidth;
    unsigned int cp;
    unsigned int barText[20];
    color currentFillColor, textColor, progressBarColor, darkenedBarColor;
    const char *localizedLabel = tr(barLabel);
    ...
    labelWidth = strLenWithoutEscapes(localizedLabel);
    if (labelWidth > 20) {
        labelWidth = 20;
    }
    labelOffset = (20 - labelWidth) / 2;
    cursor = labelOffset;

    for (src = 0; localizedLabel[src] && cursor < 20; ) {
        if (localizedLabel[src] == COLOR_ESCAPE) {
            src += 4;
            continue;
        }
        cp = decodeUtf8Codepoint(&localizedLabel[src], &cpSize);
        cpWidth = unicodeDisplayWidth(cp);
        if (cpWidth <= 0) {
            src += cpSize;
            continue;
        }
        if (cursor + cpWidth > 20) {
            break;
        }
        barText[cursor] = cp;
        if (cpWidth == 2 && cursor + 1 < 20) {
            barText[cursor + 1] = ' ';
        }
        cursor += cpWidth;
        src += cpSize;
    ...
    for (i=0; i<20; i++) {
        ...
        plotCharWithColor((enum displayGlyph) barText[i], (windowpos){ x + i, y }, &textColor, &currentFillColor);
    }
}
...
short printMonsterInfo(creature *monst, short y, boolean dim, boolean highlight) {
    ...
    if (y < ROWS - 1) {
        ...
        strncpy(monstName, tr(monstName), COLS - 1);
        monstName[COLS - 1] = '\0';
        upperCase(monstName);
    ...
    if (monst->info.maxHP > 1
        ...
        if (monst->currentHP <= 0) {
            strcpy(buf, chineseUi ? "已死亡" : "Dead");
        } else if (percent != 0) {
            if (chineseUi) {
                sprintf(buf2, "(%s%i%%)", percent > 0 ? "+" : "", percent);
                sprintf(buf, "生命值 %s", buf2);
            } else {
                strcpy(buf, "       Health       ");
                sprintf(buf2, "(%s%i%%)", percent > 0 ? "+" : "", percent);
            ...
            strcpy(buf, chineseUi ? "生命值" : "Health");
        }
    ...
    if (!player.status[STATUS_HALLUCINATING] || rogue.playbackOmniscience || monst == &player) {
        ...
        for (i=0; i<NUMBER_OF_STATUS_EFFECTS; i++) {
            if (i == STATUS_WEAKENED && monst->status[i] > 0) {
                ...
                printProgressBar(0, y++, (monst == &player ? tr("Levitating") : tr("Flying")), monst->status[i], monst->maxStatus[i], &redBar, dim);
            } else if (i == STATUS_POISONED
                       && monst->status[i] > 0) {


                if (monst->status[i] * monst->poisonAmount >= monst->currentHP) {
                    strcpy(buf, tr("Fatal Poison"));
                } else {
                    strcpy(buf, tr("Poisoned"));
                }
                ...
                printProgressBar(0, y++, tr(statusEffectCatalog[i].name), monst->status[i], monst->maxStatus[i], &redBar, dim);
            }
        }
    ...
    if (monst == &player) {
        if (y < ROWS - 1) {
            ...
            if (!rogue.armor || rogue.armor->flags & ITEM_IDENTIFIED || rogue.playbackOmniscience) {
                ...
                sprintf(buf, tr("Str: %s%i%s  Armor: %i"),
                        tempColorEscape,
        ...
        if (y < ROWS - 1 && rogue.gold) {
            sprintf(buf, tr("Gold: %li"), rogue.gold);
            buf[20] = '\0';
            printString("                    ", 0, y, &white, &black, 0);
            printString(buf, (20 - strLenWithoutEscapes(buf)) / 2, y++, (dim ? &darkGray : &gray), &black, 0);
        }
        if (y < ROWS - 1) {
            ...
            encodeMessageColor(tempColorEscape, 0, &tempColor);
            encodeMessageColor(grayColorEscape, 0, (dim ? &darkGray : &gray));
            sprintf(buf, tr("%sStealth range: %i%s"),
                    tempColorEscape,
                    rogue.stealthRange,
                    grayColorEscape);
            printString("                    ", 0, y, &white, &black, 0);
...
short printItemInfo(item *theItem, short y, boolean dim, boolean highlight) {
    ...
    if (y < ROWS - 1) {
        ...
        strncpy(name, tr(name), sizeof(name) - 1);
...
short printTextBox(char *textBuf, short x, short y, short width,
    ...
    const char *translatedText = tr(textBuf);
    ...
    while (((lineCount = wrapText(NULL, translatedText, width)) + y2) >= ROWS - 2 && width < COLS-5) {
    ...
    screenDisplayBuffer dbuf;
    clearDisplayBuffer(&dbuf);
    printStringWithWrapping(translatedText, x2, y2, width, foreColor, backColor, &dbuf);
...
unsigned long printCarriedItemDetails(item *theItem,
    ...
    if (includeButtons) {
        ...
        if (theItem->category & (FOOD | SCROLL | POTION | WAND | STAFF | CHARM)) {
            sprintf(buttons[b].text, tr("   %sa%spply   "), goldColorEscape, whiteColorEscape);
            buttons[b].hotkey[0] = APPLY_KEY;
            b++;
        }
        if (theItem->category & (ARMOR | WEAPON | RING)) {
            if (theItem->flags & ITEM_EQUIPPED) {
                sprintf(buttons[b].text, tr("  %sr%semove   "), goldColorEscape, whiteColorEscape);
                buttons[b].hotkey[0] = UNEQUIP_KEY;
                b++;
            } else {
                sprintf(buttons[b].text, tr("   %se%squip   "), goldColorEscape, whiteColorEscape);
                buttons[b].hotkey[0] = EQUIP_KEY;
                b++;
            }
        }
        sprintf(buttons[b].text, tr("   %sd%srop    "), goldColorEscape, whiteColorEscape);
        buttons[b].hotkey[0] = DROP_KEY;
        b++;

        sprintf(buttons[b].text, tr("   %st%shrow   "), goldColorEscape, whiteColorEscape);
        buttons[b].hotkey[0] = THROW_KEY;
        b++;

        if (itemCanBeCalled(theItem)) {
            sprintf(buttons[b].text, tr("   %sc%sall    "), goldColorEscape, whiteColorEscape);
            buttons[b].hotkey[0] = CALL_KEY;
            b++;
        }

        if (KEYBOARD_LABELS) {
            sprintf(buttons[b].text, tr("  %sR%selabel  "), goldColorEscape, whiteColorEscape);
