...
static void welcome() {
    ...
    message(tr("Hello and welcome, adventurer, to the Dungeons of Doom!"), 0);
    strcpy(buf, tr("Retrieve the "));
    encodeMessageColor(buf, strlen(buf), &itemMessageColor);
    strcat(buf, tr("Amulet of Yendor"));
    encodeMessageColor(buf, strlen(buf), &white);
    sprintf(buf2, tr(" from the %i%s floor and escape with it!"), gameConst->amuletLevel, getOrdinalSuffix(gameConst->amuletLevel));
    strcat(buf, buf2);
    message(buf, 0);
    if (KEYBOARD_LABELS) {
...
void gameOver(char *killedBy, boolean useCustomPhrasing) {
...

    {
        ...
        if (chineseUi) {
            if (useCustomPhrasing) {
                sprintf(buf, "%s于第%i层", tr(killedBy), rogue.depthLevel);
            } else {
                sprintf(buf, "被%s击杀于第%i层", tr(killedBy), rogue.depthLevel);
            }
        } else {
            if (useCustomPhrasing) {
    ...
    strcpy(highScoreText, buf);
    {
        ...
        if (chineseUi) {
            if (theEntry.score > 0) {
                sprintf(buf2, (numGems > 0) ? "，携带价值%li金的宝藏" : "，携带%li金", theEntry.score);
                strcat(buf, buf2);
            }
