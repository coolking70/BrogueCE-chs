    ...
    if (numberOfItemsInPack() < MAX_PACK_ITEMS || (theItem->category & GOLD) || itemWillStackWithPack(theItem)) {
        ...
        if (theItem->category & GOLD) {
            ...
            rogue.featRecord[FEAT_TONE] = false;
            {
                boolean chineseUi = strcmp(tr("Health"), "Health") != 0;
                if (chineseUi) {
                    sprintf(buf, "你找到了 %i 枚金币。", theItem->quantity);
                } else {
...
void itemName(item *theItem, char *root, boolean includeDetails, boolean includeArticle, const color *baseColor) {
    ...
    switch (theItem -> category) {
        case FOOD:
            if (theItem -> kind == FRUIT) {
                sprintf(root, tr("mango%s"), pluralization);
            } else {
                if (theItem->quantity == 1) {
                    sprintf(article, "%s", chineseUi ? "" : "some ");
                    sprintf(root, "%s", tr("food"));
                } else {
                    sprintf(root, tr("ration%s of food"), pluralization);
                }
            }
            break;
        case WEAPON:
            sprintf(root, "%s%s", tr(weaponTable[theItem->kind].name), pluralization);
            if (includeDetails) {
                ...
                if (theItem->flags & ITEM_RUNIC) {
                    if ((theItem->flags & ITEM_RUNIC_IDENTIFIED) || rogue.playbackOmniscience) {
                        ...
                        if (chineseUi) {
                            sprintf(buf, "%s（%s）%s", root, runicName, grayEscapeSequence);
                        } else {
                        ...
                        strcat(root, chineseUi ? "（未知符文）" : " (unknown runic)");
                    }
                }
            ...
            break;
        case ARMOR:
            sprintf(root, "%s", tr(armorTable[theItem->kind].name));
            if (includeDetails) {
        ...
        case SCROLL:
            if (scrollTable[theItem->kind].identified || rogue.playbackOmniscience) {
                sprintf(root, tr("scroll%s of %s"), pluralization, tr(scrollTable[theItem->kind].name));
            } else if (scrollTable[theItem->kind].called) {
                sprintf(root, tr("scroll%s called %s%s%s"),
                        pluralization,
                        purpleEscapeSequence,
                        tr(scrollTable[theItem->kind].callTitle),
                        baseEscapeSequence);
            } else {
                sprintf(root, tr("scroll%s entitled %s\"%s\"%s"),
                        pluralization,
                        purpleEscapeSequence,
                        tr(scrollTable[theItem->kind].flavor),
                        baseEscapeSequence);
            }
            break;
        case POTION:
            if (potionTable[theItem->kind].identified || rogue.playbackOmniscience) {
                sprintf(root, tr("potion%s of %s"), pluralization, tr(potionTable[theItem->kind].name));
            } else if (potionTable[theItem->kind].called) {
                sprintf(root, tr("potion%s called %s%s%s"),
                        pluralization,
                        purpleEscapeSequence,
                        tr(potionTable[theItem->kind].callTitle),
                        baseEscapeSequence);
            } else {
                sprintf(root, tr("%s%s%s potion%s"),
                        purpleEscapeSequence,
                        tr(potionTable[theItem->kind].flavor),
                        baseEscapeSequence,
                        pluralization);
            }
            break;
        case WAND:
            if (wandTable[theItem->kind].identified || rogue.playbackOmniscience) {
                sprintf(root, tr("wand%s of %s"),
                        pluralization,
                        tr(wandTable[theItem->kind].name));
            } else if (wandTable[theItem->kind].called) {
                sprintf(root, tr("wand%s called %s%s%s"),
                        pluralization,
                        purpleEscapeSequence,
                        tr(wandTable[theItem->kind].callTitle),
                        baseEscapeSequence);
            } else {
                sprintf(root, tr("%s%s%s wand%s"),
                        purpleEscapeSequence,
                        tr(wandTable[theItem->kind].flavor),
                        baseEscapeSequence,
                        pluralization);
            }
            if (includeDetails) {
                if (theItem->flags & (ITEM_IDENTIFIED | ITEM_MAX_CHARGES_KNOWN) || rogue.playbackOmniscience) {
                    ...
                    if (chineseUi) {
                        sprintf(buf, "%s%s（已使用 %i 次）", root, grayEscapeSequence, theItem->enchant2);
                    } else {
                    ...
                    if (chineseUi) {
                        sprintf(buf, "%s%s（已使用 %s 次）", root, grayEscapeSequence, (theItem->enchant2 == 2 ? "2" : "1"));
                    } else {
        ...
        case STAFF:
            if (staffTable[theItem->kind].identified || rogue.playbackOmniscience) {
                sprintf(root, tr("staff%s of %s"), pluralization, tr(staffTable[theItem->kind].name));
            } else if (staffTable[theItem->kind].called) {
                sprintf(root, tr("staff%s called %s%s%s"),
                        pluralization,
                        purpleEscapeSequence,
                        tr(staffTable[theItem->kind].callTitle),
                        baseEscapeSequence);
            } else {
                sprintf(root, tr("%s%s%s staff%s"),
                        purpleEscapeSequence,
                        tr(staffTable[theItem->kind].flavor),
                        baseEscapeSequence,
                        pluralization);
            }
            if (includeDetails) {
        ...
        case RING:
            if (ringTable[theItem->kind].identified || rogue.playbackOmniscience) {
                sprintf(root, tr("ring%s of %s"), pluralization, tr(ringTable[theItem->kind].name));
            } else if (ringTable[theItem->kind].called) {
                sprintf(root, tr("ring%s called %s%s%s"),
                        pluralization,
                        purpleEscapeSequence,
                        tr(ringTable[theItem->kind].callTitle),
                        baseEscapeSequence);
            } else {
                sprintf(root, tr("%s%s%s ring%s"),
                        purpleEscapeSequence,
                        tr(ringTable[theItem->kind].flavor),
                        baseEscapeSequence,
                        pluralization);
            }
            ...
            break;
        case CHARM:
            sprintf(root, tr("%s charm%s"), tr(charmTable[theItem->kind].name), pluralization);
            ...
            if (includeDetails) {
                ...
                if (theItem->charges) {
                    ...
                    strcat(root, grayEscapeSequence);
                    strcat(root, chineseUi ? "（就绪）" : " (ready)");
                }
            }
            break;
        case GOLD:
            sprintf(root, tr("gold piece%s"), pluralization);
            break;
        case AMULET:
            if (chineseUi) {
                sprintf(root, "%s延多尔护符%s", yellowEscapeSequence, baseEscapeSequence);
            } else {
        ...
        case GEM:
            if (chineseUi) {
                sprintf(root, "%s光明石%s（来自第 %i 层）", yellowEscapeSequence, baseEscapeSequence, theItem->originDepth);
            } else {
        ...
        case KEY:
            if (includeDetails && theItem->originDepth > 0 && theItem->originDepth != rogue.depthLevel) {
                if (chineseUi) {
                    sprintf(root, "%s%s（来自第 %i 层）",
                            tr(keyTable[theItem->kind].name),
                            grayEscapeSequence,
                            theItem->originDepth);
                } else {
                ...
                if (chineseUi) {
                    sprintf(root, "%s", tr(keyTable[theItem->kind].name));
                } else {
                    sprintf(root,
                            keyTable[theItem->kind].name,
                            "%s%s",
            ...
            break;
        default:
            sprintf(root, "%s%s", chineseUi ? "未知物品" : "unknown item", pluralization);
            break;
    }
    ...
    if (includeArticle) {
        ...
        if (chineseUi && (theItem->category & AMULET)) {
            article[0] = '\0';
        }
    }
    // strcat(buf, suffixID);
...
void itemDetails(char *buf, item *theItem) {
    ...
    chineseUi = strcmp(tr("Health"), "Health") != 0;

    // Name
    itemName(theItem, theName, true, true, NULL);
    strncpy(theName, tr(theName), sizeof(theName) - 1);
    ...
    if (tableForItemCategory(theItem->category)
        ...
        const char *srcDesc = tableForItemCategory(theItem->category)[theItem->kind].description;
        const char *localizedDesc = tr(srcDesc);
        if (chineseUi && !strcmp(localizedDesc, srcDesc)) {
            switch (theItem->category) {
                case FOOD:
                    localizedDesc = "这是一份可以恢复营养的食物。";
                    break;
                case WEAPON:
                    localizedDesc = "这是一件武器。";
                    break;
                case ARMOR:
                    localizedDesc = "这是一件护甲。";
                    break;
                case POTION:
                    localizedDesc = "这是一瓶药水。";
                    break;
                case SCROLL:
                    localizedDesc = "这是一张卷轴。";
                    break;
                case STAFF:
                    localizedDesc = "这是一根法杖。";
                    break;
                case WAND:
                    localizedDesc = "这是一根魔杖。";
                    break;
                case RING:
                    localizedDesc = "这是一枚戒指。";
                    break;
                case CHARM:
                    localizedDesc = "这是一枚护符。";
                    break;
                default:
                    localizedDesc = "这件物品的说明尚未汉化。";
                    break;
            }
        }
        strcat(buf, localizedDesc);
        ...
        switch (theItem->category) {
            case POTION:
                if (chineseUi) {
                    sprintf(buf2, "%s瓶装有一种旋转的%s液体。谁知道喝下或投掷后会有什么效果？",
                            (singular ? "这" : "这些"),
                            tr(tableForItemCategory(theItem->category)[theItem->kind].flavor));
                } else {
                    sprintf(buf2, tr("%s flask%s contain%s a swirling %s liquid. Who knows what %s will do when drunk or thrown?"),
                            (singular ? "This" : "These"),
                            (singular ? "" : "s"),
                            (singular ? "s" : ""),
                            tr(tableForItemCategory(theItem->category)[theItem->kind].flavor),
            ...
            case SCROLL:
                if (chineseUi) {
                    sprintf(buf2, "%s卷轴覆盖着无法辨认的文字，标题为\"%s\"。谁知道大声朗读后会有什么效果？",
                            (singular ? "这" : "这些"),
                            tr(tableForItemCategory(theItem->category)[theItem->kind].flavor));
                } else {
                    sprintf(buf2, tr("%s parchment%s %s covered with indecipherable writing, and bear%s a title of \"%s.\" Who knows what %s will do when read aloud?"),
                            (singular ? "This" : "These"),
                            (singular ? "" : "s"),
                            (singular ? "is" : "are"),
                            (singular ? "s" : ""),
            ...
            case STAFF:
                if (chineseUi) {
                    sprintf(buf2, "这根粗糙的%s法杖触感温暖。谁知道使用它会有什么效果？",
                            tr(tableForItemCategory(theItem->category)[theItem->kind].flavor));
                } else {
                    sprintf(buf2, tr("This gnarled %s staff is warm to the touch. Who knows what it will do when used?"),
                            tr(tableForItemCategory(theItem->category)[theItem->kind].flavor));
                }
                break;
            case WAND:
                if (chineseUi) {
                    sprintf(buf2, "这根细长的%s魔杖触感温暖。谁知道使用它会有什么效果？",
                            tr(tableForItemCategory(theItem->category)[theItem->kind].flavor));
                } else {
                    sprintf(buf2, tr("This thin %s wand is warm to the touch. Who knows what it will do when used?"),
                            tr(tableForItemCategory(theItem->category)[theItem->kind].flavor));
                }
                break;
            case RING:
                if (chineseUi) {
                    sprintf(buf2, "这条金属环上镶嵌着一颗%s宝石，在黑暗中闪闪发光。谁知道佩戴它有什么效果？ ",
                            tr(tableForItemCategory(theItem->category)[theItem->kind].flavor));
                } else {
                    sprintf(buf2, tr("This metal band is adorned with a%s %s gem that glitters in the darkness. Who knows what effect it has when worn? "),
                            isVowelish((char *) tr(tableForItemCategory(theItem->category)[theItem->kind].flavor)) ? "n" : "",
                            tr(tableForItemCategory(theItem->category)[theItem->kind].flavor));
                }
                break;
            ...
            case GEM:
                ...
                break;
            case KEY:
                strcpy(buf2, tr(keyTable[theItem->kind].description));
                break;
            case GOLD:
                if (chineseUi) {
                    sprintf(buf2, "一堆 %i 枚闪亮的金币。", theItem->quantity);
                } else {
    ...
    if (carried && theItem->originDepth > 0) {
        if (chineseUi) {
            sprintf(buf2, " （在第 %i 层%s找到。） ",
                theItem->originDepth,
                theItem->flags & ITEM_IS_KEY && !(theItem->category & KEY)
                    ? "的密室中" : "");
            strcat(buf, buf2);
        } else {
            sprintf(buf2, tr(" (You found %s%s on depth %i.) "),
                singular ? "it" : "them",
    ...
    switch (theItem->category) {
        ...
        case FOOD:
            if (chineseUi) {
                sprintf(buf2, "\n\n你%s够饿来充分享用%s。",
                        ((STOMACH_SIZE - player.status[STATUS_NUTRITION]) >= foodTable[theItem->kind].power ? "已经足" : "还不"),
                        tr(foodTable[theItem->kind].name));
                strcat(buf, buf2);
            } else {
        ...
        case ARMOR:
            ...
            if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
                ...
                if (strengthModifier(theItem)) {
                    if (chineseUi) {
                        sprintf(buf2, "，%s因力量%s%s%+.2f%s。 ",
                                (theItem->enchant1 && (theItem->enchant1 > 0) == (strengthModifier(theItem) > 0) ? "还" : ""),
                                (strengthModifier(theItem) > 0 ? "充足获得了" : "不足受到了"),
                ...
                if (strengthModifier(theItem)) {
                    if (chineseUi) {
                        sprintf(buf2, "\n\n%s%s因力量%s%s%+.2f%s。 ",
                                theName,
                                ((theItem->enchant1 > 0) && (theItem->flags & ITEM_MAGIC_DETECTED) ? "还" : ""),
                                (strengthModifier(theItem) > 0 ? "充足获得了" : "不足受到了"),
                                (strengthModifier(theItem) > 0 ? goodColorEscape : badColorEscape),
                                strengthModifier(theItem) / (double) FP_FACTOR,
                                whiteColorEscape);
                ...
                if (theItem->category & WEAPON) {
                    if (chineseUi) {
                        sprintf(buf2, "%s击败 %i 个敌人即可揭示其秘密。 ",
                                theItem->charges == gameConst->weaponKillsToAutoID ? "" : "再",
                                theItem->charges);
                        strcat(buf, buf2);
                    } else {
                    ...
                    if (chineseUi) {
                        sprintf(buf2, "%s穿戴 %i 回合即可揭示其秘密。 ",
                                theItem->charges == gameConst->armorDelayToAutoID ? "" : "再",
                                theItem->charges);
                        strcat(buf, buf2);
                    } else {
            ...
            if (!(theItem->flags & ITEM_EQUIPPED)) {
                if (theItem->category & WEAPON) {
                    ...
                    if (chineseUi) {
                        sprintf(buf2, "装备%s%s将%s你当前的命中率 %s%i%%%s，并将%s你当前的伤害 %s%i%%%s。 ",
                                theName,
                                ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) ? "" : "（假设没有隐藏属性）",
                                (((short) accuracyChange) < 0) ? "降低" : "提高",
                                (((short) accuracyChange) < 0) ? badColorEscape : (accuracyChange > 0 ? goodColorEscape : ""),
                                abs((short) accuracyChange),
                                whiteColorEscape,
                                (((short) damageChange) < 0) ? "降低" : "提高",
                                (((short) damageChange) < 0) ? badColorEscape : (damageChange > 0 ? goodColorEscape : ""),
                                abs((short) damageChange),
                                whiteColorEscape);
                    } else {
                        sprintf(buf2, tr("Wielding the %s%s will %s your current accuracy by %s%i%%%s, and will %s your current damage by %s%i%%%s. "),
                                theName,
                                ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) ? "" : tr(", assuming it has no hidden properties,"),
                                tr((((short) accuracyChange) < 0) ? "decrease" : "increase"),
                                (((short) accuracyChange) < 0) ? badColorEscape : (accuracyChange > 0 ? goodColorEscape : ""),
                                abs((short) accuracyChange),
                                whiteColorEscape,
                                tr((((short) damageChange) < 0) ? "decrease" : "increase"),
                                (((short) damageChange) < 0) ? badColorEscape : (damageChange > 0 ? goodColorEscape : ""),
                                abs((short) damageChange),
                                whiteColorEscape);
                    }
                ...
                strcat(buf, chineseUi ? buf2 : tr(buf2));
            }

            // protected?
            if (theItem->flags & ITEM_PROTECTED) {
                if (chineseUi) {
                    sprintf(buf2, "%s%s不会被酸液腐蚀。%s ", goodColorEscape, theName, whiteColorEscape);
                    strcat(buf, buf2);
                } else {
            ...
            if ((theItem->category & ARMOR)
                ...
                if (chineseUi) {
                    sprintf(buf2, "装备%s将%s%s你的隐匿范围 %i%s。 ",
                            theName,
                    ...
                    sprintf(buf2, tr("Equipping the %s will %s%s your stealth range by %i%s. "),
                            theName,
                            new > 0 ? badColorEscape : goodColorEscape,
                            tr(new > 0 ? "increase" : "decrease"),
                            abs(new),
                            whiteColorEscape);
                    strcat(buf, tr(buf2));
                }
            ...
            if (theItem->category & WEAPON) {
                ...
                if (theItem->flags & ITEM_RUNIC) {
                    if ((theItem->flags & ITEM_RUNIC_IDENTIFIED) || rogue.playbackOmniscience) {
                        if (chineseUi) {
                            sprintf(buf2, "\n\n发光的%s符文装饰着%s。 ",
                                    tr(weaponRunicNames[theItem->enchant2]),
                                    theName);
                            strcat(buf, buf2);
                        } else {
                        ...
                        if (theItem->enchant2 == W_SLAYING) {
                            ...
                            if (chineseUi) {
                                if (monsterClassHasAcidicMonster(theItem->vorpalEnemy)) {
                                    sprintf(buf2, "攻击%s时不会被腐蚀，且一击必杀。 ", tr(buf3));
                                } else {
                                    sprintf(buf2, "对%s一击必杀。 ", tr(buf3));
                                }
                                strcat(buf, buf2);
                            } else {
                                if (monsterClassHasAcidicMonster(theItem->vorpalEnemy)) {
                            ...
                            if (chineseUi) {
                                if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
                                    sprintf(buf2, "击中敌人时有 %i%% 的几率召唤 %i 个%s的幻影分身，拥有与你相同的命中率和攻击力，持续 %i 回合。（若附魔，将有 %i%% 的几率召唤 %i 个分身，持续 %i 回合。）",
                                    ...
                                    sprintf(buf2, "有时击中敌人会召唤%s的幻影分身，拥有与你相同的命中率和攻击力，持续一小段时间。",
                                            theName);
                                }
                                strcat(buf, buf2);
                            } else {
                          ...
                          if (chineseUi) {
                            if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
                                if (runicWeaponChance(theItem, false, 0) < 2
                                    ...
                                    strcpy(buf2, "因力量不足，其符文效果几乎不会触发，但有时");
                                } else {
                                    sprintf(buf2, "击中敌人时有 %i%% 的几率",
                                            runicWeaponChance(theItem, false, 0));
                                }
                                strcat(buf, buf2);
                            } else {
                            ...
                            sprintf(buf2, "%s",
                                    tr(weaponRunicEffectDescriptions[theItem->enchant2]));
                            strcat(buf, buf2);
                            ...
                            if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
                                ...
                                if (((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience)
                                    ...
                                    sprintf(buf2, "（若附魔，几率将提升至 %i%%",
                                            runicWeaponChance(theItem, true, enchant + enchantMagnitude() * enchantIncrement(theItem)));
                                    strcat(buf, buf2);
                                    if (nextLevelState) {
                                        if (theItem->enchant2 == W_FORCE) {
                                            sprintf(buf2, "，距离将增加至 %i。）",
                                                    nextLevelState);
                                        } else {
                                            sprintf(buf2, "，持续时间将增加至 %i 回合。）",
                                                    nextLevelState);
                                        }
                                    } else {
                                        strcpy(buf2, "。）");
                            ...
                            // English path for other weapon runics
                            if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
                                if (runicWeaponChance(theItem, false, 0) < 2
                                    && rogue.strength - player.weaknessAmount < theItem->strengthRequired) {
                ...
                if (theItem->flags & ITEM_EQUIPPED) {
                    if (chineseUi) {
                        if (theItem->flags & ITEM_CURSED) {
                            sprintf(buf2, "\n\n你手持%s，已做好准备，但因其被诅咒，你无法放下。 ", theName);
                        } else {
                            sprintf(buf2, "\n\n你手持%s，已做好准备。 ", theName);
                        }
                        strcat(buf, buf2);
                        ...
                        strcat(buf, tr(buf2));
                           ...
                           && (theItem->flags & ITEM_CURSED)) {
                    if (chineseUi) {
                        sprintf(buf2, "\n\n%s你能感受到%s中潜伏着一股邪恶的魔力。%s ", badColorEscape, theName, whiteColorEscape);
                        strcat(buf, buf2);
                    } else {
                ...
                if (theItem->flags & ITEM_RUNIC) {
                    if ((theItem->flags & ITEM_RUNIC_IDENTIFIED) || rogue.playbackOmniscience) {
                        if (chineseUi) {
                            sprintf(buf2, "\n\n发光的%s符文装饰着%s。 ",
                                    tr(armorRunicNames[theItem->enchant2]),
                                    theName);
                            strcat(buf, buf2);
                        } else {
                            sprintf(buf2, "\n\nGlowing runes of %s adorn the %s. ",
                                ...
                                case A_REFLECTION:
                                    if (theItem->flags & ITEM_IDENTIFIED) {
                                        if (theItem->enchant1 > 0) {
                                            ...
                                            sprintf(buf2, "穿戴时，你将偏转 %i%% 的来袭法术——其中 %i%% 的几率直接反弹回施法者。（若附魔，将分别增加至 %i%% 和 %i%%。） ",
                                                    reflectChance,
                                            ...
                                            short reflectChance = reflectionChance(enchant);
                                            short reflectChance2 = reflectionChance(enchant + enchantMagnitude() * enchantIncrement(theItem));
                                            sprintf(buf2, "穿戴时，你自己的法术有 %i%% 会偏离目标——其中 %i%% 的几率反弹回你自身。（若附魔，将分别降低至 %i%% 和 %i%%。） ",
                                                    reflectChance,
                                                    reflectChance * reflectChance / 100,
                                                    reflectChance2,
                                                    reflectChance2 * reflectChance2 / 100);
                                        ...
                                        strcpy(buf2, "穿戴时，你将偏转一定比例的来袭法术，取决于附魔等级。 ");
                                    }
                                    break;
                                case A_RESPIRATION:
                                    strcpy(buf2, "穿戴时，它会在你周围维持一层清新空气，使你免疫蒸汽和所有毒气的影响。 ");
                                    break;
                                case A_DAMPENING:
                                    strcpy(buf2, "穿戴时，它能安全吸收爆炸的冲击力（但你仍可能被灼伤）。 ");
                                    break;
                                case A_BURDEN:
                                    strcpy(buf2, "每次吸收攻击有 10% 几率永久增加力量需求。 ");
                                    break;
                                case A_VULNERABILITY:
                                    strcpy(buf2, "穿戴时，受到的攻击伤害加倍。 ");
                                    break;
                                case A_IMMOLATION:
                                    strcpy(buf2, "每次吸收攻击有 10% 几率引发火焰爆炸。 ");
                                    break;
                                default:
                                    break;
                            }
        ...
        case STAFF:
            ...
            if (chineseUi) {
                if (theItem->lastUsed[0] > 0 && theItem->lastUsed[1] > 0 && theItem->lastUsed[2] > 0) {
                    sprintf(buf2, "你上次使用它分别是 %li、%li 和 %li 回合前。 ",
              ...
              if (chineseUi) {
                switch (theItem->kind) {
                    case STAFF_LIGHTNING:
                        sprintf(buf2, "这根法杖对射线上的所有生物造成伤害；无物可免疫。（若附魔，平均伤害将提升 %i%%。）",
        ...
        case WAND:
            ...
            if (chineseUi) {
                if ((theItem->flags & (ITEM_IDENTIFIED | ITEM_MAX_CHARGES_KNOWN)) || rogue.playbackOmniscience) {
                    if (theItem->charges) {
                        sprintf(buf2, "还剩 %i 次充能。附魔将增加 %i 次充能。",
                                theItem->charges,
                                wandTable[theItem->kind].range.lowerBound * enchantMagnitude());
                    } else {
                        sprintf(buf2, "没有剩余充能。附魔将增加 %i 次充能。",
                                wandTable[theItem->kind].range.lowerBound * enchantMagnitude());
                    }
                } else {
                    if (theItem->enchant2) {
                        sprintf(buf2, "你已使用这根魔杖 %i 次，但不知道还剩多少充能。",
                                theItem->enchant2);
                    } else {
                        strcpy(buf2, "你还未使用过这根魔杖。");
                    }

                    if (wandTable[theItem->kind].identified) {
                        strcat(buf, buf2);
                        sprintf(buf2, "这类魔杖可以有 %i 到 %i 次充能。附魔将增加 %i 次充能。",
                  ...
                  if (chineseUi) {
                    switch (theItem->kind) {
                        case RING_CLAIRVOYANCE:
                            if (theItem->enchant1 > 0) {
                                sprintf(buf2, "\n\n这枚戒指提供半径 %i 的魔法视野。（若附魔，将增加至 %i。）",
                                        theItem->enchant1 + 1,
                                        theItem->enchant1 + 1 + enchantMagnitude());
                            } else {
                                sprintf(buf2, "\n\n这枚戒指会使你在半径 %i 内魔法致盲。（若附魔，将减少至 %i。）",
                        ...
                        case RING_REGENERATION:
                            sprintf(buf2, "\n\n佩戴此戒指，你将在 %li 回合内完全恢复生命值（而非 %li 回合）。（若附魔，将缩短至 %li 回合。）",
                        ...
                        case RING_TRANSFERENCE:
                            sprintf(buf2, "\n\n对生物造成直接伤害（无论近战或远程）将%s你所造成伤害的 %i%%。（若附魔，将%s至 %i%%。）",
                                    (theItem->enchant1 >= 0 ? "治疗" : "伤害"),
    ...
    for (i=0; i<itemNumber; i++) {
        ...
        if ((theItem->flags & ITEM_MAGIC_DETECTED)
            ...
            sprintf(buttons[i].text, " %c%c %s* %s* %s%s%s%s",
                    ...
                    (theItem->flags & ITEM_EQUIPPED ? ((theItem->category & WEAPON) ? tr(" (in hand) ") : tr(" (worn) ")) : ""));
    ...
    if (waitForAcknowledge) {
        ...
        if (itemSpaceRemaining) {
            if (strcmp(tr("Health"), "Health") != 0) {
                sprintf(buf, "你还能再携带 %i 个物品。", itemSpaceRemaining);
            } else {
                sprintf(buf, tr("You have room for %i more item%s."),
                        itemSpaceRemaining,
                        (itemSpaceRemaining == 1 ? "" : "s"));
            }
            sprintf(buttons[itemNumber + extraLineCount].text, "%s%s    %s",
                    grayColorEscapeSequence,
                    (magicDetected ? "  " : ""),
                    buf);
        } else {
        ...
        sprintf(buttons[itemNumber + extraLineCount].text,
                KEYBOARD_LABELS ? "%s%s %s" : "%s%s %s",
                grayColorEscapeSequence,
                (magicDetected ? "  " : ""),
                tr(KEYBOARD_LABELS ? "-- press (a-z) for more info -- " : "-- touch an item for more info -- "));
        maxLength = max(maxLength, (strLenWithoutEscapes(buttons[itemNumber + extraLineCount].text)));
        extraLineCount++;
    }
...
void strengthCheck(item *theItem, boolean noisy) {
    ...
    if (noisy && theItem) {
        if (theItem->category & WEAPON && theItem->strengthRequired > rogue.strength - player.weaknessAmount) {
            ...
            if (chineseUi) {
                sprintf(buf2, "你几乎举不起%s；还需要 %i 点力量。", buf1, strengthDeficiency);
            } else {
        ...
        if (theItem->category & ARMOR && theItem->strengthRequired > rogue.strength - player.weaknessAmount) {
            ...
            itemName(theItem, buf1, false, false, NULL);
            if (chineseUi) {
                sprintf(buf2, "你在%s的重量下摇摇欲坠；还需要 %i 点力量。", buf1, strengthDeficiency);
            } else {
...
static void rechargeItems(unsigned long categories) {
    ...
    if (categoryCount) {
        ...
        if (chineseUi) {
            i = 0;
            strcpy(buf, "一股能量涌过你的背包，为你的");
            if (x) {
                i++;
                strcat(buf, "法杖");
                if (i == categoryCount - 1) strcat(buf, "和");
                else if (i <= categoryCount - 2) strcat(buf, "、");
            }
            if (y) {
                i++;
                strcat(buf, "魔杖");
...
void throwCommand(item *theItem, boolean autoThrow) {
    ...
    //
    {
        boolean throwChineseUi = strcmp(tr("Health"), "Health") != 0;
        if (throwChineseUi) {
            sprintf(buf, "向哪里投掷%s？（<hjklyubn>、鼠标或 <tab>）", theName);
        } else {
...
void unequip(item *theItem) {
    ...
    if (!(theItem->flags & ITEM_EQUIPPED)) {
        ...
        if (chineseUi) {
            if (theItem->category & WEAPON) {
                sprintf(buf, "你不再持握%s了。", buf2);
            } else {
                sprintf(buf, "你不再装备%s了。", buf2);
            }
        } else {
...
boolean equipItem(item *theItem, boolean force, item *unequipHint) {
    ...
    if (!force) {
        if (previouslyEquippedItem) {
            ...
            if (chineseUi) {
                if (theItem->category & WEAPON) {
                    sprintf(buf1, "现在持握%s，替换了你的%s。", buf2, buf3);
                } else {
                    sprintf(buf1, "现在装备%s，替换了你的%s。", buf2, buf3);
                }
            } else {
            ...
            if (chineseUi) {
                if (theItem->category & WEAPON) {
                    sprintf(buf1, "现在持握%s。", buf2);
                } else {
                    sprintf(buf1, "现在装备%s。", buf2);
                }
            } else {
        ...
        if (theItem->flags & ITEM_CURSED) {
            ...
            switch(theItem->category) {
                case WEAPON:
                    if (chineseUi) sprintf(buf1, "你不由自主地死死攥紧了你的%s，心头一凛。", buf2);
                    else sprintf(buf1, "you wince as your grip involuntarily tightens around your %s.", buf2);
                    break;
                case ARMOR:
                    if (chineseUi) sprintf(buf1, "你的%s痛苦地紧缩在你身上。", buf2);
                    else sprintf(buf1, "your %s constricts around you painfully.", buf2);
                    break;
                case RING:
                    if (chineseUi) sprintf(buf1, "你的%s痛苦地紧绕在手指上。", buf2);
                    else sprintf(buf1, "your %s tightens around your finger painfully.", buf2);
                    break;
                default:
                    if (chineseUi) sprintf(buf1, "你的%s被一股邪恶的力量攫住。", buf2);
                    else sprintf(buf1, "your %s seizes you with a malevolent force.", buf2);
                    break;
            }
...
void shuffleFlavors() {
    ...
    for (i=0; i<NUMBER_ITEM_TITLES; i++) {
        itemTitles[i][0] = '\0';
        randNumber = rand_range(3, 4);  // 3~4 个词素，中文不加空格
