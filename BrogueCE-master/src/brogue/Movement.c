...
void playerRuns(short direction) {
  ...
  for (int dir = 0; dir < 4; dir++) {
    cardinalPassability[dir] =
        monsterAvoids(&player, posNeighborInDirection(player.loc, dir));
  }
...
// Retrieves a pointer to the flavor text of the highest-priority terrain at the
// given location
const char *tileFlavor(short x, short y) {
  return tileCatalog[pmap[x][y].layers[highestPriorityLayer(x, y, false)]]
      .flavorText;
}

// Retrieves a pointer to the description text of the highest-priority terrain
...
void describedItemBasedOnParameters(short theCategory, short theKind,
                                    short theQuantity, short theOriginDepth,
                                    char *buf) {
...
/// @brief Describes an item, primarily as a component of the location
/// description that displays just above the menu bar at the bottom of the
/// screen. The complete location description needs to fit on a single line the
/// width of the dungeon, so an attempt is made to fit as much detail as
/// possible within a loose length constraint. If there is enough room, item
/// details are included. If not, a terse description is used. Note that the
/// final description may exceed the max length.
...
void describeLocation(char *buf, short x, short y) {
  ...
  if ((x == player.loc.x) && (y == player.loc.y) && !theItem) {
    if (player.status[STATUS_LEVITATING]) {
      if (chineseUi) {
        sprintf(buf, "你正悬浮在%s上方。", tr(tileText(x, y)));
      } else {
  ...
  monst = NULL;
  standsInTerrain =
      ((tileCatalog[pmap[x][y].layers[highestPriorityLayer(x, y, false)]]
            .mechFlags &
        TM_STAND_IN_TILE)
           ? true
           : false);
  ...
  if (theItem && !playerCanSeeOrSense(x, y) &&
      (theItem->flags & ITEM_MAGIC_DETECTED) && itemMagicPolarity(theItem)) {
    magicItem = theItem;
  } else if (monst && !canSeeMonster(monst) && monst->carriedItem &&
  ...
  if (magicItem && !(pmap[x][y].flags & DISCOVERED)) {
    ...
    case 1:
      strcpy(object, magicItem->category == AMULET ? "the Amulet of Yendor"
                                                   : "benevolent magic");
      break;
    case -1:
  ...
  // telepathy
  if (monst && !monsterIsPlayer && !canSeeMonster(monst) &&
      monsterRevealed(monst)) {

    strcpy(adjective, (((!player.status[STATUS_HALLUCINATING] ||
                         rogue.playbackOmniscience) &&
                        !monst->info.isLarge) ||
                               (player.status[STATUS_HALLUCINATING] &&
                                !rogue.playbackOmniscience && rand_range(0, 1))
                           ? "small"
                           : "large"));
    ...
    if (chineseUi) {
      const char *sizeStr =
          (strcmp(adjective, "small") == 0) ? "细弱的" : "强烈的";
      if (pmap[x][y].flags & DISCOVERED) {
        const char *preStr;
        if (monst->bookkeepingFlags & MB_SUBMERGED)
          preStr = "位于%s之下";
          ...
          preStr = "位于%s上方";
        char locBuf[COLS * 3];
        sprintf(locBuf, preStr, tr(object));
        sprintf(buf, "你感知到%s精神感应，%s。", sizeStr, locBuf);
      } else {
        sprintf(buf, "你在此处感知到%s精神感应。", sizeStr);
      }
    } else {
      sprintf(buf, "you can sense a %s psychic emanation %s%s.", adjective,
              preposition, object);
    }
    restoreRNG;
    return;
  ...
  if (!playerCanSeeOrSense(x, y)) {
    if (pmap[x][y].flags & DISCOVERED) { // memory
      if (pmap[x][y].rememberedItemCategory) {
        if (player.status[STATUS_HALLUCINATING] && !rogue.playbackOmniscience) {
          ...
          describedItemBasedOnParameters(
              pmap[x][y].rememberedItemCategory, pmap[x][y].rememberedItemKind,
              pmap[x][y].rememberedItemQuantity,
              pmap[x][y].rememberedItemOriginDepth, object);
        }
      } else {
        strcpy(object, tileCatalog[pmap[x][y].rememberedTerrain].description);
      }
      if (chineseUi) {
        sprintf(buf, "你记得在这里看到过%s。", tr(object));
      } else {
      ...
      if (chineseUi) {
        sprintf(buf, "你预感这里有%s。",
                tr(tileCatalog[pmap[x][y].rememberedTerrain].description));
      } else {
        sprintf(buf, "you expect %s to be here.",
                tileCatalog[pmap[x][y].rememberedTerrain].description);
      }
      restoreRNG;
      return;
    }
  ...
  if (monst) {
    ...
    if (pmap[x][y].layers[GAS] && monst->status[STATUS_INVISIBLE] &&
        !monsterIsPlayer) { // phantoms in gas
    ...
    subjectMoving = (monst->turnsSpentStationary == 0 &&
                     !(monst->info.flags &
                       (MONST_GETS_TURN_ON_ACTIVATION | MONST_IMMOBILE)) &&
    ...
    if ((monst->info.flags & MONST_ATTACKABLE_THRU_WALLS) &&
      ...
      strcat(verb,
             (monsterIsPlayer ? " hovering"
                              : (subjectMoving ? " flying" : " hovering")));
      strcpy(preposition, "over");
      prepositionLocked = true;
    } else if (monsterCanSubmergeNow(monst)) {
      strcat(verb, (subjectMoving ? " gliding" : " drifting"));
    } else if (cellHasTerrainFlag((pos){x, y}, T_MOVES_ITEMS) &&
               !(monst->info.flags & MONST_SUBMERGES)) {
    ...
    if (theItem) {
      if (chineseUi) {
        if (monsterIsPlayer) {
          sprintf(buf, "你在%s%s，脚下有X。", tr(tileText(x, y)),
                  standsInTerrain ? "中" : "上");
          describedItemName(theItem, object, DCOLS - strlen(buf) + 1);
          sprintf(buf, "你在%s%s，脚下有%s。", tr(tileText(x, y)),
                  standsInTerrain ? "中" : "上", object);
        } else {
          sprintf(buf, "%s在%s%s，它脚下有X。", subject, tr(tileText(x, y)),
                  standsInTerrain ? "中" : "上");
          describedItemName(theItem, object, DCOLS - strlen(buf) + 1);
          sprintf(buf, "%s在%s%s，它脚下有%s。", subject, tr(tileText(x, y)),
                  standsInTerrain ? "中" : "上", object);
        }
        restoreRNG;
        return;
      ...
      sprintf(buf, "%s %s %s %s%s.", subject, verb, preposition, object,
              itemLocation);
      describedItemName(theItem, object, DCOLS - strlen(buf));

    } else {
      if (!prepositionLocked) {
        strcpy(preposition, subjectMoving
      ...
      if (chineseUi) {
        // Direct Chinese construction for monster position (no item)
        const char *objTr = tr(object);
        boolean isBurning = (strstr(verb, "burning") != NULL);
        // Find core verb (skip "is "/"are " prefix)
        const char *vCore = verb;
        if (strncmp(verb, "is ", 3) == 0)
          vCore = verb + 3;
        else if (strncmp(verb, "are ", 4) == 0)
          vCore = verb + 4;

        char zhVerb[COLS * 3];
        zhVerb[0] = '\0';

        if (strstr(vCore, "hovering") || strstr(vCore, "suspended")) {
          sprintf(zhVerb, "在%s上方盘旋", objTr);
        } else if (strstr(vCore, "flying")) {
          sprintf(zhVerb, "在%s上方飞行", objTr);
        } else if (strcmp(preposition, "through") == 0 ||
                   strcmp(preposition, "across") == 0) {
          if (strstr(vCore, "fleeing"))
            sprintf(zhVerb, "穿越%s逃跑", objTr);
          else if (strstr(vCore, "charging"))
            sprintf(zhVerb, "冲过%s", objTr);
          else if (strstr(vCore, "wandering"))
            sprintf(zhVerb, "穿越%s游荡", objTr);
          else if (strstr(vCore, "staggering"))
            sprintf(zhVerb, "踉跄穿越%s", objTr);
          else if (strstr(vCore, "gliding"))
            sprintf(zhVerb, "在%s中滑翔", objTr);
          else if (strstr(vCore, "drifting"))
            sprintf(zhVerb, "在%s中漂流", objTr);
          else if (strstr(vCore, "swimming"))
            sprintf(zhVerb, "游过%s", objTr);
          else if (strstr(vCore, "following"))
            sprintf(zhVerb, "穿越%s跟随你", objTr);
          else
            sprintf(zhVerb, "穿越%s", objTr);
        } else if (strcmp(preposition, "under") == 0) {
          if (strstr(vCore, "gliding"))
            sprintf(zhVerb, "在%s下滑行", objTr);
          else if (strstr(vCore, "drifting"))
            sprintf(zhVerb, "在%s下漂流", objTr);
          else
            sprintf(zhVerb, "在%s之下", objTr);
        } else {
          // "in" or "on"
          const char *ps = (strcmp(preposition, "in") == 0) ? "中" : "上";
          if (strstr(vCore, "standing"))
            sprintf(zhVerb, "站在%s%s", objTr, ps);
          else if (strstr(vCore, "lying"))
            sprintf(zhVerb, "躺在%s%s", objTr, ps);
          else if (strstr(vCore, "sleeping"))
            sprintf(zhVerb, "在%s%s睡眠", objTr, ps);
          else if (strstr(vCore, "resting"))
            sprintf(zhVerb, "在%s%s休息", objTr, ps);
          else if (strstr(vCore, "wandering"))
            sprintf(zhVerb, "在%s%s游荡", objTr, ps);
          else if (strstr(vCore, "fleeing"))
            sprintf(zhVerb, "在%s%s逃跑", objTr, ps);
          else if (strstr(vCore, "charging"))
            sprintf(zhVerb, "在%s%s猛冲", objTr, ps);
          else if (strstr(vCore, "staggering"))
            sprintf(zhVerb, "在%s%s踉跄前行", objTr, ps);
          else if (strstr(vCore, "gliding"))
            sprintf(zhVerb, "在%s%s滑行", objTr, ps);
          else if (strstr(vCore, "drifting"))
            sprintf(zhVerb, "在%s%s漂流", objTr, ps);
          else if (strstr(vCore, "swimming"))
            sprintf(zhVerb, "在%s%s游泳", objTr, ps);
          else if (strstr(vCore, "struggling"))
            sprintf(zhVerb, "在%s%s挣扎", objTr, ps);
          else if (strstr(vCore, "following"))
            sprintf(zhVerb, "在%s%s跟随你", objTr, ps);
          else if (strstr(vCore, "embedded"))
            sprintf(zhVerb, "嵌入%s中", objTr);
          else if (strstr(vCore, "trapped"))
            sprintf(zhVerb, "被困于%s中", objTr);
          else if (strstr(vCore, "shackled"))
            sprintf(zhVerb, "被锁链锁定在%s%s", objTr, ps);
          else if (strstr(vCore, "frozen"))
            sprintf(zhVerb, "被冻结在%s%s", objTr, ps);
          else if (strstr(vCore, "entangled"))
            sprintf(zhVerb, "在%s%s被缠绕", objTr, ps);
          else
            sprintf(zhVerb, "在%s%s", objTr, ps);
        }

        if (isBurning) {
          sprintf(buf, "%s%s（燃烧中）。", subject, zhVerb);
        } else {
          sprintf(buf, "%s%s。", subject, zhVerb);
        }
        restoreRNG;
        return;
      }
    ...
    if (theItem) {
      ...
      if (player.status[STATUS_HALLUCINATING] && !rogue.playbackOmniscience) {
        ...
        strcpy(verb, (theItem->quantity > 1 || (theItem->category & GOLD))
                         ? "are"
                         : "is");
      }
      ...
      if (chineseUi) {
        // Direct Chinese construction: "<item>躺在/漂浮在<terrain>中/上。"
        const char *objTr = tr(object);
        boolean isDrifting = (strstr(verb, "drifting") != NULL);
        boolean isEnclosed = (strstr(verb, "enclosed") != NULL);
        // "in" / "through" → 中; "on" / "across" → 上
        const char *ps = (strcmp(preposition, "in") == 0 ||
                          strcmp(preposition, "through") == 0)
                             ? "中"
                             : "上";
        if (isEnclosed) {
          sprintf(buf, "%s封闭在%s中。", subject, objTr);
        } else if (isDrifting) {
          sprintf(buf, "%s在%s%s漂浮。", subject, objTr, ps);
        } else {
          sprintf(buf, "%s躺在%s%s。", subject, objTr, ps);
        }
        restoreRNG;
        return;
      }
      ...
      if (chineseUi) {
        sprintf(buf, "%s%s。",
                (playerCanDirectlySee(x, y) ? "你看到了" : "你感知到了"),
                tr(object));
      } else {
        sprintf(buf, tr("you %s %s."),
                (playerCanDirectlySee(x, y) ? tr("see") : tr("sense")),
                tr(object));
      }
      restoreRNG;
      return;
...
void useKeyAt(item *theItem, short x, short y) {
  ...
  disposable = false;
  for (i = 0; i < KEY_ID_MAXIMUM &&
              (theItem->keyLoc[i].loc.x || theItem->keyLoc[i].machine);
       i++) {
    if (posEq(theItem->keyLoc[i].loc, (pos){x, y}) &&
...
short randValidDirectionFrom(creature *monst, short x, short y,
  ...
  if (count == 0) {
    // Rare, and important in this case that the function returns BEFORE a
    // random roll is made to avoid OOS.
    return NO_DIRECTION;
  }
...
void vomit(creature *monst) {
  ...
  spawnDungeonFeature(monst->loc.x, monst->loc.y,
                      &dungeonFeatureCatalog[DF_VOMIT], true, false);

  if (canDirectlySeeMonster(monst) && !rogue.automationActive) {

    monsterName(monstName, monst, true);
    sprintf(buf, "%s vomit%s profusely", monstName,
            (monst == &player ? "" : "s"));
...
static void moveEntrancedMonsters(enum directions dir) {
  ...
  for (creatureIterator it = iterateCreatures(monsters); hasNextCreature(it);) {
    ...
    if (monst->status[STATUS_ENTRANCED] && !monst->status[STATUS_STUCK] &&
...
void freeCaptive(creature *monst) {
  ...
  if (chineseUi) {
    sprintf(buf, "你释放了感激不尽的%s，获得了一位忠实的盟友。", monstName);
  } else {
    sprintf(buf, "you free the grateful %s and gain a faithful ally.",
            monstName);
  }
  message(buf, 0);
}
...
static boolean abortAttackAgainstDiscordantAlly(const creature *hitList[8]) {
  ...
  for (int i = 0; i < 8; i++) {
    if (hitList[i] && hitList[i]->creatureState == MONSTER_ALLY &&
        hitList[i]->status[STATUS_DISCORDANT] && canSeeMonster(hitList[i])) {
...
/// @brief Determines if a player attack against the given creature(s) should be
/// aborted. A confirmation dialog is shown when attempting to attack an acidic
/// monster or discordant ally, unless confused or hallucinating (but not
/// telepathic).
/// @param hitList the creature(s) getting attacked
/// @return true to abort the attack
static boolean abortAttack(const creature *hitList[8]) {
  ...
  if (player.status[STATUS_CONFUSED] || (player.status[STATUS_HALLUCINATING] &&
  ...
  return false; // either the player confirmed the attack or the confirmation
...
boolean handleWhipAttacks(creature *attacker, enum directions dir,
  ...
  // The neighboring position in the attack direction must not be diagonally
  // blocked. Generally speaking, the attacker must be able to move one tile in
  // the attack direction.
  if (diagonalBlocked(originLoc.x, originLoc.y, targetLoc.x, targetLoc.y,
                      attacker == &player)) {
    return false;
  }
  ...
  getImpactLoc(&strikeLoc, originLoc, targetLoc, 5, false,
               &boltCatalog[BOLT_WHIP]);

  defender = monsterAtLoc(strikeLoc);
  if (defender && (attacker != &player || canSeeMonster(defender)) &&
...
boolean handleSpearAttacks(creature *attacker, enum directions dir,
  ...
  if (diagonalBlocked(attacker->loc.x, attacker->loc.y, neighborLoc.x,
                      neighborLoc.y, attacker == &player)) {
    return false;
  }

  for (i = 0; i < range; i++) {
    const pos targetLoc = (pos){attacker->loc.x + (1 + i) * nbDirs[dir][0],
    ...
    */
    ...
    if (cellHasTerrainFlag(targetLoc,
    ...
    if (!rogue.playbackFastForward) {
      for (i = 0; i < range; i++) {
        ...
        if (isPosInMap(targetLoc) &&
          ...
          visualEffect = true;
          plotForegroundChar(boltChar[dir], targetLoc.x, targetLoc.y,
                             &lightBlue, true);
        }
      }
    }
...
static void buildFlailHitList(const short x, const short y, const short newX,
                              const short newY, const creature *hitList[16]) {
  short mx, my;
  short i = 0;
...
boolean playerMoves(short direction) {
  ...
  boolean playerMoved = false, specialAttackAborted = false,
          anyAttackHit = false;
  creature *defender = NULL, *tempMonst = NULL;
  const creature *hitList[16] = {NULL};
  char monstName[COLS];
  char buf[COLS * 3];
  const int directionKeys[8] = {UP_KEY,      DOWN_KEY,     LEFT_KEY,
                                RIGHT_KEY,   UPLEFT_KEY,   DOWNLEFT_KEY,
                                UPRIGHT_KEY, DOWNRIGHT_KEY};
  ...
  boolean committed =
      false; // as long as this is false, the keystroke can be cancelled

  if (player.status[STATUS_CONFUSED]) {
    // Confirmation dialog if you're moving while confused and you're next to
    // lava and not levitating or immune to fire.
    if (player.status[STATUS_LEVITATING] <= 1 &&
      ...
      for (i = 0; i < 8; i++) {
        ...
        if (coordinatesAreInMap(newestX, newestY) &&
            ...
            !cellHasTerrainFlag((pos){newestX, newestY},
                                T_OBSTRUCTS_PASSABILITY | T_ENTANGLES) &&
            !((pmap[newestX][newestY].flags & HAS_MONSTER) &&
              canSeeMonster(monsterAtLoc((pos){newestX, newestY})) &&
              monsterAtLoc((pos){newestX, newestY})->creatureState !=
                  MONSTER_ALLY)) {
  ...
  // If there's no enemy at the movement location that the player is aware of,
  // consider terrain promotions.
  if (!defender || (!canSeeMonster(defender) && !monsterRevealed(defender)) ||
      !monstersAreEnemies(&player, defender)) {

    if (cellHasTerrainFlag((pos){newX, newY}, T_OBSTRUCTS_PASSABILITY) &&
        cellHasTMFlag((pos){newX, newY}, TM_PROMOTES_ON_PLAYER_ENTRY)) {
      layer = layerWithTMFlag(newX, newY, TM_PROMOTES_ON_PLAYER_ENTRY);
      if (tileCatalog[pmap[newX][newY].layers[layer]].flags &
  ...
  if (((!cellHasTerrainFlag((pos){newX, newY}, T_OBSTRUCTS_PASSABILITY) ||
        (cellHasTMFlag((pos){newX, newY}, TM_PROMOTES_WITH_KEY) &&
         keyInPackFor((pos){newX, newY}))) &&
       !diagonalBlocked(x, y, newX, newY, false) &&
       (!cellHasTerrainFlag((pos){x, y}, T_OBSTRUCTS_PASSABILITY) ||
        (cellHasTMFlag((pos){x, y}, TM_PROMOTES_WITH_KEY) &&
         keyInPackFor((pos){x, y})))) ||
    ...
    if (handleWhipAttacks(&player, direction, &specialAttackAborted) ||
      ...
      playerTurnEnded();
      return true;
    } else if (specialAttackAborted) { // Canceled an attack against an acidic
                                       // monster or discordant ally
    ...
    if (defender) {
      ...
      if (defender->bookkeepingFlags & MB_CAPTIVE) {
        ...
        if (chineseUi) {
          sprintf(buf, "释放被俘的%s吗？", monstName);
        } else {
          sprintf(buf, "Free the captive %s?", monstName);
        }
        if (committed || confirm(buf, false)) {
          committed = true;
          if (cellHasTMFlag((pos){newX, newY}, TM_PROMOTES_WITH_KEY) &&
              keyInPackFor((pos){newX, newY})) {
      ...
      if (defender->creatureState != MONSTER_ALLY ||
        ...
        // We separate this tallying phase from the actual attacking phase
        // because sometimes the attacks themselves create more monsters, and
        // those shouldn't be attacked in the same turn.
    ...
    if (player.bookkeepingFlags & MB_SEIZED) {
      for (creatureIterator it = iterateCreatures(monsters);
        ...
        if ((tempMonst->bookkeepingFlags & MB_SEIZING) &&
            ...
            !diagonalBlocked(player.loc.x, player.loc.y, tempMonst->loc.x,
                             tempMonst->loc.y, false) &&
            !tempMonst->status[STATUS_ENTRANCED]) {

          monsterName(monstName, tempMonst, true);
          if (committed || !canSeeMonster(tempMonst)) {
            committed = true;
            sprintf(buf, "you struggle but %s is holding your legs!",
                    monstName);
            moveEntrancedMonsters(direction);
            message(buf, 0);
            playerTurnEnded();
            ...
            sprintf(buf, "you cannot move; %s is holding your legs!",
                    monstName);
            message(buf, 0);
            cancelKeystroke();
            return false;
    ...
    if (pmap[newX][newY].flags & (DISCOVERED | MAGIC_MAPPED) &&
    ...
    } else if (playerCanSee(newX, newY) && !player.status[STATUS_CONFUSED] &&
      ...
      return false;
               ...
               !player.status[STATUS_BURNING] &&
               cellHasTerrainFlag((pos){newX, newY},
                                  T_CAUSES_CONFUSION | T_CAUSES_PARALYSIS) &&
               (!rogue.armor || !(rogue.armor->flags & ITEM_RUNIC) ||
    ...
    if (rogue.weapon && (rogue.weapon->flags & ITEM_LUNGE_ATTACKS)) {
      ...
      if (coordinatesAreInMap(newestX, newestY) &&
        ...
        if (tempMonst &&
            ...
            (!cellHasTerrainFlag(tempMonst->loc, T_OBSTRUCTS_PASSABILITY) ||
    ...
    if (player.status[STATUS_STUCK] &&
        cellHasTerrainFlag((pos){x, y}, T_ENTANGLES)) {
    ...
    if (rogue.downLoc.x == newX && rogue.downLoc.y == newY) {
      ...
      if (defender &&
          defender->creatureState == MONSTER_ALLY) { // Swap places with ally.
        pmapAt(defender->loc)->flags &= ~HAS_MONSTER;
        defender->loc.x = x;
        defender->loc.y = y;
        if (monsterAvoids(defender, (pos){x, y})) {
          defender->loc = getQualifyingPathLocNear(
              player.loc, true, forbiddenFlagsForMonster(&(defender->info)), 0,
              0, (HAS_PLAYER | HAS_MONSTER | HAS_STAIRS), false);
        }
        // getQualifyingLocNear(loc, player.loc.x, player.loc.y, true, NULL,
        // forbiddenFlagsForMonster(&(defender->info)) & ~(T_IS_DF_TRAP |
        // T_IS_DEEP_WATER | T_SPONTANEOUSLY_IGNITES), HAS_MONSTER, false,
        // false); defender->loc.x = loc[0]; defender->loc.y = loc[1];
        pmapAt(defender->loc)->flags |= HAS_MONSTER;
      }
      ...
      for (i = 0; i < 16; i++) {
        if (hitList[i]) {
          if (attack(&player, hitList[i],
                     (rogue.weapon &&
                      (rogue.weapon->flags & ITEM_LUNGE_ATTACKS)))) {
            anyAttackHit = true;
          }
        }
      }
    ...
    i = pmap[newX][newY]
            .layers[layerWithFlag(newX, newY, T_OBSTRUCTS_PASSABILITY)];
...
void dijkstraScan(short **distanceMap, char passMap[DCOLS][DROWS], boolean
allowDiagonals) { short i, j, maxDir; enum directions dir; boolean
somethingChanged;
            ...
            for (j=1; j<DROWS-1; j++) {
                if (!passMap || passMap[i][j]) {
                    for (dir = 0; dir < maxDir; dir++) {
                        if (coordinatesAreInMap(i + nbDirs[dir][0], j +
nbDirs[dir][1])
                            && (!passMap || passMap[i + nbDirs[dir][0]][j +
nbDirs[dir][1]])
                            && distanceMap[i + nbDirs[dir][0]][j +
nbDirs[dir][1]] >= distanceMap[i][j] + 2) { distanceMap[i + nbDirs[dir][0]][j +
nbDirs[dir][1]] = distanceMap[i][j] + 1; somethingChanged = true;
                        }
                    }
                }
            }
...
// removes the lowest value from the queue, populates x/y/value variables and
updates min caching void dequeue(short *x, short *y, short *val, distanceQueue
*dQ) { short i, minIndex;
    ...
    *val = dQ->qMinVal;
    ...
    for (minIndex = dQ->qLen - 1; minIndex >= 0 && dQ->qVal[minIndex] != *val;
minIndex--);
...
void dijkstraScan(short **distanceMap, char passMap[DCOLS][DROWS], boolean
allowDiagonals) { short i, j, maxDir, val; enum directions dir; distanceQueue
dQ;
    ...
    while (dQ.qLen) {
        dequeue(&i, &j, &val, &dQ);
        if (distanceMap[i][j] == val) { // if it hasn't been improved since
joining the queue for (dir = 0; dir < maxDir; dir++) { if (coordinatesAreInMap(i
+ nbDirs[dir][0], j + nbDirs[dir][1])
                    && (!passMap || passMap[i + nbDirs[dir][0]][j +
nbDirs[dir][1]])
                    && distanceMap[i + nbDirs[dir][0]][j + nbDirs[dir][1]] >=
distanceMap[i][j] + 2) {

                    distanceMap[i + nbDirs[dir][0]][j + nbDirs[dir][1]] =
distanceMap[i][j] + 1;

                    enqueue(i + nbDirs[dir][0], j + nbDirs[dir][1],
distanceMap[i][j] + 1, &dQ);
                }
            }
        }
    }
...
/*
void calculateDistances(short **distanceMap, short destinationX, short
destinationY, unsigned long blockingTerrainFlags, creature *traveler) { short i,
j; boolean somethingChanged;

    for (i=0; i<DCOLS; i++) {
        for (j=0; j<DROWS; j++) {
            distanceMap[i][j] = ((traveler && traveler == &player &&
!(pmap[i][j].flags & (DISCOVERED | MAGIC_MAPPED)))
                                 || ((traveler && monsterAvoids(traveler,
(pos){i, j}))
                                     || cellHasTerrainFlag((pos){ i, j },
blockingTerrainFlags))) ? -1 : 30000;
        }
    }
...
short nextStep(short **distanceMap, pos target, creature *monst,
               boolean preferDiagonals) {
  ...
  for (dir = (preferDiagonals ? 7 : 0);
    ...
    if (coordinatesAreInMap(newX, newY)) {
      ...
      if (monst && monsterAvoids(monst, (pos){newX, newY})) {

        blocked = true;
      } else if (monst && blocker && !canPass(monst, blocker) &&
                 !monstersAreTeammates(monst, blocker) &&
                 !monstersAreEnemies(monst, blocker)) {
        blocked = true;
      }
      if ((distanceMap[target.x][target.y] - distanceMap[newX][newY]) >
              bestScore &&
          !diagonalBlocked(target.x, target.y, newX, newY, monst == &player) &&
          knownToPlayerAsPassableOrSecretDoor((pos){newX, newY}) && !blocked) {
...
static void displayRoute(short **distanceMap, boolean removeRoute) {
  ...
  if (distanceMap[player.loc.x][player.loc.y] < 0 ||
      distanceMap[player.loc.x][player.loc.y] == 30000) {
    return;
  }
  do {
    if (removeRoute) {
...
void travelRoute(pos path[1000], short steps) {
  ...
  for (i = 0; i < steps && !rogue.disturbed; i++) {
    for (j = i + 1; j < steps - 1; j++) {
      // Check to see if the path has become obstructed or avoided since the
      // last time we saw it.
      if (diagonalBlocked(path[j - 1].x, path[j - 1].y, path[j].x, path[j].y,
                          true) ||
...
void travel(pos target, boolean autoConfirm) {
  ...
  if (D_WORMHOLING) {
    recordMouseClick(mapToWindowX(target.x), mapToWindowY(target.y), true,
                     false);
    pmapAt(player.loc)->flags &= ~HAS_PLAYER;
    refreshDungeonCell(player.loc);
    player.loc = target;
  ...
  if (abs(player.loc.x - target.x) + abs(player.loc.y - target.y) == 1) {
    ...
    for (int i = 0; i < 4; i++) {
      if (nbDirs[i][0] == (target.x - player.loc.x) &&
          nbDirs[i][1] == (target.y - player.loc.y)) {
        playerMoves(i);
        break;
      }
    }
  ...
  if (distanceMap[player.loc.x][player.loc.y] < 30000) {
    if (autoConfirm) {
      ...
      } while (theEvent.eventType != MOUSE_UP &&
      ...
      if ((theEvent.eventType == MOUSE_UP &&
           windowToMapX(theEvent.param1) == target.x &&
           windowToMapY(theEvent.param2) == target.y) ||
          (theEvent.eventType == KEYSTROKE &&
           (theEvent.param1 == 'Y' || theEvent.param1 == 'y' ||
    ...
    for (j = 0; j < DROWS; j++) {
      if (cellHasTerrainFlag((pos){i, j}, T_OBSTRUCTS_PASSABILITY) &&
          (!cellHasTMFlag((pos){i, j}, TM_IS_SECRET) ||
           (discoveredTerrainFlagsAtLoc((pos){i, j}) &
            T_OBSTRUCTS_PASSABILITY))) {

        costMap[i][j] =
            cellHasTerrainFlag((pos){i, j}, T_OBSTRUCTS_DIAGONAL_MOVEMENT)
                ? PDS_OBSTRUCTION
                : PDS_FORBIDDEN;
      } else if (cellHasTerrainFlag((pos){i, j},
                                    T_PATHING_BLOCKER &
                                        ~T_OBSTRUCTS_PASSABILITY)) {
...
void getLocationFlags(const short x, const short y, unsigned long *tFlags,
  ...
  if (limitToPlayerKnowledge &&
      (pmap[x][y].flags & (DISCOVERED | MAGIC_MAPPED)) && !playerCanSee(x, y)) {
  ...
  for (i = 0; i < DCOLS; i++) {
    for (j = 0; j < DROWS; j++) {
      ...
      if ((tFlags & T_OBSTRUCTS_PASSABILITY) &&
        ...
        costMap[i][j] = (tFlags & T_OBSTRUCTS_DIAGONAL_MOVEMENT)
                            ? PDS_OBSTRUCTION
                            : PDS_FORBIDDEN;
        continue;
      }
      ...
      if ((tFlags & T_LAVA_INSTA_DEATH) &&
          !(monst->info.flags &
            (MONST_IMMUNE_TO_FIRE | MONST_FLIES | MONST_INVULNERABLE)) &&
          (monst->status[STATUS_LEVITATING] ||
           monst->status[STATUS_IMMUNE_TO_FIRE]) &&
          max(monst->status[STATUS_LEVITATING],
              monst->status[STATUS_IMMUNE_TO_FIRE]) <
              (rogue.mapToShore[i][j] +
               distanceBetween((pos){i, j}, monst->loc) * monst->movementSpeed /
                   100)) {
        // Only a temporary effect will permit the monster to survive the lava,
        // and the remaining duration either isn't enough to get it to the spot,
        // or it won't suffice to let it return to shore if it does get there.
        // Treat these locations as obstacles.
        costMap[i][j] = PDS_FORBIDDEN;
        continue;
      }

      if (((tFlags & T_AUTO_DESCENT) ||
           (tFlags & T_IS_DEEP_WATER) &&
          ...
          monst->status[STATUS_LEVITATING] <
              (rogue.mapToShore[i][j] +
               distanceBetween((pos){i, j}, monst->loc) * monst->movementSpeed /
                   100)) {
        // Only a temporary effect will permit the monster to levitate over the
        // chasm/water, and the remaining duration either isn't enough to get it
        // to the spot, or it won't suffice to let it return to shore if it does
        // get there. Treat these locations as obstacles.
        costMap[i][j] = PDS_FORBIDDEN;
        continue;
      }
      ...
      if (cFlags & HAS_MONSTER) {
        ...
        if (currentTenant &&
            (currentTenant->info.flags &
             (MONST_IMMUNE_TO_WEAPONS | MONST_INVULNERABLE)) &&
      ...
      if (!(monst->info.flags & MONST_INVULNERABLE)) {
        if ((tFlags & T_CAUSES_NAUSEA) ||
            ...
            (tFlags & T_ENTANGLES) &&
                !(monst->info.flags & MONST_IMMUNE_TO_WEBS)) {
...
enum directions adjacentFightingDir() {
  ...
  for (enum directions dir = 0; dir < DIRECTION_COUNT; dir++) {
    ...
    if (monst && canSeeMonster(monst) &&
        (!diagonalBlocked(player.loc.x, player.loc.y, newLoc.x, newLoc.y,
                          false) ||
         (monst->info.flags & MONST_ATTACKABLE_THRU_WALLS)) &&
...
#define exploreGoalValue(x, y)                                                 \
  (0 - abs((x) - DCOLS / 2) / 3 - abs((x) - DCOLS / 2) / 4)

void getExploreMap(short **map,
                   boolean headingToStairs) { // calculate explore map
  short i, j;
  short **costMap;
  item *theItem;
    ...
    for (j = 0; j < DROWS; j++) {
      ...
      if (!(pmap[i][j].flags & DISCOVERED)) {
        if ((pmap[i][j].flags & MAGIC_MAPPED) &&
            (tileCatalog[pmap[i][j].layers[DUNGEON]].flags |
             tileCatalog[pmap[i][j].layers[LIQUID]].flags) &
                T_PATHING_BLOCKER) {
          // Magic-mapped cells revealed as obstructions should be treated as
          // such even though they're not discovered.
          costMap[i][j] =
              cellHasTerrainFlag((pos){i, j}, T_OBSTRUCTS_DIAGONAL_MOVEMENT)
                  ? PDS_OBSTRUCTION
                  : PDS_FORBIDDEN;
      ...
      } else if (theItem && !monsterAvoids(&player, (pos){i, j})) {
...
boolean explore(short frameDelay) {
  ...
  if (dir != NO_DIRECTION &&
      startFighting(dir,
                    (player.status[STATUS_HALLUCINATING] ? true : false))) {

    return true;
  }

  if (!rogue.autoPlayingLevel) {
    message(KEYBOARD_LABELS ? "Exploring... press any key to stop."
                            : "Exploring... touch anywhere to stop.",
            0);
    // A little hack so the exploring message remains bright while exploring and
    // then auto-dims when another message is displayed:
    confirmMessages();
    printString(KEYBOARD_LABELS ? "Exploring... press any key to stop."
                                : "Exploring... touch anywhere to stop.",
...
void autoPlayLevel(boolean fastForward) {
  ...
  confirmMessages();
  message(KEYBOARD_LABELS ? "Playing... press any key to stop."
  ...
  do {
    ...
    if (!madeProgress && rogue.downLoc.x == player.loc.x &&
        rogue.downLoc.y == player.loc.y) {
      useStairs(1);
      madeProgress = true;
    }
...
boolean startFighting(enum directions dir, boolean tillDeath) {
  ...
  int expectedDamage = monst->info.damage.upperBound *
                       monsterDamageAdjustmentAmount(monst) / FP_FACTOR;
  ...
  } while (!rogue.disturbed && !rogue.gameHasEnded &&
           (tillDeath || player.currentHP > expectedDamage) &&
           (pmapAt(neighborLoc)->flags & HAS_MONSTER) &&
           monsterAtLoc(neighborLoc) == monst);
  ...
  if (cellHasTMFlag((pos){x, y}, TM_IS_SECRET)) {
    ...
    for (layer = 0; layer < NUMBER_TERRAIN_LAYERS; layer++) {
      if (tileCatalog[pmap[x][y].layers[layer]].mechFlags & TM_IS_SECRET) {
        feat = &dungeonFeatureCatalog[tileCatalog[pmap[x][y].layers[layer]]
                                          .discoverType];
...
boolean search(short searchStrength) {
  ...
  for (i = x - radius; i <= x + radius; i++) {
    for (j = y - radius; j <= y + radius; j++) {
      if (coordinatesAreInMap(i, j) && playerCanDirectlySee(i, j)) {

        percent =
            searchStrength - distanceBetween((pos){x, y}, (pos){i, j}) * 10;
        if (cellHasTerrainFlag((pos){i, j}, T_OBSTRUCTS_PASSABILITY)) {
          percent = percent * 2 / 3;
        }
        if (percent >= 100) {
...
boolean useStairs(short stairDirection) {
  ...
  if (stairDirection == 1) {
    if (rogue.depthLevel < gameConst->deepestLevel) {
      ...
      // irisFadeBetweenBuffers(fromBuf, toBuf, mapToWindowX(player.loc.x),
      // mapToWindowY(player.loc.y), 20, false);
    } else if (numberOfMatchingPackItems(AMULET, 0, 0, false)) {
      victory(true);
    } else {
      confirmMessages();
      messageWithColor(
          "the crystal archway repels you with a mysterious force!", &lightBlue,
          0);
      messageWithColor("(Only the bearer of the Amulet of Yendor may pass.)",
    ...
    if (rogue.depthLevel > 1 ||
      ...
      if (rogue.depthLevel == 0) {
        ...
        startLevel(rogue.depthLevel + 1, stairDirection);
        // irisFadeBetweenBuffers(fromBuf, toBuf, mapToWindowX(player.loc.x),
        // mapToWindowY(player.loc.y), 20, true);
      }
      succeeded = true;
    } else {
      confirmMessages();
...
void storeMemories(const short x, const short y) {
  ...
  pmap[x][y].rememberedTerrain =
      pmap[x][y].layers[highestPriorityLayer(x, y, false)];
}
  ...
  for (i = 0; i < DCOLS; i++) {
    for (j = DROWS - 1; j >= 0; j--) {
      if (pmap[i][j].flags & IN_FIELD_OF_VIEW &&
          (max(0, tmap[i][j].light[0]) + max(0, tmap[i][j].light[1]) +
               max(0, tmap[i][j].light[2]) >
      ...
      if ((pmap[i][j].flags & VISIBLE) &&
          !(pmap[i][j].flags &
            WAS_VISIBLE)) { // if the cell became visible this move
        if (!(pmap[i][j].flags & DISCOVERED) && rogue.automationActive) {
          if (pmap[i][j].flags & HAS_ITEM) {
            ...
            if (theItem && (theItem->category & KEY)) {
              ...
              if (chineseUi)
                sprintf(buf, "你看到了%s。", tr(name));
              else
                sprintf(buf, tr("you see %s."), name);
              messageWithColor(buf, &itemMessageColor, 0);
            }
          }
          if (!(pmap[i][j].flags & MAGIC_MAPPED) &&
            ...
            strcpy(name,
                   tileCatalog[pmap[i][j].layers[layerWithTMFlag(
                                   i, j, TM_INTERRUPT_EXPLORATION_WHEN_SEEN)]]
                       .description);
            if (chineseUi)
              sprintf(buf, "你看到了%s。", tr(name));
            else
        ...
        if (refreshDisplay) {
          refreshDungeonCell((pos){i, j});
                 ...
                 (pmap[i][j].flags &
                  WAS_VISIBLE)) { // if the cell ceased being visible this move
        storeMemories(i, j);
        if (refreshDisplay) {
          refreshDungeonCell((pos){i, j});
        }
      } else if (!(pmap[i][j].flags & CLAIRVOYANT_VISIBLE) &&
                 (pmap[i][j].flags &
                  WAS_CLAIRVOYANT_VISIBLE)) { // ceased being clairvoyantly
                                              // visible
        storeMemories(i, j);
        if (refreshDisplay) {
          refreshDungeonCell((pos){i, j});
        }
      } else if (!(pmap[i][j].flags & WAS_CLAIRVOYANT_VISIBLE) &&
                 (pmap[i][j].flags &
                  CLAIRVOYANT_VISIBLE)) { // became clairvoyantly visible
        pmap[i][j].flags &= ~STABLE_MEMORY;
        if (refreshDisplay) {
          refreshDungeonCell((pos){i, j});
        }
      } else if (!(pmap[i][j].flags & TELEPATHIC_VISIBLE) &&
                 (pmap[i][j].flags &
                  WAS_TELEPATHIC_VISIBLE)) { // ceased being telepathically
                                             // visible
        storeMemories(i, j);
        if (refreshDisplay) {
          refreshDungeonCell((pos){i, j});
        }
      } else if (!(pmap[i][j].flags & WAS_TELEPATHIC_VISIBLE) &&
                 (pmap[i][j].flags &
                  TELEPATHIC_VISIBLE)) { // became telepathically visible
        if (!(pmap[i][j].flags & DISCOVERED) &&
        ...
        if (refreshDisplay) {
          refreshDungeonCell((pos){i, j});
                 ...
                 (tmap[i][j].light[0] != tmap[i][j].oldLight[0] ||
                  ...
                  tmap[i][j].light[2] !=
                      tmap[i][j].oldLight[2])) { // if the cell's light color
                                                 // changed this move
        ...
        if (refreshDisplay) {
          ...
          updateDancingTerrain && playerCanSee(i, j) &&
...
// Returns a boolean grid indicating whether each square is in the field of view
// of (loc.x, loc.y). forbiddenTerrain is the set of terrain flags that will
// block vision (but the blocking cell itself is illuminated); forbiddenFlags is
// the set of map flags that will block vision. If cautiousOnWalls is set, we
// will not illuminate blocking tiles unless the tile one space closer to the
// origin is visible to the player; this is to prevent lights from illuminating
// a wall when the player is on the other side of the wall.
void getFOVMask(char grid[DCOLS][DROWS], short xLoc, short yLoc,
  ...
  for (int i = 1; i <= 8; i++) {
    scanOctantFOV(grid, loc.x, loc.y, i, maxRadius, 1,
                  LOS_SLOPE_GRANULARITY * -1, 0, forbiddenTerrain,
...
void scanOctantFOV(char grid[DCOLS][DROWS], short xLoc, short yLoc,
  ...
  short i, a, b, iStart, iEnd, x, y, x2,
      y2; // x and y are temporary variables on which we do the octant transform
  long newStartSlope, newEndSlope;
  boolean cellObstructed;

  newStartSlope = startSlope;

  a = ((LOS_SLOPE_GRANULARITY / -2 + 1) + startSlope * columnsRightFromOrigin) /
      LOS_SLOPE_GRANULARITY;
  b = ((LOS_SLOPE_GRANULARITY / -2 + 1) + endSlope * columnsRightFromOrigin) /
      LOS_SLOPE_GRANULARITY;
  ...
  if ((columnsRightFromOrigin * columnsRightFromOrigin + iEnd * iEnd) >=
      maxRadius * maxRadius / FP_FACTOR / FP_FACTOR) {
    return;
  }
  if ((columnsRightFromOrigin * columnsRightFromOrigin + iStart * iStart) >=
      maxRadius * maxRadius / FP_FACTOR / FP_FACTOR) {
    iStart = (int)(-1 *
                   fp_sqrt((maxRadius * maxRadius / FP_FACTOR) -
                           (columnsRightFromOrigin * columnsRightFromOrigin *
                            FP_FACTOR)) /
                   FP_FACTOR);
  }
  ...
  boolean currentlyLit = coordinatesAreInMap(x, y) &&
                         !(cellHasTerrainFlag((pos){x, y}, forbiddenTerrain) ||
  ...
  for (i = iStart; i <= iEnd; i++) {
    ...
    cellObstructed = (cellHasTerrainFlag((pos){x, y}, forbiddenTerrain) ||
                      (pmap[x][y].flags & forbiddenFlags));
    // if we're cautious on walls and this is a wall:
    if (cautiousOnWalls && cellObstructed) {
      // (x2, y2) is the tile one space closer to the origin from the tile we're
      // on:
    ...
    if (!cellObstructed && !currentlyLit) { // next column slope starts here
      newStartSlope =
          (long int)((LOS_SLOPE_GRANULARITY * (i)-LOS_SLOPE_GRANULARITY / 2) /
      ...
      if (newStartSlope <= newEndSlope) {
        // run next column
        scanOctantFOV(grid, loc.x, loc.y, octant, maxRadius,
                      columnsRightFromOrigin + 1, newStartSlope, newEndSlope,
