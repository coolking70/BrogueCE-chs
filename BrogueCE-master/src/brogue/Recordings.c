...
static const long keystrokeTable[] = {
    UP_ARROW,   LEFT_ARROW, DOWN_ARROW, RIGHT_ARROW, ESCAPE_KEY, RETURN_KEY,
    DELETE_KEY, TAB_KEY,    NUMPAD_0,   NUMPAD_1,    NUMPAD_2,   NUMPAD_3,
    NUMPAD_4,   NUMPAD_5,   NUMPAD_6,   NUMPAD_7,    NUMPAD_8,   NUMPAD_9};

static const int keystrokeCount =
    sizeof(keystrokeTable) / sizeof(*keystrokeTable);

enum recordingSeekModes { RECORDING_SEEK_MODE_TURN, RECORDING_SEEK_MODE_DEPTH };
...
static void recordChar(unsigned char c) {
  if (locationInRecordingBuffer < INPUT_RECORD_BUFFER_MAX_SIZE) {
    ...
    printf(
        "Recording buffer length exceeded at location %li! Turn number %li.\n",
...
static void numberToString(uint64_t number, short numberOfBytes,
                           unsigned char *recordTo) {
  short i;
  uint64_t n;
...
// All other events: Event type, x-coordinate of the event, y-coordinate of the
// event, modifier flags. (4 bytes.) Note: these must be sanitized, because user
// input may contain more than one byte per parameter.
...
static void loadNextAnnotation() {
  ...
  for (;;) {
    ...
    if (currentReadTurn > rogue.playerTurnNumber ||
        (currentReadTurn <= 1 && rogue.playerTurnNumber <= 1 &&
         currentReadTurn >= rogue.playerTurnNumber)) {
...
void displayAnnotation() {
  if (rogue.playbackMode &&
    ...
    if (!rogue.playbackFastForward) {
      ...
      const SavedDisplayBuffer rbuf = saveDisplayBuffer();
      printTextBox(rogue.nextAnnotation, player.loc.x, 0, 0, &black, &white,
                   NULL, 0);
...
// according to the global pattern. The Major and Minor versions must match
// ours. Returns true if successful.
static boolean getPatchVersion(char *versionString,
                               unsigned short *patchVersion) {
  return sscanf(versionString, gameConst->patchVersionPattern, patchVersion) ==
         1;
}
...
void initRecording() {
  ...
  if (rogue.playbackMode) {
    ...
    if (getPatchVersion(versionString, &recPatch) &&
        recPatch <= gameConst->patchVersion) {
      // Major and Minor match ours, Patch is less than or equal to ours: we are
      // compatible.
      rogue.patchVersion = recPatch;
    } else if (strcmp(versionString, gameConst->recordingVersionString) != 0) {
      // We have neither a compatible pattern match nor an exact match: we
      // cannot load it.
      rogue.playbackMode = false;
      rogue.playbackFastForward = false;
      ...
      if (!nonInteractivePlayback) {
        snprintf(
            buf, 1000,
            "This file is from version %s and cannot be opened in version %s.",
    ...
    rogue.howManyTurns =
        recallNumber(4); // how many turns are in this recording
    maxLevelChanges =
        recallNumber(4); // how many times the player changes depths
    ...
    flushBufferToFile(); // header info never makes it into inputRecordBuffer
                         // when recording
...
// compare a random number once per player turn so we instantly know if we are
...
static void printPlaybackHelpScreen() {
  ...
  char helpText[PLAYBACK_HELP_LINE_COUNT][80] = {
      ...
      "l or right arrow: ****advance one turn (shift for 5 turns; control for "
      "20)",
      "",
  ...
  for (i = 0; i < PLAYBACK_HELP_LINE_COUNT; i++) {
    printString(helpText[i], mapToWindowX(5), mapToWindowY(i),
                &itemMessageColor, &black, &dbuf);
  }
...
static void seek(unsigned long seekTarget, enum recordingSeekModes seekMode) {
  unsigned long progressBarRefreshInterval = 1, startTurnNumber = 0,
                targetTurnNumber = 0, avgTurnsPerLevel = 1;
  rogueEvent theEvent;
  ...
  if ((seekMode == RECORDING_SEEK_MODE_TURN &&
       seekTarget < rogue.playerTurnNumber) ||
      (seekMode == RECORDING_SEEK_MODE_DEPTH &&
       seekTarget <= rogue.depthLevel)) {
  ...
  if (useProgressBar) {
    clearDisplayBuffer(&dbuf);
    rectangularShading((COLS - 20) / 2, ROWS / 2, 20, 1, &black,
  ...
  while (!arrivedAtDestination && !rogue.gameHasEnded && !rogue.playbackOOS) {
    if (useProgressBar &&
        !(rogue.playerTurnNumber % progressBarRefreshInterval)) {
      ...
      while (pauseBrogue(
          0, PAUSE_BEHAVIOR_DEFAULT)) { // pauseBrogue(0) is necessary to flush
                                        // the display to the window in SDL
        if (rogue.gameHasEnded) {
          return;
        }
        rogue.creaturesWillFlashThisTurn =
            false; // prevent monster flashes from showing up on screen
        nextBrogueEvent(&theEvent, true, false,
                        true); // eat the input if it isn't clicking x
      }
      rogue.playbackFastForward = true;
    }

    rogue.RNG =
        RNG_COSMETIC; // dancing terrain colors can't influence recordings
    rogue.playbackDelayThisTurn = 0;
    nextBrogueEvent(&theEvent, false, true, false);
    rogue.RNG = RNG_SUBSTANTIVE;
    executeEvent(&theEvent);

    if ((seekMode == RECORDING_SEEK_MODE_DEPTH &&
         rogue.depthLevel == seekTarget) ||
        (seekMode == RECORDING_SEEK_MODE_TURN &&
         rogue.playerTurnNumber >= seekTarget)) {
...
static void promptToAdvanceToLocation(short keystroke) {
  ...
  if (rogue.playbackOOS || !rogue.playbackPaused || unpause()) {
    ...
    rogue.playbackMode = false;
    enteredText = getInputTextString(entryText, "Go to turn number: ", 9, buf,
                                     "", TEXT_INPUT_NUMBERS, false);
    confirmMessages();
    rogue.playbackMode = true;
    ...
    if (enteredText && entryText[0] != '\0') {
      ...
      if (rogue.playbackOOS && destinationFrame > rogue.playerTurnNumber) {
        ...
        seek(min(destinationFrame, rogue.howManyTurns),
...
void pausePlayback() {
  ...
  if (!rogue.playbackPaused) {
    rogue.playbackPaused = true;
    messageWithColor(KEYBOARD_LABELS ? "recording paused. Press space to play."
                                     : "recording paused.",
                     &teal, 0);
    refreshSideBar(-1, -1, false);
    // oldRNG = rogue.RNG;
...
boolean executePlaybackInput(rogueEvent *recordingInput) {
  ...
  if (recordingInput->eventType == KEYSTROKE) {
    ...
    case UP_ARROW:
    case UP_KEY:
      newDelay = max(1, min(rogue.playbackDelayPerTurn * 2 / 3,
                            rogue.playbackDelayPerTurn - 1));
    ...
    case DOWN_ARROW:
    case DOWN_KEY:
      newDelay = min(3000, max(rogue.playbackDelayPerTurn * 3 / 2,
                               rogue.playbackDelayPerTurn + 1));
      if (newDelay != rogue.playbackDelayPerTurn) {
    ...
    case LEFT_ARROW:
      ...
      if (frameCount < 0) {
        ...
        destinationFrame =
            min(rogue.playerTurnNumber + frameCount, rogue.howManyTurns);
      }
      ...
      if (hasGraphics) {
        ...
        case TILES_GRAPHICS:
          messageWithColor(KEYBOARD_LABELS
                               ? "Switched to graphical tiles. Press 'G' again "
    ...
    if (windowToMapX(x) >= 0 && windowToMapX(x) < DCOLS && y >= 0 &&
        y < MESSAGE_LINES) {
      // If the click location is in the message block, display the message
      // archive.
      rogue.playbackMode = false;
      displayMessageArchive();
      rogue.playbackMode = true;
...
// where N is the lowest counting number that doesn't collide with an existing
// file.
void getAvailableFilePath(char *returnPath, const char *defaultPath,
                          const char *suffix) {
...
static void getDefaultFilePath(char *defaultPath, boolean gameOver) {
  ...
  if (serverMode) {
    // With WebBrogue, filenames must fit into 30 bytes, including extension and
    // terminal \0. It is enough for the seed and the optional file counter. The
    // longest filename will be:
  ...
  if (!gameOver) {
    sprintf(defaultPath, "Saved %s #%s at depth %d", gameConst->versionString,
            seed, rogue.depthLevel);
  } else if (rogue.quit) {
    sprintf(defaultPath, "%s #%s Quit at depth %d", gameConst->versionString,
            seed, rogue.depthLevel);
  } else if (player.bookkeepingFlags & MB_IS_DYING) {
    sprintf(defaultPath, "%s #%s Died at depth %d", gameConst->versionString,
            seed, rogue.depthLevel);
  } else if (rogue.depthLevel > 26) {
    sprintf(defaultPath, "%s #%s Mastered the dungeons",
            gameConst->versionString, seed);
  } else {
    sprintf(defaultPath, "%s #%s Escaped the dungeons",
            gameConst->versionString, seed);
  }
...
#define MAX_TEXT_INPUT_FILENAME_LENGTH                                         \
  (COLS - 12) // max length including the suffix

void saveGame() {
  char filePathWithoutSuffix[BROGUE_FILENAME_MAX - sizeof(GAME_SUFFIX)],
      filePath[BROGUE_FILENAME_MAX], defaultPath[BROGUE_FILENAME_MAX];
  boolean askAgain;

  if (rogue.playbackMode) {
    return; // Call me paranoid, but I'd rather it be impossible to embed
            // malware in a recording.
  }
  ...
  do {
    askAgain = false;
    if (getInputTextString(
            filePathWithoutSuffix,
            "Save game as (<esc> to cancel): ", MAX_TEXT_INPUT_FILENAME_LENGTH,
            filePathWithoutSuffix, GAME_SUFFIX, TEXT_INPUT_FILENAME, true)) {
      snprintf(filePath, BROGUE_FILENAME_MAX, "%s%s", filePathWithoutSuffix,
               GAME_SUFFIX);
      if (!fileExists(filePath) ||
          confirm("File of that name already exists. Overwrite?", true)) {
        remove(filePath);
        flushBufferToFile();
        rename(currentFilePath, filePath);
        strcpy(currentFilePath, filePath);
...
void saveRecording(char *filePathWithoutSuffix) {
  ...
  showSaveButtons(); // 显示 Esc/Enter 触摸按钮

  do {
    askAgain = false;
    if (getInputTextString(
            filePathWithoutSuffix, "Save recording as (<esc> to cancel): ",
            MAX_TEXT_INPUT_FILENAME_LENGTH, filePathWithoutSuffix,
            RECORDING_SUFFIX, TEXT_INPUT_FILENAME, true)) {
    ...
    } else { // Declined to save recording; save it anyway as LastRecording, and
             // delete LastRecording if it already exists.
      snprintf(filePath, BROGUE_FILENAME_MAX, "%s%s", LAST_RECORDING_NAME,
               RECORDING_SUFFIX);
      if (fileExists(filePath)) {
        remove(filePath);
      }
  ...
  hideSaveButtons(); // 隐藏 Esc/Enter 触摸按钮
}

static void copyFile(char *fromFilePath, char *toFilePath,
                     unsigned long fromFileLength) {
  unsigned long m, n;
...
// at the end of loading a saved game, this function transitions into active
// play mode.
...
boolean loadSavedGame() {
  ...
  rogue.playbackMode = true;
  rogue.playbackFastForward = true;
  initializeRogue(0); // Calls initRecording(). Seed argument is ignored because
                      // we're initially in playback mode.
  if (!rogue.gameHasEnded) {
    blackOutScreen();
    startLevel(rogue.depthLevel, 1);
  }
  ...
  if (rogue.howManyTurns > 0) {
    ...
    while (recordingLocation < lengthOfPlaybackFile &&
           rogue.playerTurnNumber < rogue.howManyTurns && !rogue.gameHasEnded &&
           !rogue.playbackOOS) {
      ...
      if (recordingLocation / progressBarInterval !=
              previousRecordingLocation / progressBarInterval &&
          !rogue.playbackOOS) {
        rogue.playbackFastForward =
            false; // so that pauseBrogue looks for inputs
        printProgressBar((COLS - 20) / 2, ROWS / 2, "[     Loading...   ]",
                         recordingLocation, lengthOfPlaybackFile, &darkPurple,
                         false);
        while (pauseBrogue(
            0, PAUSE_BEHAVIOR_DEFAULT)) { // pauseBrogue(0) is necessary to
                                          // flush the display to the window in
                                          // SDL, as well as look for inputs
          rogue.creaturesWillFlashThisTurn =
              false; // prevent monster flashes from showing up on screen
          nextBrogueEvent(&theEvent, true, false, true);
          if (rogue.gameHasEnded || theEvent.eventType == KEYSTROKE &&
...
// the following functions are used to create human-readable descriptions of
// playback files for debugging purposes

static void describeKeystroke(unsigned char key, char *description) {
  short i;
  long c;
  const long keyList[51] = {
      UP_KEY, DOWN_KEY, LEFT_KEY, RIGHT_KEY, UP_ARROW, LEFT_ARROW, DOWN_ARROW,
      RIGHT_ARROW, UPLEFT_KEY, UPRIGHT_KEY, DOWNLEFT_KEY, DOWNRIGHT_KEY,
      DESCEND_KEY, ASCEND_KEY, REST_KEY, AUTO_REST_KEY, SEARCH_KEY,
      INVENTORY_KEY, ACKNOWLEDGE_KEY, EQUIP_KEY, UNEQUIP_KEY, APPLY_KEY,
      THROW_KEY, RELABEL_KEY, DROP_KEY, CALL_KEY,
      // FIGHT_KEY, FIGHT_TO_DEATH_KEY,
      BROGUE_HELP_KEY, FEATS_KEY, DISCOVERIES_KEY, RETURN_KEY, EXPLORE_KEY,
      AUTOPLAY_KEY, SEED_KEY, EASY_MODE_KEY, ESCAPE_KEY, RETURN_KEY, DELETE_KEY,
      TAB_KEY, PERIOD_KEY, VIEW_RECORDING_KEY, NUMPAD_0, NUMPAD_1, NUMPAD_2,
      NUMPAD_3, NUMPAD_4, NUMPAD_5, NUMPAD_6, NUMPAD_7, NUMPAD_8, NUMPAD_9,
      UNKNOWN_KEY};
  const char descList[51][30] = {
      "up", "down", "left", "right", "up arrow", "left arrow", "down arrow",
      "right arrow", "upleft", "upright", "downleft", "downright", "descend",
      "ascend", "rest", "auto rest", "search", "inventory", "acknowledge",
      ...
      "numpad 6", "numpad 7", "numpad 8", "numpad 9", "unknown", "ERROR"};
...
void parseFile() {
  ...
  unsigned long oldFileLoc, oldRecLoc, oldLength, oldBufLoc, i, numTurns,
      numDepths, fileLength, startLoc;
  uint64_t seed;
  unsigned char c;
  ...
  if (selectFile("Parse recording: ", "Recording.broguerec", "")) {
    ...
    fprintf(descriptionFile,
            ...
            currentFilePath, versionString, (unsigned long long)seed, numTurns,
            numDepths, fileLength);
    for (i = 0; recordingLocation < fileLength; i++) {
      ...
      fprintf(descriptionFile, "\nEvent %li, loc %li, length %li:%s\t%s", i,
              startLoc, recordingLocation - startLoc, (i < 10 ? " " : ""),
              description);
    }
