...
void monsterName(char *buf, creature *monst, boolean includeArticle) {
    ...
    if (canSeeMonster(monst) || rogue.playbackOmniscience) {
        if (player.status[STATUS_HALLUCINATING] && !rogue.playbackOmniscience && !player.status[STATUS_TELEPATHIC]) {
            ...
            //assureCosmeticRNG;
            if (chineseUi) {
                sprintf(buf, "%s", tr(monsterCatalog[rand_range(1, NUMBER_MONSTER_KINDS - 1)].monsterName));
            } else {
        ...
        if (chineseUi) {
            sprintf(buf, "%s", tr(monst->info.monsterName));
        } else {
        ...
        strcpy(buf, chineseUi ? "某物" : "something");
        return;
    }
}
...
static void getMonsterAbilitiesText(const creature *monst, char *abilitiesText, boolean includeNegatable, boolean includeNonNegatable) {
    ...
    if (includeNegatable && (monst->mutationIndex >= 0) && mutationCatalog[monst->mutationIndex].canBeNegated) {
        strcat(buf, tr("has a rare mutation"));
    }
    ...
    if ((includeNegatable && monst->attackSpeed != monst->info.attackSpeed)
        ...
        if (monst->attackSpeed < 100) {
            strcat(buf, "&");
            strcat(buf, tr("attacks quickly"));
        } else if (monst->attackSpeed > 100) {
            strcat(buf, "&");
            strcat(buf, tr("attacks slowly"));
        }
    }
    ...
    if ((includeNegatable && monst->movementSpeed != monst->info.movementSpeed)
        ...
        if (monst->movementSpeed < 100) {
            strcat(buf, "&");
            strcat(buf, tr("moves quickly"));
        } else if (monst->movementSpeed > 100) {
            strcat(buf, "&");
            strcat(buf, tr("moves slowly"));
        }
    }
    ...
    if (includeNonNegatable) {
        if (monst->info.turnsBetweenRegen == 0) {
            strcat(buf, "&");
            strcat(buf, tr("does not regenerate"));
        } else if (monst->info.turnsBetweenRegen < 5000) {
            strcat(buf, "&");
            strcat(buf, tr("regenerates quickly"));
        }
    }
    ...
    for (int i = 0; monst->info.bolts[i] != BOLT_NONE; i++) {
        if (boltCatalog[monst->info.bolts[i]].abilityDescription[0]) {
            if ((includeNegatable && !(boltCatalog[monst->info.bolts[i]].flags & BF_NOT_NEGATABLE))
                ...
                strcat(buf, "&");
                strcat(buf, tr(boltCatalog[monst->info.bolts[i]].abilityDescription));
            }
        }
    }
    ...
    for (int i=0; i<32; i++) {
        if ((monst->info.abilityFlags & (Fl(i)))
            ...
            if ((includeNegatable && monsterAbilityCatalog[i].isNegatable)
                ...
                strcat(buf, "&");
                strcat(buf, tr(monsterAbilityCatalog[i].description));
            }
        }
    }
    ...
    for (int i=0; i<32; i++) {
        if ((monst->info.flags & (Fl(i)))
            ...
            if ((includeNegatable && monsterBehaviorCatalog[i].isNegatable)
                ...
                strcat(buf, "&");
                strcat(buf, tr(monsterBehaviorCatalog[i].description));
            }
        }
    }
    ...
    for (int i=0; i<32; i++) {
        if ((monst->bookkeepingFlags & (Fl(i)))
            ...
            if ((includeNegatable && (monst->bookkeepingFlags & MB_SEIZING))
                ...
                strcat(buf, "&");
                strcat(buf, tr(monsterBookkeepingFlagDescriptions[i]));
            }
        }
    }
                ...
                if ((boltCatalog[boltForItem(theItem)].flags & BF_FIERY) && (monst->status[STATUS_IMMUNE_TO_FIRE])
                    ...
                    if (staffDamageLow(enchant) >= monst->currentHP) {
                        sprintf(newText, tr("\n     Your %s (%c) will %s %s in one hit."),
                                theItemName,
                                theItem->inventoryLetter,
                                tr((monst->info.flags & MONST_INANIMATE) ? "destroy" : "kill"),
                                monstName);
                    } else {
...
void monsterDetails(char buf[], creature *monst) {
    ...
    if (!(monst->info.flags & MONST_RESTRICTED_TO_LIQUID)
        ...
        sprintf(newText, "     %s\n     ", tr(monsterText[monst->info.monsterID].flavorText));
        strcat(buf, newText);
    }
    ...
    if (!(monst->info.flags & MONST_ATTACKABLE_THRU_WALLS)
        ...
        sprintf(newText, tr("%s is trapped %s %s.\n     "),
                capMonstName,
    ...
    if (monst->creatureState == MONSTER_ALLY) {
        ...
        sprintf(newText, tr("%s is your ally.\n     "), capMonstName);
        strcat(buf, newText);
        if (monst->newPowerCount > 0) {
            ...
            if (monst->newPowerCount == 1) {
                strncpy(newText, tr("$HESHE seems ready to learn something new.\n     "), sizeof(newText) - 1);
                newText[sizeof(newText) - 1] = '\0';
            } else {
                sprintf(newText, tr("$HESHE seems ready to learn %i new talents.\n     "), monst->newPowerCount);
            }
    ...
    if ((monst->info.flags & MONST_RESTRICTED_TO_LIQUID) && !cellHasTMFlag(monst->loc, TM_ALLOWS_SUBMERGING)) {
        sprintf(newText, tr("     %s writhes helplessly on dry land.\n     "), capMonstName);
    } else if (rogue.armor
               && (rogue.armor->flags & ITEM_RUNIC)
               && (rogue.armor->flags & ITEM_RUNIC_IDENTIFIED)
        ...
        if (monst->info.abilityFlags & MA_POISONS) {
            ...
            if (i == 0) {
                ...
                sprintf(newText, tr("%s has a %i%% chance to poison you and typically poisons for %i turns.\n     "),
                        capMonstName,
                        combatMath2,
                        (int) ((monst->info.damage.lowerBound + monst->info.damage.upperBound) * monsterDamageAdjustmentAmount(monst) / 2 / FP_FACTOR));
            } else {
            sprintf(newText, tr("%s has a %i%% chance to poison you, typically poisons for %i turns, and at worst, could fatally poison you in %i hit%s.\n     "),
                    capMonstName,
                    combatMath2,
                    (int) ((monst->info.damage.lowerBound + monst->info.damage.upperBound) * monsterDamageAdjustmentAmount(monst) / 2 / FP_FACTOR),
                    i,
            ...
            sprintf(newText, tr("%s has a %i%% chance to hit you, typically hits for %i%% of your current health, and at worst, could defeat you in %i hit%s.\n     "),
                    capMonstName,
                    combatMath2,
    ...
    if (playerKnownMaxDamage == 0) {
        ...
        sprintf(newText, "%s", tr("You deal no direct damage."));
        ...
        sprintf(newText, tr("Your %s will slay %s in one stroke."), theItemName, monstName);
    } else if (monst->info.flags & (MONST_INVULNERABLE | MONST_IMMUNE_TO_WEAPONS)) {
        i = strlen(buf);
        i = encodeMessageColor(buf, i, &white);
        sprintf(newText, tr("%s is immune to your attacks."), monstName);
    } else if (monst->bookkeepingFlags & MB_CAPTIVE) {
        i = strlen(buf);
        i = encodeMessageColor(buf, i, &goodMessageColor);

        sprintf(newText, tr("%s is being held captive."), capMonstName);
    } else {
        i = strlen(buf);
        i = encodeMessageColor(buf, i, &goodMessageColor);
        ...
        sprintf(newText, tr("You have a %i%% chance to hit %s, typically hit for %i%% of $HISHER current health, and at best, could defeat $HIMHER in %i hit%s."),
                combatMath2,
                monstName,
    ...
    if (monsterIsNegatable(monst)) {
        if (pack.hasNegationCharm) {
            strcpy(buf2, tr("negation charm"));
        }
        if (pack.hasNegationScroll) {
            strcat(buf2, tr("&scroll of negation"));
        }
        if (pack.hasNegationWand && !(monst->info.abilityFlags & MA_REFLECT_100)) {
            strcat(buf2, tr("&wand of negation"));
        }
    }
    ...
    if (monst->info.flags & MONST_ATTACKABLE_THRU_WALLS) {
        ...
        if (pack.hasShatteringCharm) {
            strcpy(buf2, tr("shattering charm"));
        }
        if (pack.hasShatteringScroll) {
            strcat(buf2, tr("&scroll of shattering"));
        }
        if (pack.hasTunnelingStaff) {
            strcat(buf2, tr("&staff of tunneling"));
        }
    ...
    if (monst->carriedItem) {
        ...
        sprintf(newText, tr("%s has %s."), capMonstName, theItemName);
        upperCase(newText);
        strcat(buf, "\n     ");
        strcat(buf, newText);
    }
    ...
    if (monst->wasNegated && monst->newPowerCount == monst->totalPowerCount) {
        ...
        sprintf(newText, tr("%s is stripped of $HISHER special traits."), capMonstName);
        resolvePronounEscapes(newText, monst);
        upperCase(newText);
        strcat(buf, "\n     ");
        strcat(buf, newText);
    ...
    if (((monst->info.flags & MONST_DIES_IF_NEGATED) && negationMethodText[0]) || !negationMethodText[0]) {
        ...
        if (abilitiesText[0]) {
            sprintf(newText, tr("\n     %s%s has special traits that can be removed by a "), capMonstName, hasNonNegatableAbilities ? tr(" also") : "");
