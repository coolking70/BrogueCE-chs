...
void drawButton(brogueButton *button, enum buttonDrawStates highlight, screenDisplayBuffer *dbuf) {
    ...
    for (int i = 0, textLoc = 0; i < width && i + button->x < COLS;) {
        short cpSize = 1;
        short cpWidth = 1;
        unsigned int cp;
        ...
        cp = decodeUtf8Codepoint(&button->text[textLoc], &cpSize);
        ...
        enum displayGlyph displayCharacter = (enum displayGlyph) cp;
        if (cp == '*') {
        ...
        if (locIsInWindow((windowpos){ button->x + i, button->y }) && button->x + i + cpWidth <= COLS) {
...
void setButtonText(brogueButton *button, const char *textWithHotkey, const char *textWithoutHotkey) {
    ...
    const char *localizedWithHotkey = tr(textWithHotkey);
    const char *localizedWithoutHotkey = tr(textWithoutHotkey);
    ...
    if (KEYBOARD_LABELS) {
        snprintf(textBuf2, BUTTON_TEXT_SIZE - sizeof(textColorEscape) - 1, localizedWithHotkey, hotkeyColorEscape, textColorEscape);
    } else {
        strncpy(textBuf2, localizedWithoutHotkey, BUTTON_TEXT_SIZE - 1);
    }
