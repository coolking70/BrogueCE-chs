...
/* Combat rules:
 * Each combatant has an accuracy rating. This is the percentage of their
 * attacks that will ordinarily hit; higher numbers are better for them. Numbers
 * over 100 are permitted.
 *
 * Each combatant also has a defense rating. The "hit probability" is calculated
 * as given by this formula:
 *
 *          hit probability = (accuracy) * 0.987 ^ (defense)
 *
 * when hit determinations are made. Negative numbers and numbers over 100 are
 * permitted. The hit is then randomly determined according to this final
 * percentage.
 *
 * Some environmental factors can modify these numbers. An unaware, sleeping,
 * stuck or paralyzed combatant is always hit. An unaware, sleeping or paralyzed
 * combatant also takes triple damage.
 *
 * If the hit lands, damage is calculated in the range provided. However, the
 * clumping factor affects the probability distribution. If the range is 0-10
 * with a clumping factor of 1, it's a uniform distribution. With a clumping
 * factor of 2, it's calculated as 2d5 (with d5 meaing a die numbered from 0
 * through 5). With 3, it's 3d3, and so on. Note that a range not divisible by
 * the clumping factor is defective, as it will never be resolved in the top few
 * numbers of the range. In fact, the top (rangeWidth % clumpingFactor) will
 * never succeed. Thus we increment the maximum of the first (rangeWidth %
 * clumpingFactor) die by 1, so that in fact 0-10 with a CF of 3 would be 1d4 +
 * 2d3. Similarly, 0-10 with CF 4 would be 2d3 + 2d2. By playing with the
 * numbers, one can approximate a gaussian distribution of any mean and standard
 * deviation.
 *
 * Player combatants take their base defense value of their actual armor. Their
 * accuracy is a combination of weapon, armor and strength.
 *
 * Players have a base accuracy value of 100 throughout the game. Each point of
 * weapon enchantment (net of strength penalty/benefit) increases
 */
...
// does NOT account for auto-hit from sleeping or unaware defenders; does
// account for auto-hit from stuck or captive defenders and from weapons of
// slaying.
...
static void addMonsterToContiguousMonsterGrid(short x, short y, creature *monst,
...
static short alliedCloneCount(creature *monst) {
  ...
  for (creatureIterator it = iterateCreatures(monsters); hasNextCreature(it);) {
    ...
    if (temp != monst && temp->info.monsterID == monst->info.monsterID &&
...
// The contiguous group is supplemented with the given (x, y) coordinates, if
// any; this is so that jellies et al. can spawn behind the player in a hallway.
void splitMonster(creature *monst, creature *attacker) {
  ...
  if ((monst->info.abilityFlags & MA_CLONE_SELF_ON_DEFEND) &&
      alliedCloneCount(monst) < 100 && monst->currentHP > 0 &&
  ...
  addMonsterToContiguousMonsterGrid(monst->loc.x, monst->loc.y, monst,
                                    monsterGrid);
    ...
    for (int j = 0; j < DROWS; j++) {
      if (monsterGrid[i][j]) {
        for (int dir = 0; dir < 4; dir++) {
          ...
          if (coordinatesAreInMap(newX, newY) && !eligibleGrid[newX][newY] &&
  ...
  //        temporaryMessage("Jelly spawn possibilities (green = eligible, blue
  //        = monster):", REQUIRE_ACKNOWLEDGMENT); displayLevel();
  //    }
    ...
    for (int i = 0; i < DCOLS; i++) {
      for (int j = 0; j < DROWS; j++) {
        if (eligibleGrid[i][j] && !--randIndex) {
          ...
          // Mutation effects can be inherited, however; they're not learned
          // abilities.
          if (monst->mutationIndex >= 0) {
            clone->info.flags &=
                (monsterCatalog[clone->info.monsterID].flags |
                 mutationCatalog[monst->mutationIndex].monsterFlags);
            clone->info.abilityFlags &=
                (monsterCatalog[clone->info.monsterID].abilityFlags |
          ...
          for (int b = 0; b < 20; b++) {
            clone->info.bolts[b] =
                monsterCatalog[clone->info.monsterID].bolts[b];
          }
...
// This function is called whenever one creature acts aggressively against
// another in a way that directly causes damage. This can be things like melee
// attacks, fire/lightning attacks or throwing a weapon.
void moralAttack(creature *attacker, creature *defender) {
  ...
  if (defender->currentHP > 0 && !(defender->bookkeepingFlags & MB_IS_DYING)) {
    ...
    if (defender->status[STATUS_PARALYZED]) {
      ...
      // Paralyzed creature gets a turn to react before the attacker moves
      // again.
    ...
    if ((defender->info.abilityFlags & MA_AVOID_CORRIDORS)) {
      defender->status[STATUS_ENRAGED] = defender->maxStatus[STATUS_ENRAGED] =
          4;
    }
    ...
    if ((attacker == &player || attacker->creatureState == MONSTER_ALLY) &&
        defender != &player && defender->creatureState != MONSTER_ALLY) {
...
/// @brief Determine if the action forfeits the paladin feat. In general, the
/// player fails the feat if they attempt to deal direct damage to a non-hunting
/// creature that they are aware of and the creature would be damaged by the
/// attack.
/// @param attacker
/// @param defender
void handlePaladinFeat(creature *defender) {
  if (rogue.featRecord[FEAT_PALADIN] &&
      ...
      !(defender->info.flags & (MONST_INANIMATE | MONST_TURRET |
                                MONST_IMMOBILE | MONST_INVULNERABLE)) &&
      !(player.bookkeepingFlags & MB_SEIZED) && defender != &player) {
    rogue.featRecord[FEAT_PALADIN] = false;
  }
}

static boolean playerImmuneToMonster(creature *monst) {
  if (monst != &player && rogue.armor && (rogue.armor->flags & ITEM_RUNIC) &&
  ...
  if (defender == &player) {
    ...
    if (attacker->info.abilityFlags & MA_HIT_HALLUCINATE) {
      ...
      player.status[STATUS_HALLUCINATING] +=
          gameConst->onHitHallucinateDuration;
      player.maxStatus[STATUS_HALLUCINATING] =
          max(player.maxStatus[STATUS_HALLUCINATING],
              player.status[STATUS_HALLUCINATING]);
    }
    ...
    if (attacker->info.abilityFlags & MA_HIT_STEAL_FLEE &&
        !(attacker->carriedItem) && (packItems->nextItem) &&
        attacker->currentHP > 0 &&
        !attacker->status[STATUS_CONFUSED] // No stealing from the player if you
                                           // bump him while confused.
      ...
      if (itemCandidates) {
        ...
        if (theItem) {
          if (theItem->category &
              WEAPON) { // Monkeys will steal half of a stack of weapons, and
                        // one of any other stack.
          ...
          theItem->flags &=
              ~ITEM_PLAYER_AVOIDS; // Explore will seek the item out if it ends
...
static boolean forceWeaponHit(creature *defender, item *theItem) {
  ...
  if (canDirectlySeeMonster(defender) &&
      !cellHasTerrainFlag(newLoc,
                          T_OBSTRUCTS_PASSABILITY | T_OBSTRUCTS_VISION) &&
      !(pmapAt(newLoc)->flags & (HAS_MONSTER | HAS_PLAYER))) {
    sprintf(buf, "you launch %s backward with the force of your blow",
            monstName);
    buf[DCOLS] = '\0';
  ...
  if (!(defender->bookkeepingFlags & MB_IS_DYING) &&
      ...
      distanceBetween(oldLoc, defender->loc) <
          weaponForceDistance(netEnchant(theItem))) {

    if (pmap[defender->loc.x + newLoc.x - oldLoc.x]
            [defender->loc.y + newLoc.y - oldLoc.y]
                .flags &
        (HAS_MONSTER | HAS_PLAYER)) {
      otherMonster = monsterAtLoc((pos){defender->loc.x + newLoc.x - oldLoc.x,
                                        defender->loc.y + newLoc.y - oldLoc.y});
      monsterName(buf2, otherMonster, true);
    } else {
      otherMonster = NULL;
      strcpy(buf2,
             tileCatalog[pmap[defender->loc.x + newLoc.x -
                              oldLoc.x][defender->loc.y + newLoc.y - oldLoc.y]
                             .layers[highestPriorityLayer(
                                 defender->loc.x + newLoc.x - oldLoc.x,
    ...
    if (!(defender->info.flags &
      ...
      if (canDirectlySeeMonster(defender)) {
        ...
        sprintf(buf, "%s %s on impact with %s", monstName,
                (defender->info.flags & MONST_INANIMATE) ? "is destroyed"
                                                         : "dies",
                buf2);
        buf[DCOLS] = '\0';
        combatMessage(buf, messageColorFromVictim(defender));
    ...
    if (otherMonster && !(otherMonster->info.flags &
                          (MONST_IMMUNE_TO_WEAPONS | MONST_INVULNERABLE))) {

      if (inflictDamage(NULL, otherMonster, forceDamage, &white, false)) {
        if (canDirectlySeeMonster(otherMonster)) {
          sprintf(buf, "%s %s%s when %s slams into $HIMHER", buf2,
                  (knowFirstMonsterDied ? "also " : ""),
                  (otherMonster->info.flags & MONST_INANIMATE) ? "is destroyed"
                                                               : "dies",
                  monstName);
      ...
      if (otherMonster->creatureState != MONSTER_ALLY) {
        // Allies won't defect if you throw another monster at them, even though
        // it hurts.
...
void magicWeaponHit(creature *defender, item *theItem, boolean backstabbed) {
  ...
  const color *effectColors[NUMBER_WEAPON_RUNIC_KINDS] = {
      &white, &black, &yellow,  &pink,   &green, &confusionGasColor,
      NULL,   NULL,   &darkRed, &rainbow};
  //  W_SPEED, W_QUIETUS, W_PARALYSIS, W_MULTIPLICITY, W_SLOWING, W_CONFUSION,
  //  W_FORCE, W_SLAYING, W_MERCY, W_PLENTY
  short chance, i;
  fixpt enchant;
  ...
  // If the defender is already dead, proceed only if the runic is speed or
  // multiplicity. (Everything else acts on the victim, which would literally be
  // overkill.)
  if ((defender->bookkeepingFlags & MB_IS_DYING) &&
      theItem->enchant2 != W_SPEED && theItem->enchant2 != W_MULTIPLICITY) {
    return;
  }
  ...
  if (chance > 0 && rand_percent(chance)) {
    ...
    case W_SPEED:
      if (player.ticksUntilTurn != -1) {
        sprintf(buf, "your %s trembles and time freezes for a moment",
                theItemName);
        buf[DCOLS] = '\0';
        combatMessage(buf, 0);
    ...
    case W_QUIETUS:
      ...
      sprintf(buf, "%s suddenly %s", monstName,
      ...
      break;
    case W_PARALYSIS:
      defender->status[STATUS_PARALYZED] = max(
          defender->status[STATUS_PARALYZED], weaponParalysisDuration(enchant));
      defender->maxStatus[STATUS_PARALYZED] =
    ...
    case W_MULTIPLICITY:
      sprintf(buf,
              "Your %s emits a flash of light, and %sspectral duplicate%s "
              "appear%s!",
              theItemName, (weaponImageCount(enchant) == 1 ? "a " : ""),
      ...
      for (i = 0; i < (weaponImageCount(enchant)); i++) {
        ...
        newMonst->loc = getQualifyingPathLocNear(
            defender->loc, true,
            T_DIVIDES_LEVEL & avoidedFlagsForMonster(&(newMonst->info)),
            HAS_PLAYER, avoidedFlagsForMonster(&(newMonst->info)),
            (HAS_PLAYER | HAS_MONSTER | HAS_STAIRS), false);
        newMonst->bookkeepingFlags |=
            (MB_FOLLOWER | MB_BOUND_TO_LEADER | MB_DOES_NOT_TRACK_LEADER |
        ...
        newMonst->info.accuracy =
            player.info.accuracy + (5 * netEnchant(theItem) / FP_FACTOR);
        newMonst->info.damage = player.info.damage;
        newMonst->status[STATUS_LIFESPAN_REMAINING] =
            newMonst->maxStatus[STATUS_LIFESPAN_REMAINING] =
    ...
    case W_SLOWING:
      ...
      break;
    case W_CONFUSION:
      defender->status[STATUS_CONFUSED] = max(defender->status[STATUS_CONFUSED],
...
static void attackVerb(char returnString[DCOLS], creature *attacker,
                       short hitPercentile) {
  short verbCount, increment;

  if (attacker != &player &&
      (player.status[STATUS_HALLUCINATING] || !canSeeMonster(attacker))) {
  ...
  for (verbCount = 0;
       verbCount < 4 &&
       monsterText[attacker->info.monsterID].attack[verbCount + 1][0] != '\0';
       verbCount++)
    ;
  ...
  strcpy(
      returnString,
      monsterText[attacker->info.monsterID].attack[hitPercentile / increment]);
...
void applyArmorRunicEffect(char returnString[DCOLS], creature *attacker,
  ...
  case A_MULTIPLICITY:
    if (melee &&
        !(attacker->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE)) &&
        rand_percent(33)) {
      for (i = 0; i < armorImageCount(enchant); i++) {
        ...
        monst->info.abilityFlags &=
            ~(MA_CAST_SUMMON |
              MA_DF_ON_DEATH); // No summoning by spectral images. Gotta draw
                               // the line! Also no exploding or infecting by
                               // spectral clones.
        monst->leader = &player;
        monst->creatureState = MONSTER_ALLY;
        monst->status[STATUS_DISCORDANT] =
            0; // Otherwise things can get out of control...
        ...
        if (monst->carriedMonster) {
          ...
          killCreature(carried, true); // Otherwise you can get infinite
                                       // phoenices from a discordant phoenix.
        }
        ...
        // Temporary guest!
        monst->status[STATUS_LIFESPAN_REMAINING] =
            monst->maxStatus[STATUS_LIFESPAN_REMAINING] = 3;
        monst->currentHP = monst->info.maxHP = 1;
        monst->info.defense = 0;

        if (strLenWithoutEscapes(attacker->info.monsterName) <= 6) {
          sprintf(monst->info.monsterName, "spectral %s",
                  attacker->info.monsterName);
        } else {
      ...
      sprintf(returnString,
              "Your %s flashes, and spectral images of %s appear!", armorName,
              attackerName);
    }
    break;
  case A_MUTUALITY:
      ...
      if (count) {
        for (i = 0; i < 8; i++) {
          if (hitList[i] && !(hitList[i]->bookkeepingFlags & MB_IS_DYING)) {
            ...
            if (inflictDamage(&player, hitList[i],
                              (*damage + count) / (count + 1), &blue, true)) {
              if (canSeeMonster(hitList[i])) {
                sprintf(buf, "%s %s", monstName,
                        ((hitList[i]->info.flags & MONST_INANIMATE)
                             ? "is destroyed"
                             : "dies"));
        ...
        if (!runicKnown) {
          sprintf(returnString,
                  "Your %s pulses, and the damage is shared with %s!",
                  armorName,
    ...
    *damage -= rand_range(1, armorAbsorptionMax(enchant));
    if (*damage <= 0) {
      *damage = 0;
      runicDiscovered = true;
      if (!runicKnown) {
        sprintf(returnString, "your %s pulses and absorbs the blow!",
                armorName);
      }
    }
    break;
  case A_REPRISAL:
    if (melee &&
        !(attacker->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE))) {
      newDamage = max(1, armorReprisalPercent(enchant) * (*damage) /
                             100); // 5% reprisal per armor level
      if (inflictDamage(&player, attacker, newDamage, &blue, true)) {
        if (canSeeMonster(attacker)) {
          sprintf(returnString, "your %s pulses and %s drops dead!", armorName,
                  attackerName);
          runicDiscovered = true;
        }
        ...
        if (canSeeMonster(attacker)) {
          sprintf(returnString, "your %s pulses and %s shudders in pain!",
                  armorName, attackerName);
          runicDiscovered = true;
        }
      }
    }
  ...
  case A_VULNERABILITY:
    *damage *= 2;
    if (!runicKnown) {
      sprintf(returnString, "your %s pulses and you are wracked with pain!",
              armorName);
      runicDiscovered = true;
    }
    break;
  case A_IMMOLATION:
    if (rand_percent(10)) {
      sprintf(returnString, "flames suddenly explode out of your %s!",
              armorName);
      message(returnString, runicKnown ? 0 : REQUIRE_ACKNOWLEDGMENT);
      returnString[0] = '\0';
      spawnDungeonFeature(player.loc.x, player.loc.y,
                          &(dungeonFeatureCatalog[DF_ARMOR_IMMOLATION]), true,
                          false);
      runicDiscovered = true;
    }
  default:
...
static void decrementWeaponAutoIDTimer() {
  ...
  if (rogue.weapon && !(rogue.weapon->flags & ITEM_IDENTIFIED) &&
    ...
    sprintf(buf, "%s %s.", (rogue.weapon->quantity > 1 ? "they are" : "it is"),
            buf2);
    messageWithColor(buf, &itemMessageColor, 0);
  }
}
...
void processStaggerHit(creature *attacker, creature *defender) {
  if ((defender->info.flags &
       (MONST_INVULNERABLE | MONST_IMMOBILE | MONST_INANIMATE)) ||
      (defender->bookkeepingFlags & MB_CAPTIVE) ||
  ...
  short newX =
      clamp(defender->loc.x - attacker->loc.x, -1, 1) + defender->loc.x;
  short newY =
      clamp(defender->loc.y - attacker->loc.y, -1, 1) + defender->loc.y;
  if (coordinatesAreInMap(newX, newY) &&
...
boolean attack(creature *attacker, creature *defender, boolean lungeAttack) {
  ...
  char buf[COLS * 2], buf2[COLS * 2], attackerName[COLS], defenderName[COLS],
      verb[DCOLS], explicationClause[DCOLS] = "", armorRunicString[DCOLS * 3];
  char chineseExplication[DCOLS] = "";
  boolean sneakAttack, defenderWasAsleep, defenderWasParalyzed,
      degradesAttackerWeapon, sightUnseen;
  boolean chineseUi = strcmp(tr("Health"), "Health") != 0;
  ...
  if (attacker == &player && rogue.weapon && rogue.featRecord[FEAT_PURE_MAGE] &&
  ...
  degradesAttackerWeapon =
      (defender->info.flags & MONST_DEFEND_DEGRADE_WEAPON ? true : false);

  sightUnseen = !canSeeMonster(attacker) && !canSeeMonster(defender);

  if (defender->status[STATUS_LEVITATING] &&
      (attacker->info.flags & MONST_RESTRICTED_TO_LIQUID)) {
    return false; // aquatic or other liquid-bound monsters cannot attack flying
                  // opponents
  }
  ...
  if (defender->info.flags & MONST_INANIMATE) {
    ...
    sneakAttack = (defender != &player && attacker == &player &&
                           (defender->creatureState == MONSTER_WANDERING)
                       ? true
                       : false);
    defenderWasAsleep =
        (defender != &player && (defender->creatureState == MONSTER_SLEEPING)
             ? true
             : false);
  ...
  if ((attacker->info.abilityFlags & MA_SEIZES) &&
      ...
      (distanceBetween(attacker->loc, defender->loc) == 1 &&
       !diagonalBlocked(attacker->loc.x, attacker->loc.y, defender->loc.x,
                        defender->loc.y, false))) {

    attacker->bookkeepingFlags |= MB_SEIZING;
    defender->bookkeepingFlags |= MB_SEIZED;

    // if the player is seized by a submerged monster they can see (i.e. player
    // is also submerged), it immediately surfaces so it can be targeted with
    // staffs/wands
    if (defender == &player && (attacker->bookkeepingFlags & MB_SUBMERGED) &&
        canSeeMonster(attacker)) {
      attacker->bookkeepingFlags &= ~MB_SUBMERGED;
      monsterName(attackerName, attacker, true);
    }

    if (canSeeMonster(attacker) || canSeeMonster(defender)) {
      sprintf(buf, "%s seizes %s!", attackerName,
              (defender == &player ? "your legs" : defenderName));
  ...
  if (sneakAttack || defenderWasAsleep || defenderWasParalyzed || lungeAttack ||
    ...
    damage =
        (defender->info.flags & (MONST_IMMUNE_TO_WEAPONS | MONST_INVULNERABLE)
             ? 0
             : randClump(attacker->info.damage) *
                   monsterDamageAdjustmentAmount(attacker) / FP_FACTOR);

    if (sneakAttack || defenderWasAsleep || defenderWasParalyzed) {
      if (defender != &player) {
        // The non-player defender doesn't hit back this turn because it's still
        // flat-footed.
        defender->ticksUntilTurn +=
            max(defender->movementSpeed, defender->attackSpeed);
    ...
    if (attacker == &player && rogue.reaping &&
        !(defender->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE))) {

      specialDamage = min(damage, defender->currentHP) *
                      rogue.reaping; // Maximum reaped damage can't exceed the
    ...
    if (damage == 0) {
      sprintf(explicationClause, " but %s no damage",
              (attacker == &player ? "do" : "does"));
      if (attacker == &player) {
        rogue.disturbed = true;
      }
      strcpy(chineseExplication, "（但未造成伤害）");
    } else if (lungeAttack) {
      strcpy(explicationClause, " with a vicious lunge attack");
      strcpy(chineseExplication, "（猛烈突刺）");
    } else if (defenderWasParalyzed) {
      sprintf(explicationClause, " while $HESHE %s paralyzed",
              (defender == &player ? "are" : "is"));
      strcpy(chineseExplication, "（趁其麻痹）");
    } else if (defenderWasAsleep) {
      strcpy(explicationClause, " in $HISHER sleep");
      strcpy(chineseExplication, "（趁其熟睡）");
    } else if (sneakAttack) {
      strcpy(explicationClause, ", catching $HIMHER unaware");
      strcpy(chineseExplication, "（偷袭）");
    } else if (defender->status[STATUS_STUCK] ||
               defender->bookkeepingFlags & MB_CAPTIVE) {
      sprintf(explicationClause, " while %s dangle%s helplessly",
              (canSeeMonster(defender) ? "$HESHE" : "it"),
              (defender == &player ? "" : "s"));
      strcpy(chineseExplication, "（趁其束手无策）");
    }
    ...
    if (inflictDamage(attacker, defender, damage, &red,
                      false)) { // if the attack killed the defender
      if (chineseUi) {
        const char *killVerb;
        if (defender->info.flags & MONST_INANIMATE) {
          killVerb = "摧毁了";
        } else if (defenderWasAsleep || sneakAttack || defenderWasParalyzed ||
                   lungeAttack) {
          killVerb = "消灭了";
        } else {
          killVerb = "击败了";
        }
        sprintf(buf, "%s%s%s%s", attackerName, killVerb, defenderName,
                chineseExplication);
      } else if (defenderWasAsleep || sneakAttack || defenderWasParalyzed ||
                 lungeAttack) {
        sprintf(buf, "%s %s %s%s", attackerName,
                ((defender->info.flags & MONST_INANIMATE) ? "destroyed"
                                                          : "dispatched"),
      ...
      if (sightUnseen) {
        ...
        combatMessage(buf,
                      (damage > 0 ? messageColorFromVictim(defender) : &white));
      }
      killCreature(defender, false);
      if (&player == defender) {
      ...
      if (!rogue.blockCombatText &&
          (canSeeMonster(attacker) || canSeeMonster(defender))) {
        attackVerb(
            verb, attacker,
            max(damage - (attacker->info.damage.lowerBound *
                          monsterDamageAdjustmentAmount(attacker) / FP_FACTOR),
                0) *
                100 /
                max(1, (attacker->info.damage.upperBound -
                        attacker->info.damage.lowerBound) *
                           monsterDamageAdjustmentAmount(attacker) /
                           FP_FACTOR));
        if (chineseUi) {
          sprintf(buf, "%s%s%s%s", attackerName, tr(verb), defenderName,
                  chineseExplication);
        } else {
          sprintf(buf, "%s %s %s%s", attackerName, verb, defenderName,
                  explicationClause);
        }
        if (sightUnseen) {
      ...
      if (attacker == &player && rogue.weapon &&
          (rogue.weapon->flags & ITEM_ATTACKS_STAGGER)) {
        processStaggerHit(attacker, defender);
      }
      if (attacker->info.abilityFlags & SPECIAL_HIT) {
        specialHit(attacker, defender,
                   (attacker->info.abilityFlags & MA_POISONS) ? poisonDamage
                                                              : damage);
      }
      if (armorRunicString[0]) {
        message(armorRunicString, 0);
        if (rogue.armor && (rogue.armor->flags & ITEM_RUNIC) &&
            rogue.armor->enchant2 == A_BURDEN) {
    ...
    if (attacker == &player && rogue.weapon &&
      ...
      magicWeaponHit(defender, rogue.weapon,
                     sneakAttack || defenderWasAsleep || defenderWasParalyzed);
    }
    ...
    if (degradesAttackerWeapon && attacker == &player && rogue.weapon &&
        ...
        // Can't damage a Weapon of Acid Mound Slaying by attacking an acid
        // mound... just ain't right!
        && !((rogue.weapon->flags & ITEM_RUNIC) &&
             rogue.weapon->enchant2 == W_SLAYING &&
             monsterIsInClass(defender, rogue.weapon->vorpalEnemy)) &&
    ...
    if (!rogue.blockCombatText) {
      if (sightUnseen) {
        ...
        sprintf(buf, tr("%s missed %s"), attackerName, defenderName);
        combatMessage(buf, 0);
      }
    }
    return false;
...
// Gets the length of a string without the four-character color escape
// sequences, since those aren't displayed.
...
static unsigned int decodeUtf8Codepoint(const char *s, short *size) {
  short n = utf8CodepointSize(s);
  unsigned int cp;
  ...
  case 2:
    ...
    return cp >= 0x80 ? cp : (unsigned char)s[0];
  case 3:
    cp = ((unsigned int)(s[0] & 0x0F) << 12) |
         ((unsigned int)(s[1] & 0x3F) << 6) | ((unsigned int)(s[2] & 0x3F));
    return cp >= 0x800 ? cp : (unsigned char)s[0];
  case 4:
    cp = ((unsigned int)(s[0] & 0x07) << 18) |
         ((unsigned int)(s[1] & 0x3F) << 12) |
         ((unsigned int)(s[2] & 0x3F) << 6) | ((unsigned int)(s[3] & 0x3F));
    return (cp >= 0x10000 && cp <= 0x10FFFF) ? cp : (unsigned char)s[0];
  default:
    return (unsigned char)s[0];
  }
}

static short unicodeDisplayWidth(unsigned int cp) {
  if (cp == 0)
    return 0;
  if (cp < 0x20 || (cp >= 0x7F && cp < 0xA0))
    return 0;

  // East Asian Wide / Fullwidth + common emoji ranges.
...
short strLenWithoutEscapes(const char *str) {
  ...
  for (int i = 0; str[i];) {
    ...
    cp = decodeUtf8Codepoint(&str[i], &size);
    count += unicodeDisplayWidth(cp);
    i += size;
  }
  return count;
}
...
void displayCombatText() {
  ...
  for (end = start; *end != '\0'; end++) {
    if (*end == '\n') {
      *end = '\0';
      message(start,
              FOLDABLE | (rogue.cautiousMode ? REQUIRE_ACKNOWLEDGMENT : 0));
      start = end + 1;
    }
  }
...
// flashColor indicates the color that the damage will cause the creature to
// flash
boolean inflictDamage(creature *attacker, creature *defender, short damage,
  ...
  if (!ignoresProtectionShield && defender->status[STATUS_SHIELDED]) {
    ...
    if (defender->status[STATUS_SHIELDED] > damage * 10) {
      ...
      defender->status[STATUS_SHIELDED] = defender->maxStatus[STATUS_SHIELDED] =
          0;
    }
  }

  defender->bookkeepingFlags &=
      ~MB_ABSORBING; // Stop eating a corpse if you are getting hurt.
  ...
  if (defender == &player && rogue.mode == GAME_MODE_EASY && damage > 0) {
    damage = max(1, damage / 5);
  }

  if (((attacker == &player && rogue.transference) ||
       (attacker && attacker != &player &&
        (attacker->info.abilityFlags & MA_TRANSFERENCE))) &&
      !(defender->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE))) {

    transferenceAmount = min(
        damage, defender->currentHP); // Maximum transferred damage can't exceed
                                      // the victim's remaining health.

    if (attacker == &player) {
      transferenceAmount = transferenceAmount * rogue.transference /
                           gameConst->playerTransferenceRatio;
      if (transferenceAmount == 0) {
        transferenceAmount = ((rogue.transference > 0) ? 1 : -1);
      }
    } else if (attacker->creatureState == MONSTER_ALLY) {
      transferenceAmount =
          transferenceAmount * 4 / 10; // allies get 40% recovery rate
    } else {
      transferenceAmount = transferenceAmount * 9 /
                           10; // enemies get 90% recovery rate, deal with it
    }
  ...
  if (defender->currentHP <= damage) { // killed
    ...
    if (flashColor && damage > 0) {
      flashMonster(defender, flashColor,
                   MIN_FLASH_STRENGTH + (100 - MIN_FLASH_STRENGTH) * damage /
                                            defender->info.maxHP);
    }
  }
...
void addPoison(creature *monst, short durationIncrement,
...
// Marks the decedent as dying, but does not remove it from the monster chain to
// avoid iterator invalidation; that is done in `removeDeadMonsters`. Use
// "administrativeDeath" if the monster is being deleted for administrative
// purposes, as opposed to dying as a result of physical actions.
// AdministrativeDeath means the monster simply disappears, with no messages,
// dropped item, DFs or other effect.
void killCreature(creature *decedent, boolean administrativeDeath) {
  ...
  if (!administrativeDeath && (decedent->info.abilityFlags & MA_DF_ON_DEATH) &&
    ...
    spawnDungeonFeature(decedent->loc.x, decedent->loc.y,
                        &dungeonFeatureCatalog[decedent->info.DFType], true,
                        false);

    if (monsterText[decedent->info.monsterID].DFMessage[0] &&
        canSeeMonster(decedent)) {
      monsterName(monstName, decedent, true);
      {
        boolean chineseUi = strcmp(tr("Health"), "Health") != 0;
        if (chineseUi) {
          const char *translatedDF =
              tr(monsterText[decedent->info.monsterID].DFMessage);
          snprintf(buf, DCOLS * 3, "%s%s", monstName, translatedDF);
        } else {
          snprintf(buf, DCOLS * 3, "%s %s", monstName,
                   monsterText[decedent->info.monsterID].DFMessage);
        }
      }
  ...
  if (decedent == &player) { // the player died
    ...
    if (!administrativeDeath && decedent->creatureState == MONSTER_ALLY &&
        !canSeeMonster(decedent) &&
        (!(decedent->info.flags & MONST_INANIMATE) ||
         (monsterCatalog[decedent->info.monsterID].abilityFlags &
          MA_ENTER_SUMMONS)) &&
    ...
    // This must be done at the same time as removing the HAS_MONSTER flag, or
    // game state might end up inconsistent.
...
void buildHitList(const creature **hitList, const creature *attacker,
