...
typedef struct ScreenTile {
    ...
    unsigned int unicodeCodepoint; // optional Unicode codepoint for UI text rendering
...
#ifdef BROGUE_UTF8_TEXT
static TTF_Font *UnicodeFont = NULL;

static short utf8EncodeCodepoint(unsigned int cp, char out[5]) {
    if (cp <= 0x7F) {
        out[0] = (char) cp;
        out[1] = '\0';
        return 1;
...
static void initUnicodeFont() {
    ...
    if (TTF_Init() < 0) {
        fprintf(stderr, "Warning: SDL_ttf init failed: %s\n", TTF_GetError());
        return;
    }

    for (i = 0; i < sizeof(candidates) / sizeof(candidates[0]); i++) {
        sprintf(filename, "%s/assets/%s", dataDirectory, candidates[i]);
        UnicodeFont = TTF_OpenFont(filename, 32);
        if (UnicodeFont) {
            break;
        }
    }
    ...
    if (!UnicodeFont) {
        fprintf(stderr, "Warning: UTF-8 font not loaded. Chinese text may show '?'.\n");
        return;
    }
}

static void renderUnicodeGlyph(SDL_Renderer *renderer, unsigned int cp, SDL_Rect dest, short r, short g, short b) {
    if (!UnicodeFont || cp == 0) return;

    char utf8[5];
    SDL_Color fg = { (Uint8) r, (Uint8) g, (Uint8) b, 255 };
    utf8EncodeCodepoint(cp, utf8);

    SDL_Surface *surface = TTF_RenderUTF8_Blended(UnicodeFont, utf8, fg);
    if (!surface) return;

    SDL_Texture *texture = SDL_CreateTextureFromSurface(renderer, surface);
    if (texture) {
        // 等比缩放：保持字形原始宽高比（汉字等双宽字符为正方形），
        // 在 dest 矩形内按最小缩放比例适配，并水平/垂直居中，
        // 避免被拉伸为"扁"形（宽屏时 2×tileWidth > tileHeight 的情况）。
        int gw = surface->w, gh = surface->h;
        if (gw > 0 && gh > 0 && dest.w > 0 && dest.h > 0) {
            float scaleX = (float)dest.w / gw;
            float scaleY = (float)dest.h / gh;
            float scale  = scaleX < scaleY ? scaleX : scaleY;
            int rw = (int)(gw * scale);
            int rh = (int)(gh * scale);
            SDL_Rect fitDest = {
                .x = dest.x + (dest.w - rw) / 2,
                .y = dest.y + (dest.h - rh) / 2,
                .w = rw,
                .h = rh
            };
            SDL_RenderCopy(renderer, texture, NULL, &fitDest);
        }
...
void updateTileUnicode(int row, int column, unsigned int unicodeCodepoint,
    short foreRed, short foreGreen, short foreBlue,
    short backRed, short backGreen, short backBlue)
{
    screenTiles[row][column] = (ScreenTile){
    ...
    if (UnicodeFont) {
        for (int x = 0; x < COLS; x++) {
            ...
            for (int y = 0; y < ROWS; y++) {
                ...
                if (tileHeight == 0) continue;
                ScreenTile *tile = &screenTiles[y][x];
                if (!tile->unicodeCodepoint) continue;
                if (softwareRendering && !tile->needsRefresh) continue;

                // 判断是否为双宽字符（CJK汉字等），若是则渲染宽度扩展为2格，
                // 避免字符只占半格导致右侧漏出背景地图的间隙问题。
                unsigned int cp = tile->unicodeCodepoint;
                int glyphCols = (
                       (cp >= 0x1100 && cp <= 0x115F)   // Hangul Jamo
                    || (cp >= 0x2329 && cp <= 0x232A)   // CJK brackets
                    || (cp >= 0x2E80 && cp <= 0xA4CF)   // CJK Unified (含常用汉字)
                ...
                SDL_Rect dest = {
                    .x = x * outputWidth / COLS,
                    .y = y * outputHeight / ROWS,
                    .w = ((x + glyphCols) * outputWidth / COLS) - (x * outputWidth / COLS),
                    .h = tileHeight
                };
                renderUnicodeGlyph(renderer, cp, dest,
                    (short) round(2.55 * tile->foreRed),
                    (short) round(2.55 * tile->foreGreen),
                    (short) round(2.55 * tile->foreBlue));
            }
        }
    }
#endif
