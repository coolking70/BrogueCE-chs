...
int main() {
    ...
    char src[200];
    sprintf(src, "(Your %c%c%c%c柳木法杖 must be %c%c%c%cfirebolt 法杖.)", 25, 50, 50, 50, 25, 50, 50, 50);
    
    boolean res1 = templateMatch("(Your %s must be ", src, capPtrs, &capCount, 32);
    printf("Result 1 (Your %%s must be ): %d\n", res1);

    capCount = 0;
    boolean res2 = templateMatch("(Your %s must be %s.)", src, capPtrs, &capCount, 32);
    printf("Result 2 (Your %%s must be %%s.): %d\n", res2);
