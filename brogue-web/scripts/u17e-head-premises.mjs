...
const cwd=process.cwd(),out=path.join(cwd,'ai_docs/reports/u-17e-evidence'),label=process.argv[2]??'discovery';
const base=fs.mkdtempSync(path.join(os.tmpdir(),'u17e-head-')),temp=path.join(base,'brogue-web');fs.mkdirSync(temp);
try{
