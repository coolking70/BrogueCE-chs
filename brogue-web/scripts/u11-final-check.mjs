...
const files=[...walk('src'),...walk('public'),...walk('scripts'),...walk('../BrogueCE-master/src'),'progress.md',`${dir}/browser.json`,`${dir}/closure.json`,'ai_docs/reports/w-22.report.md','ai_docs/reports/u-07.report.md','ai_docs/reports/u-08.report.md','ai_docs/reports/u-09.report.md','ai_docs/reports/u-10.report.md','ai_docs/tasks/u-11.prompt.md','ai_docs/reports/x-0-survey.report.md',...fs.readdirSync('.').filter(f=>/^(package.*json|tsconfig.*json|vite.config.ts|index.html)$/.test(f))].sort();
...
const gates=[
 ['ce','node',['scripts/u11-ce-audit.mjs']],
 ['boundary','node',['scripts/u11-audit.mjs']],
 ['build','npm',['run','build']],
...
const after=snapshot(),changed=files.filter(f=>before[f]!==after[f]);fs.writeFileSync(`${dir}/final-input-after.json`,JSON.stringify(after,null,2)+'\n');fs.writeFileSync(`${dir}/final-check.json`,JSON.stringify({startedAt,finishedAt:new Date().toISOString(),outcomes,changed,inputCount:files.length,inputManifestSha256:hash(`${dir}/final-input-after.json`),final:changed.length===0&&outcomes.length===gates.length&&outcomes.every(r=>r.status===0)},null,2)+'\n');
