// U02a: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const patterns = {
    ...
    expanded: /randomNumbersGenerated|rngStates|currentRNG|loadSnapshot|loadReplay|replaySeek|flavor|Flavor/,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-02a-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
