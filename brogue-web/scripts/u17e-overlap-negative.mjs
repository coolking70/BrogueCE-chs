...
const cwd=process.cwd(),out=path.join(cwd,'ai_docs/reports/u-17e-evidence'),base=fs.mkdtempSync(path.join(os.tmpdir(),'u17e-overlap-negative-'));
...
const behavior='two overlapping drops';
const cases=[{name:'two-items-one-floor-slot',file:'src/engine/Core/Game.ts',edit:s=>change(s,'const floorItems = [...slots.values()];','const floorItems = this.items;')}];const results=[];
try{
 const walk=d=>fs.readdirSync(d,{withFileTypes:true}).flatMap(e=>e.isDirectory()?walk(`${d}/${e.name}`):[`${d}/${e.name}`]);
 fs.writeFileSync(`${out}/overlap-negative-inputs.json`,JSON.stringify(Object.fromEntries(walk('src').map(p=>[p,crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex')])),null,2)+'\n');
...
}finally{fs.writeFileSync(`${out}/overlap-negative-summary.json`,JSON.stringify(results,null,2)+'\n');fs.rmSync(base,{recursive:true,force:true});}
