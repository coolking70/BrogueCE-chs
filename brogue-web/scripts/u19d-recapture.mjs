...
const out='ai_docs/reports/u-19d-evidence',file='src/test/fixtures/generation_baseline.json';
if(fs.existsSync(`${out}/recapture.json`))throw Error('Baseline already recaptured; refusing second write');
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'u19d-final-capture-'));
const sha=p=>crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex');
try{
 await build({entryPoints:['scripts/u19d-observe.ts'],outfile:`${tmp}/capture.mjs`,bundle:true,platform:'node',format:'esm'});
 ...
 base.note='U19d: restore CE18 secret-lever eligibility and CE28 caged adoption; complete action chains and CE selection contracts verified, individually attributed, one recapture; see u-19d.report.md.';
