...
const production=['src/data/arcana.json','src/engine/Combat/Bolt.ts','src/engine/Items/ArcanaRecharge.ts','src/engine/Items/ItemLoader.ts','src/locales/zh_CN.json'];
const allowedTests=['src/engine/Combat/BoltCatalog.test.ts','src/test/w_5_arcana_instance.test.ts','src/test/w_6_arcana_recharge.test.ts','src/test/w_9_directed_status.test.ts','src/test/w_14_obstruction.test.ts','src/test/w_15_shielding.test.ts','src/test/w_24_wand_catalog.test.ts','src/test/w_25_staff_catalog.test.ts','src/test/w_26_arcana_closure.test.ts'];
...
const oldStaffsUnchanged=JSON.stringify(dataNew.staffs.filter(s=>!added.includes(s.id)))===JSON.stringify(dataOld.staffs);
const completeCatalog=dataNew.staffs.length===13&&dataNew.staffs.filter(s=>!s.excludeFromGeneration).length===12&&dataNew.staffs.filter(s=>!s.excludeFromGeneration).reduce((n,s)=>n+s.frequency,0)===110;
const printer=ts.createPrinter({removeComments:true});
const code=s=>printer.printFile(ts.createSourceFile('input.ts',s,99,true));
const effectUnchanged=head('src/engine/Core/Game.ts').equals(fs.readFileSync('src/engine/Core/Game.ts'));
const rechargeCodeUnchanged=code(head('src/engine/Items/ArcanaRecharge.ts').toString())===code(fs.readFileSync('src/engine/Items/ArcanaRecharge.ts','utf8'));
const loaderOld=methods(head('src/engine/Items/ItemLoader.ts').toString()),loaderNow=methods(fs.readFileSync('src/engine/Items/ItemLoader.ts','utf8'));
const loaderMethodsUnchanged=Object.keys(loaderNow).every(k=>code(loaderOld[k])===code(loaderNow[k]));
const baselineHash=sha(fs.readFileSync('src/test/fixtures/generation_baseline.json'));
const boundary={production,changedMethods,changedExistingTests:changed.filter(f=>f.endsWith('.test.ts')),unexpected,unchangedCategories,retired,oldStaffsUnchanged,completeCatalog,effectUnchanged,rechargeCodeUnchanged,loaderMethodsUnchanged,baselineHash};
fs.writeFileSync(`${dir}/boundary.json`,JSON.stringify(boundary,null,2)+'\n');
if(unexpected.length||changedMethods.length||!unchangedCategories||!retired||!oldStaffsUnchanged||!completeCatalog||!effectUnchanged||!rechargeCodeUnchanged||!loaderMethodsUnchanged||baselineHash!==JSON.parse(fs.readFileSync(`${dir}/baseline-hashes.json`)).after)throw Error('Scope changed');
execFileSync(process.execPath,['scripts/w26-test-scope.mjs'],{stdio:'inherit'});
...
const manifest=()=>[...new Set([...walk('src'),...walk('public'),...walk('scripts'),...walk('../BrogueCE-master/src'),...tracked,'progress.md','ai_docs/tasks/w-26.prompt.md','ai_docs/reports/w-0-survey.report.md',...Array.from({length:25},(_,i)=>`ai_docs/reports/w-${i+1}.report.md`),`${dir}/closure.json`,`${dir}/browser-states.json`,`${dir}/drift-attribution.json`,`${dir}/drift-summary-before-recapture.md`])].sort().map(f=>`${sha(fs.readFileSync(f))}  ${f}`).join('\n')+'\n';
...
const run=(name,bin,args)=>{const start=new Date().toISOString(),log=fs.openSync(`${dir}/final-${name}.log`,'w');console.log(`${start} START ${name}`);
 const r=spawnSync(bin,args,{stdio:['ignore',log,log],env:{...process.env,W26_EVIDENCE:'1'}});fs.closeSync(log);const row={name,bin,args,start,end:new Date().toISOString(),exit:r.status,error:r.error?.message};console.log(`${row.end} END ${name}: ${r.status}`);return row;};
