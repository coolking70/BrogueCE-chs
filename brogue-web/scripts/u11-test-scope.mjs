...
const planned = ['src/entities/Monster.ts','src/engine/Core/Game.ts','src/engine/Core/EntitySnapshot.ts','src/engine/Combat/Combat.ts','src/engine/Combat/MonsterAbsorption.ts','src/engine/Combat/MonsterBlink.ts'];
...
const patterns = {
    required: /anyoneWantABite|canAbsorb|moveAlly|deathProcessed|removeDeadMonsters|corpse|absorb|targetCorpseLoc|targetCorpseName|corpseAbsorptionCounter|absorptionFlags|absorbBehavior|absorptionBolt|MB_ABSORBING|isAbsorbing|newPowerCount|totalPowerCount/i,
    expanded: /serializeMonster|deserializeMonster|restoreEntityGraph|createMonsterFromSnapshot|restoreMonster|monsterEntersLevel|monstersFall|cloneMonster|polymorph|negateCreatureMagic|inflictDamage|absorbShieldDamage|resolveBurningDamage|resolveExplosionDamage|generateMonsterDetail/i,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-11-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
