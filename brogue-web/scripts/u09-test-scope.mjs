// U09: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const planned = ['src/entities/Monster.ts','src/entities/Creature.ts','src/engine/Core/Game.ts','src/engine/Combat/Bolt.ts'];
...
const patterns = {
    required: /monsterCanLearnBolt|negatable|TELEPORT|POLYMORPH|invisible|weakness|TRANSFERENCE|TURRET/i,
    expanded: /LEARNABLE_|NOT_LEARNABLE|onHitStatus|statusImmunities|abilities|CONJURATION|DOMINATION|INVISIBILITY|LIGHTNING|POISON|ENTRANCEMENT|SLOW|syncFlagDerivedStatuses|castMonsterBolt|specificallyValidBoltTarget/i,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-09-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
