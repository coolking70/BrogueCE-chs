...
const out = 'ai_docs/reports/u-18a-evidence';
...
const files = [...walk('src'), ...walk('scripts')].filter(p => /\.(ts|tsx|js|mjs|cjs|vue)$/.test(p) && !p.includes('/u18a-'));
const seeds = ['engine/Map/Grid', 'engine/Map/TerrainCatalog', 'engine/Map/Scent', 'engine/Map/SafetyMap', 'engine/Map/WaypointMap', 'entities/Monster', 'engine/Core/Game'].map(p => `src/${p}.ts`);
const imports = new Map();
const sourceHits = [];
const legacyAccesses = [];
const pattern = /isPassable|isOpaque|\bpassable\b|\bopaque\b|T_PATHING_BLOCKER|T_DIVIDES_LEVEL|canMoveTo/;
...
const selected = new Set([...closure, ...sourceHits.map(h => h.file)]);
