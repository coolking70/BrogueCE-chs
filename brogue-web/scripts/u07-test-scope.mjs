// U07: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const planned = ['src/entities/Monster.ts','src/engine/Core/Game.ts','src/engine/Combat/Bolt.ts','src/engine/Combat/BoltTrajectory.ts','src/engine/Combat/BoltReflection.ts','src/engine/Map/SafetyMap.ts','src/engine/Map/Pathfinding.ts'];
...
const patterns = {
    required: /blinkToPreferenceMap|monsterBlink|specificallyValidBoltTarget|BLINK|40/,
    expanded: /SafetyMap|safetySnapshot|scent|CreaturePlacement|castMonsterBolt|perimeterCoords|MA_CAST|MONST_CAST_SPELLS_SLOWLY/,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-07-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
