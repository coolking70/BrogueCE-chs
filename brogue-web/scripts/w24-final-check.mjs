...
const production=['src/data/arcana.json','src/engine/Combat/Bolt.ts','src/engine/Core/Game.ts','src/engine/Items/ArcanaInstance.ts','src/engine/Items/ItemLoader.ts','src/locales/zh_CN.json'];
const allowedTests=['src/engine/Combat/BoltCatalog.test.ts','src/test/w_5_arcana_instance.test.ts','src/test/w_17_domination.test.ts','src/test/w_23_negation.test.ts'];
const changed=execFileSync('git',['diff','HEAD','--name-only','--relative','--','src'],{encoding:'utf8'}).trim().split('\n');
const unexpected=changed.filter(f=>!production.includes(f)&&!allowedTests.includes(f)&&f!=='src/test/fixtures/generation_baseline.json');
const dataOld=JSON.parse(head('src/data/arcana.json')),dataNew=JSON.parse(fs.readFileSync('src/data/arcana.json'));
const unchangedCategories=Object.keys(dataOld).filter(k=>k!=='wands').every(k=>JSON.stringify(dataOld[k])===JSON.stringify(dataNew[k]));
const retired=['wand_of_fire','wand_of_lightning'].every(id=>JSON.stringify(dataOld.wands.find(w=>w.id===id))===JSON.stringify(dataNew.wands.find(w=>w.id===id)));
const empowermentTextUnchanged=dataOld.wands.find(w=>w.id==='wand_of_empowerment').description===dataNew.wands.find(w=>w.id==='wand_of_empowerment').description;
const baselineHash=sha(fs.readFileSync('src/test/fixtures/generation_baseline.json'));
const boundary={production,changedMethods,changedExistingTests:changed.filter(f=>f.endsWith('.test.ts')),unexpected,unchangedCategories,retired,empowermentTextUnchanged,baselineHash};
fs.writeFileSync(`${dir}/boundary.json`,JSON.stringify(boundary,null,2)+'\n');
if(unexpected.length||changedMethods.some(k=>!['toSnapshot','loadSnapshot'].includes(k))||!unchangedCategories||!retired||!empowermentTextUnchanged||baselineHash!==JSON.parse(fs.readFileSync(`${dir}/baseline-hashes.json`)).after)throw Error('Scope changed');
execFileSync(process.execPath,['scripts/w24-test-scope.mjs'],{stdio:'inherit'});
...
const run=(name,bin,args)=>{const start=new Date().toISOString(),log=fs.openSync(`${dir}/final-${name}.log`,'w');console.log(`${start} START ${name}`);
 const r=spawnSync(bin,args,{stdio:['ignore',log,log],env:{...process.env,W24_EVIDENCE:'1'}});fs.closeSync(log);const row={name,bin,args,start,end:new Date().toISOString(),exit:r.status,error:r.error?.message};console.log(`${row.end} END ${name}: ${r.status}`);return row;};
