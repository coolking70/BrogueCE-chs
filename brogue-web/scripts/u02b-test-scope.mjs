// U02b: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const planned = ['src/engine/Random.ts', 'src/engine/Core/Game.ts', 'src/App.vue', 'src/components/MainMenu.vue', 'src/engine/Seed.ts', 'src/engine/Items/ItemLoader.ts', 'src/engine/Map/WaypointMap.ts'];
...
const patterns = {
    required: /seed|levelSeed|RNG_SUBSTANTIVE|getState|seedRandomGenerator/,
    expanded: /rand64bits|visited|levelSeeds|resetCounters|initializeLevelSeeds|randomNumbersGenerated|rngStates|currentRNG|loadSnapshot|loadReplay|replaySeek|flavor|Flavor/,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-02b-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
