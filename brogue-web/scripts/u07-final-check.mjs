...
const files=[...walk('src'),...walk('public'),...walk('scripts'),...walk('../BrogueCE-master/src'),'progress.md',`${dir}/ce-blink.c`,`${dir}/ce-blink.json`,`${dir}/closure.json`,`${dir}/initial-hashes.json`,'ai_docs/tasks/u-07.prompt.md','ai_docs/reports/x-0-survey.report.md','ai_docs/reports/u-18a.report.md',...fs.readdirSync('.').filter(f=>/^(package.*json|tsconfig.*json|vite.config.ts|index.html)$/.test(f))].sort();
...
const gates=[
 ['ce-golden','node',['scripts/u07-ce-golden.mjs']],['boundary','node',['scripts/u07-audit.mjs']],
 ['build','npm',['run','build']],
