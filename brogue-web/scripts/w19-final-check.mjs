...
const dir='ai_docs/reports/w-19-evidence';
...
const allowed=['createMonsterFromSnapshot','generateTestDepth','applyBoltEffect','negateCreatureMagic','serializeMonster','deserializeMonster','loadSnapshot','becomeAllyWith'];
...
if(boundary.changedGameMethods.some(k=>!allowed.includes(k))||boundary.addedGameMethods.some(k=>k!=='demoteMonsterFromLeadership')||Object.values(boundary.unchangedFiles).some(x=>!x.equal)||Object.values(boundary.protectedMethods).some(x=>!x))throw Error('Unexpected boundary change');
