...
const out='ai_docs/reports/u-19e-evidence',files=['src/engine/Generator/BlueprintEngine.ts','src/engine/Core/Game.ts'];
const sha=s=>crypto.createHash('sha256').update(s).digest('hex');
const saved=(f,s)=>fs.readFileSync(`${out}/${path.basename(f,'.ts')}-${s}.ts.txt`,'utf8');
const source=(s,f)=>s==='s0'?saved(f,'before'):s==='s1'?saved(f,f===files[0]?'s1':'before'):s==='s2'?saved(f,f===files[0]?'s2':'before'):fs.readFileSync(f,'utf8');
const stages=['s0','s1','s2','s3'];const names=process.argv.slice(2).length?process.argv.slice(2):[...stages,'undo-s1','undo-s2','undo-s3'];
const temp=fs.mkdtempSync(path.join(os.tmpdir(),'u19e-stage-'));
try{for(const name of names){const inputs={};for(const file of files){
 ...
 await build({entryPoints:['scripts/u19e-observe.ts'],outfile:`${temp}/run.mjs`,platform:'node',format:'esm',bundle:true,plugins:[{name:'stage',setup(b){b.onLoad({filter:/\/(BlueprintEngine|Game)\.ts$/},a=>{const f=files.find(f=>a.path.endsWith('/'+f));if(f)return{contents:inputs[f],loader:'ts'};});}}]});
...
if(![...stages,...stages.slice(1).map(s=>'undo-'+s)].every(s=>fs.existsSync(`${out}/generation-${s}.json.gz`)))process.exit(0);
