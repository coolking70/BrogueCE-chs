// U00: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const planned = ['src/engine/Core/Game.ts', 'src/App.vue', 'src/test/harness.ts',
    'src/entities/Creature.ts', 'src/engine/Items/ItemLoader.ts', 'src/engine/Systems/Logger.ts'];
...
const patterns = {
    lifecycle: /startNewGame|replayRestart|stats|pendingFallenByDepth|isGameOver/,
    expanded: /resetEntityIds|allocateEntityId|ensureEntityIdAbove|initConsumables|setDormantAwakener|discardInFlightAdvancement|pendingBoltFrames/,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-00-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
    fs.readFileSync(f, 'utf8').split('\n').forEach((line, i) => {
        if (Object.values(patterns).some(re => re.test(line))) hits.push(`${f}:${i + 1}:${line}`);
    });
}
fs.writeFileSync(`${dir}/semantic-hits.txt`, hits.join('\n') + '\n');
const tree = ts.createSourceFile('Game.ts', fs.readFileSync('src/engine/Core/Game.ts', 'utf8'), ts.ScriptTarget.Latest, true);
const game = tree.statements.find(n => ts.isClassDeclaration(n) && n.name.text === 'Game');
const fields = game.members.filter(ts.isPropertyDeclaration).map(n => ({
    name:n.name.getText(tree), static:!!n.modifiers?.some(m=>m.kind===ts.SyntaxKind.StaticKeyword),
    line:tree.getLineAndCharacterOfPosition(n.getStart(tree)).line+1,
    declaration:n.getText(tree),
}));
fs.writeFileSync(`${dir}/game-fields.json`, JSON.stringify(fields,null,2)+'\n');
