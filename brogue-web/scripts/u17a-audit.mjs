...
const files = [...walk('src'), ...walk('scripts')].filter(p => /\.(ts|tsx|js|mjs|cjs|vue)$/.test(p) && !p.includes('/u17a-'));
const seeds = ['engine/Map/Grid','engine/Map/TerrainCatalog','engine/Map/DungeonFeatureCatalog','engine/UI/TerrainAppearanceCatalog','engine/Map/DungeonFeature','engine/Map/Promotion','engine/Map/Scent','engine/Generator/Architect','engine/Generator/BlueprintEngine','engine/Map/AutoGenerator','engine/Core/Game','entities/Creature','engine/Status/statusConfig','engine/UI/Appearance'].map(p => `src/${p}.ts`).concat(['src/components/GameCanvas.vue']);
const imports = new Map();
const sourceHits = [];
const legacyAccesses = [];
const pattern = /spawnDungeonFeature|fillSpawnMap|DungeonFeature|DFF_|DF_MISSING_TILES|T_OBSTRUCTS_ITEMS|T_AUTO_DESCENT|swapItem|refreshSideEffects|evacuat|aggravat|obstruction|subsequentDF|flavorMessage|description|readFileSync|readFile/;
