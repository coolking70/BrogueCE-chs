...
const files=[...walk('src'),...walk('public'),...walk('scripts'),...walk('../BrogueCE-master/src'), 'ai_docs/reports/u-06-evidence/ce-staff-damage.json', 'ai_docs/reports/u-06-evidence/closure.json', 'progress.md', 'ai_docs/tasks/u-06.prompt.md', 'ai_docs/reports/x-0-survey.report.md',...fs.readdirSync('.').filter(f=>/^(package.*json|tsconfig.*json|vite.config.ts|index.html)$/.test(f))].sort();
...
const gates=[
 ['ce-golden','node',['scripts/u06-ce-golden.mjs']],
 ['audit','node',['scripts/u06-audit.mjs']],
 ['build','npm',['run','build']],
