...
const cwd=process.cwd(),out=path.join(cwd,'ai_docs/reports/u-17c-evidence'),label=process.argv[2]??'discovery';
const base=fs.mkdtempSync(path.join(os.tmpdir(),'u17c-head-')),temp=path.join(base,'brogue-web');fs.mkdirSync(temp);
try{
