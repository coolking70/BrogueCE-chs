...
const traceCapture=JSON.parse(fs.readFileSync(`${out}/trace-check.json`)); for(const r of traceCapture)if(sha(r.file)!==r.sha)throw Error('Trace changed after attribution');
...
execFileSync(process.execPath,['scripts/u15d3-audit.mjs'],{stdio:'inherit'});
fs.writeFileSync(`${out}/failures-final.ndjson`,'');
const before=hash();save('frozen-inputs-before',before);const gates=[];
for(const [name,cmd,args] of [['build','npm',['run','build']],['regression','npm',['test','--','--maxWorkers=4','--reporter=default','--reporter=json','--reporter=./scripts/u15d3-failure-reporter.mjs',`--outputFile=${out}/regression-final.json`]],['drift','npm',['run','test:drift']],['diff-check','git',['diff','--check']]]){
...
const unexpectedBaselineChanges=Object.keys(baselineBefore).filter(f=>baselineAfter[f] !== (f.endsWith('/generation_baseline.json') ? capture.after : baselineBefore[f]));
...
const named=['p1_30','u24','u_01','u_03','u_05','u_15d','u_16','armor_runic','b_4a','w_20_','w_23_','u_r2','u_r4','Discoveries'];const namedCoverage=Object.fromEntries(named.map(n=>[n,[...actual].filter(f=>f.split('/').at(-1).startsWith(n))]));save('named-guard-coverage',namedCoverage);const missingNamed=named.filter(n=>!namedCoverage[n].length);
