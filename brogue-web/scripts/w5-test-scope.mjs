// Run from brogue-web: node scripts/w5-test-scope.mjs
...
const seeds=['src/engine/Items/Item.ts','src/engine/Items/ItemLoader.ts','src/engine/Core/Game.ts','src/engine/Items/ArcanaInstance.ts'];
...
fs.writeFileSync('ai_docs/reports/w-5-evidence/closure.json',JSON.stringify({seeds,patterns:Object.fromEntries(Object.entries(patterns).map(([k,v])=>[k,v.source])),R,S,all},null,2)+'\n');
fs.writeFileSync('/tmp/w5-tests.txt',all.filter(f=>!f.endsWith('/generation_baseline.test.ts')).join('\n')+'\n');
