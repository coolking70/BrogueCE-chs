// Run from brogue-web: R(actual inputs) + W-0 S(G,Q,B,U,D), identity/save/flavor consumers and source/fixture readers.
...
const planned = ["src/data/arcana.json", "src/engine/Items/ArcanaRecharge.ts", "src/components/GameCanvas.vue", "src/engine/UI/DetailGenerator.ts", "src/engine/UI/Appearance.ts", "src/engine/Combat/BlinkTargeting.ts", "src/engine/Items/ItemLoader.ts", "src/engine/Combat/Bolt.ts", "src/engine/Core/Game.ts", "src/locales/zh_CN.json"];
...
const patterns = {
    ...
    G: /ItemLoader|arcana\.json|terrainFingerprint|generateDepth|chooseKind/,
    D: /DungeonFeature|TerrainCatalog|Promotion|FORCEFIELD/,
    U: /InputManager|engine\/Input|InventoryOverlay|GameCanvas|handlePlayerAction/,
    persistence: /saveSnapshot|loadSnapshot|toSnapshot|Flavors|flavor|MAGIC_POLARITY|marketValue/,
...
for (const f of files) fs.readFileSync(f, 'utf8').split('\n').forEach((line, i) => {
    if (/staff|wand|magicPolarity|kindPolarity|itemMagicPolarity|magicCharDiscoverySuffix|arcanaFlavor|marketValue/i.test(line)) hits.push(`${f}:${i + 1}:${line}`);
});
