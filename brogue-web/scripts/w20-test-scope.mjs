// Run from brogue-web: R(actual production files) + W-0 S(M,C,B) + clone/identity/save consumers, plus source/fixture readers.
...
const seeds = ['src/engine/Core/Game.ts','src/entities/Monster.ts','src/engine/Combat/Bolt.ts','src/engine/Combat/Cloning.ts','src/locales/zh_CN.json'];
...
const patterns = {
    ...
    clone: /cloneMonster|copyForClone|cloneState|cloneLocation|isClone|PLENTY|polymorph|mutation|carriedMonster|carriedItem|typeId|deathEffectTriggered|seized|seizing|dormant|captive|saveSnapshot|loadSnapshot|toSnapshot/,
