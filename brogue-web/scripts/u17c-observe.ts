...
for(const seed of base.seeds){
 ...
 for(let depth=1;depth<=26;depth++){
  ...
  rows.push({seed,depth,fp:terrainFingerprint(g.grid),n:g.monsters.length,
   ...
   carriers:Object.fromEntries(['MACHINE_PRESSURE_PLATE_USED','TRAP_DOOR','TRAP_DOOR_HIDDEN','WALL_LEVER','WALL_LEVER_PULLED','MACHINE_TRIGGER_FLOOR_REPEATING'].map(name=>[name,g.grid.cells.flat().filter((c:any)=>c.layers.includes((T as any)[name])).length]))});
 }
}
