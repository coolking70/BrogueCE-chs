// U01: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const planned = ['src/engine/Core/Game.ts', 'src/engine/Items/Item.ts', 'src/entities/Creature.ts', 'src/entities/Player.ts', 'src/entities/Monster.ts', 'src/engine/Core/EntitySnapshot.ts'];
...
const patterns = {
    required: /GameSnapshotItem|serializeMonster|originDepth|disposableHere|ticksUntilTurn|machineHome/,
    expanded: /toSnapshot|loadSnapshot|deserializeItem|keyLoc|snapshotForm|cloneState|carriedItem|carriedMonster|safetySnapshot|hungerTransition/,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-01-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
...
const fields = [];
for (const [name,file] of [['Item','src/engine/Items/Item.ts'],['Creature','src/entities/Creature.ts'],['Monster','src/entities/Monster.ts'],['Player','src/entities/Player.ts']]) {
 ...
 for(const n of cls.members.filter(ts.isPropertyDeclaration)) fields.push({class:name,field:n.name.getText(tree),line:tree.getLineAndCharacterOfPosition(n.getStart(tree)).line+1,declaration:n.getText(tree)});
}
fs.writeFileSync(`${dir}/entity-fields.json`, JSON.stringify(fields,null,2)+'\n');
