...
const manifest = () => [...walk('src'), ...walk('public'), ...walk('scripts'), 'progress.md', 'ai_docs/tasks/w-9.prompt.md', 'ai_docs/reports/w-0-survey.report.md', '../BrogueCE-master/src/brogue/Items.c', '../BrogueCE-master/src/brogue/PowerTables.c', '../BrogueCE-master/src/variants/GlobalsBrogue.c', ...fs.readdirSync('.').filter(f => /^(package.*\.json|tsconfig.*\.json|vite\.config\..*|index\.html)$/.test(f))].sort().map(p => `${sha(fs.readFileSync(p))}  ${p}`).join('\n') + '\n';
...
const unchangedFiles = ['src/data/arcana.json', 'src/engine/Combat/BoltCatalog.ts', 'src/engine/Combat/Combat.ts', 'src/engine/Combat/BoltTrajectory.ts', 'src/engine/Items/Item.ts', 'src/engine/Items/ItemLoader.ts', 'src/engine/Items/ArcanaInstance.ts', 'src/engine/Items/ArcanaRecharge.ts', 'src/engine/Items/ArcanaEnchantment.ts', 'src/engine/Random.ts', 'src/entities/Creature.ts', 'src/entities/Monster.ts', 'src/entities/Player.ts', 'src/engine/Combat/StaffDamage.ts', 'src/engine/Combat/BoltReflection.ts', 'src/engine/Combat/BoltTargeting.ts', 'src/test/scroll_effects.test.ts', 'src/test/p4_1b_monster_casting.test.ts', 'src/test/p2_2_real_speed.test.ts', 'src/test/fixtures/generation_baseline.json'];
...
for (const name of ['applyBoltEffect', 'applyMonsterBoltHit']) {
    ...
    effectBoundary[name] = Object.fromEntries(Object.keys(before).filter(k => ![
        'BoltEffect.SLOW', 'BoltEffect.HASTE', 'BoltEffect.HEALING', 'BoltEffect.INVISIBILITY', 'BoltEffect.DISCORD',
    ].includes(k)).map(k => [k, before[k] === after[k]]));
}
const boundary = {
    effectBoundary,
    head: execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(), changedMethods, addedMethods,
    protectedMethods: Object.fromEntries(['castMonsterBolt', 'applyDirectBoltDamage', 'generateDepth', 'tickArcanaResources', 'tickCreatureStatuses', 'applyStatusToMonster', 'applyTimedStatus', 'discordBlastFromPlayer', 'negateCreatureMagic', 'trySplitMonster', 'exposeCreatureToFire', 'applyBoltTerrainAt'].map(k => [k, oldMethods[k] === newMethods[k]])),
...
if (Object.values(effectBoundary).some(cases => Object.values(cases).some(v => !v))
    || changedMethods.some(k => !['applyBoltResult', 'applyMonsterBoltHit', 'applyBoltEffect'].includes(k))
    || addedMethods.join(',') !== 'applyBasicBoltEffect' || Object.values(boundary.protectedMethods).some(v => !v)
    || Object.values(boundary.unchangedFiles).some(v => !v.equal) || boundary.playerDamageHasPhysicalAttack) throw Error('W-9 boundary mismatch');
execFileSync(process.execPath, ['scripts/w9-test-scope.mjs']);
