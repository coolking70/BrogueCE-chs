...
const dir='ai_docs/reports/u-14a-evidence'; fs.mkdirSync(dir,{recursive:true});
...
`;
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'u14a-ce-'));
try {
 ...
 const keys=['weakness','damageFP','accuracy','defense','strengthModifierFP','netEnchantFP','weaponLow','weaponHigh','playerDefense','throwDistance'];
 const rows=out.trim().split('\n').map(l=>Object.fromEntries(l.split(' ').map((v,i)=>[keys[i],Number(v)])));
 fs.writeFileSync(`${dir}/ce-weakness.c`,source);fs.writeFileSync(`${dir}/ce-weakness.json`,JSON.stringify(rows,null,2)+'\n');console.log(`${rows.length} original CE golden rows`);
