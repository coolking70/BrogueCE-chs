...
for(const seed of base.seeds){
 ...
 for(let depth=1;depth<=26;depth++){
  ...
  rows.push({seed,depth,fp:terrainFingerprint(g.grid),n:g.monsters.length,
   ...
   carriers:Object.fromEntries(['TRAMPLED_FOLIAGE','ACTIVE_BRIMSTONE','BRIMSTONE_FIRE','OPEN_IRON_DOOR_INERT','BRIDGE_FALLING'].map(name=>[name,g.grid.cells.flat().filter((c:any)=>c.layers.includes((T as any)[name])).length]))});
 }
}
