...
for(const seed of base.seeds){
 ...
 for(let depth=1;depth<=26;depth++){
  ...
  rows.push({seed,depth,fp:terrainFingerprint(g.grid),n:g.monsters.length,
   ...
   carriers:Object.fromEntries(['RAT_TRAP_WALL_CRACKING', 'STATUE_CRACKING', 'COFFIN_OPEN', 'WORM_TUNNEL_MARKER_ACTIVE', 'PORTAL_LIGHT', 'FLOOR_FLOODABLE', 'RUBBLE'].map(name=>[name,g.grid.cells.flat().filter((c:any)=>c.layers.includes((T as any)[name])).length]))});
 }
}
