// U14a: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const planned = ['src/engine/Core/Game.ts', 'src/entities/Creature.ts', 'src/entities/Player.ts', 'src/entities/Monster.ts', 'src/engine/Status/statusConfig.ts', 'src/components/Sidebar.vue'];
...
const patterns = {
    required: /NAUSEOUS|DARKNESS|MAGICAL_FEAR|weaknessAmount|maxStatus|nauseous|darkness|magical_fear|weakened/,
    expanded: /MA_CAUSES_WEAKNESS|T_CAUSES_NAUSEA|negateCreatureStatusEffects|applyMonsterOnHitStatus|STUCK|DONNING|ENRAGED|LIFESPAN|effectiveStrength/,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-14a-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
