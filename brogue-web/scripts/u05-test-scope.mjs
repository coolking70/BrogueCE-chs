// U05: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const planned = ['src/engine/Core/Game.ts','src/engine/Generator/BlueprintEngine.ts','src/engine/Items/ItemLoader.ts',...files.filter(f=>/Items\/Arcana/.test(f)), 'src/engine/Items/MachineItemGeneration.ts'];
...
const patterns = {
    machine: /itemQualifiers|MF_REQUIRE_GOOD_RUNIC|MF_NO_THROWING_WEAPONS|MF_REQUIRE_HEAVY_WEAPON|spawnBlueprintItem|STAFF|WAND|priorItemIds|generatedItems/,
    required: /u_01|u_03|u24|i18n|machine|carriedItem|identityId|runic|Arcana/,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-05-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
