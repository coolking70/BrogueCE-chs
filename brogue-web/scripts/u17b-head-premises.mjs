...
const cwd=process.cwd(),out=path.join(cwd,'ai_docs/reports/u-17b-evidence');
const base=fs.mkdtempSync(path.join(os.tmpdir(),'u17b-head-'));
...
try{
 ...
 const red=JSON.parse(fs.readFileSync(`${out}/${label==='original'?'old-guards-final-code':`${label}-old-guards`}.json`));const titles=[],files=[];
 for(const f of red.testResults){const failed=f.assertionResults.filter(a=>a.status==='failed');if(failed.length){files.push(path.relative(cwd,f.name));titles.push(...failed.map(a=>a.title));}}
 if(!titles.length)throw Error('No failed premises selected; refuse an unfiltered run');
 const escape=s=>s.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
 const args=[path.join(cwd,'node_modules/vitest/vitest.mjs'),'run',...files,...(label==='b1'?[]:['-t',titles.map(escape).join('|')]),'--maxWorkers=2','--reporter=json',`--outputFile=${out}/head-premises-${label}.json`];
 ...
 fs.writeFileSync(`${out}/premise-counterfactual-${label}.json`,JSON.stringify({head:execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),files,titles,exit:r.status},null,2)+'\n');process.exitCode=r.status;
