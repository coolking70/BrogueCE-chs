...
const out='ai_docs/reports/u-19e-evidence',natural=JSON.parse(fs.readFileSync(`${out}/actions.json`)).rows;
 ...
 for(const r of natural){
  ...
  for(let index=0;index<r.commands.length;index++){
   ...
   for(const phase of r.phases.filter(p=>p.commands===index+1)){
    ...
    assert.deepEqual(state.player,phase.player,`CE${r.ce}/${phase.label}/location`);assert.deepEqual(state.inventory,phase.inventory,`CE${r.ce}/${phase.label}/inventory`);assert.equal(state.hp,phase.hp,`CE${r.ce}/${phase.label}/hp`);
    for(const tile of phase.tiles)assert.deepEqual(state.tiles.find(t=>t.x===tile.x&&t.y===tile.y).layers,tile.layers,`CE${r.ce}/${phase.label}/tile ${tile.x},${tile.y}`);
    if(['swapped','resurrected','closed','reopened','replaced','activated','awakened','sacrificed','final'].includes(phase.label)){await page.waitForTimeout(60);await page.screenshot({path:`${out}/browser-ce${r.ce}-${phase.label}.png`});}
