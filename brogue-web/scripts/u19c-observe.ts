...
const base=JSON.parse(fs.readFileSync('ai_docs/reports/u-19c-evidence/baseline-before.json','utf8'));
...
for(const seed of base.seeds){
 ...
 for(let depth=1;depth<=26;depth++){
  ...
  rows.push({seed,depth,fp:terrainFingerprint(g.grid),n:g.monsters.length,
   ...
   committed:machines.filter(m=>m.status==='committed').map(m=>({ce:m.ceBlueprintId,number:m.machineNumber,features:m.features,products:m.products}))});
 }
 views=[];machines=[];
}
