// U10: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const planned = ['src/entities/Monster.ts','src/engine/Core/Game.ts','src/engine/Core/EntitySnapshot.ts','src/engine/UI/DetailGenerator.ts','src/components/DetailPanel.vue'];
...
const patterns = {
    required: /targetCorpseLoc|targetCorpseName|corpseAbsorptionCounter|absorptionFlags|absorbBehavior|absorptionBolt|MB_ABSORBING|isAbsorbing|newPowerCount|totalPowerCount/i,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-10-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
