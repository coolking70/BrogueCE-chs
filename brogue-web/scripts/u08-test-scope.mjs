// U08: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const planned = ['src/entities/Monster.ts','src/engine/Core/Game.ts','src/engine/Combat/Bolt.ts','src/engine/Map/DungeonFeature.ts','src/engine/Map/DungeonFeatureCatalog.ts','src/engine/Map/TerrainCatalog.ts','src/engine/Map/Grid.ts','src/engine/UI/Appearance.ts'];
...
const patterns = {
    required: /SPIDERWEB|ANCIENT_SPIRIT_VINES|DF_SPIDERWEB|DF_ANCIENT_SPIRIT_VINES|DF_WEB_SMALL|DF_WEB_LARGE/,
    expanded: /T_ENTANGLES|T_CAUSES_DAMAGE|effect.?===?.?null|KNOWN_GAP.*BOLT|castMonsterBolt|specificallyValidBoltTarget|monsterCanLearnBolt|web|vines|spider|mangrove_dryad/i,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-08-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
