...
for(const seed of [424242,777]){
 ...
 for(let depth=1;depth<=21;depth++){
  if(depth>1){g.depth=depth;g.generateDepth(false,false);}
  if((seed===424242&&[5,8].includes(depth))||(seed===777&&[14,21].includes(depth)))for(const m of machines.filter(m=>m.observation?.ceBlueprintId===47))cases.push({seed,depth,origin:m.door??m.center,trace:JSON.parse(JSON.stringify(m.observation)),snapshot:JSON.parse(JSON.stringify(g.toSnapshot()))});
 }
}
for(const c of cases)for(const mode of ['origin','derived']){
 ...
 fs.writeFileSync('ai_docs/reports/u-19f-evidence/altar-probe.json',JSON.stringify(results,null,2)+'\n');
}
