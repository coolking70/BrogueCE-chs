// U03: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const planned = ['src/engine/Core/Game.ts', 'src/engine/Map/Grid.ts', 'src/engine/Map/Scent.ts', 'src/entities/Monster.ts', 'src/App.vue', 'src/engine/Core/EntitySnapshot.ts'];
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-03-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
