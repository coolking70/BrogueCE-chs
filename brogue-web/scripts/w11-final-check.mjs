...
const unchangedFiles = ['src/data/arcana.json', 'src/data/monsters.json', 'src/data/mutations.json', 'src/engine/Combat/Bolt.ts', 'src/engine/Combat/BoltCatalog.ts', 'src/engine/Combat/BoltTrajectory.ts', 'src/engine/Items/Item.ts', 'src/engine/Items/ItemLoader.ts', 'src/engine/Items/ArcanaInstance.ts', 'src/engine/Items/ArcanaRecharge.ts', 'src/engine/Items/ArcanaEnchantment.ts', 'src/engine/Random.ts', 'src/engine/Environment/Gas.ts', 'src/engine/Map/DungeonFeature.ts', 'src/engine/Map/TerrainCatalog.ts', 'src/entities/Creature.ts', 'src/entities/Monster.ts', 'src/entities/Player.ts', 'src/test/fixtures/generation_baseline.json'];
...
for (const name of ['applyBoltEffect', 'applyMonsterBoltHit']) {
    ...
    effectBoundary[name] = Object.fromEntries(Object.keys(before).filter(k => ![
        ...(name === 'applyBoltEffect' ? ['BoltEffect.TELEPORT'] : []),
...
if (Object.values(effectBoundary).some(cases => Object.values(cases).some(v => !v))
    ...
    || Object.values(boundary.unchangedFiles).some(v => !v.equal) || boundary.playerDamageHasPhysicalAttack) throw Error('W-11 boundary mismatch');
execFileSync(process.execPath, ['scripts/w11-test-scope.mjs']);
