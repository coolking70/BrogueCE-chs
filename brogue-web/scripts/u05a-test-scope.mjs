// U05a: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const planned = ['src/engine/Core/Game.ts', 'src/engine/Generator/BlueprintEngine.ts', 'src/engine/Items/ItemLoader.ts'];
...
const patterns = {
    required: /adoptedItem|adoptiveItem|itemSpawns|carriedItem|MF_MONSTER_TAKE_ITEM/,
    expanded: /MF_ADOPT_ITEM|MF_OUTSOURCE_ITEM_TO_MACHINE|spawnBlueprintItem|spawnHordeAtFeature|subMachines/,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-05a-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
