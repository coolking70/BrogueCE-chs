...
const cwd=process.cwd(),out=path.join(cwd,'ai_docs/reports/u-19f-evidence'),label=process.argv[2]??'discovery';
const base=fs.mkdtempSync(path.join(os.tmpdir(),'u19f-head-')),temp=path.join(base,'brogue-web');fs.mkdirSync(temp);
try{
