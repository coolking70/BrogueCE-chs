...
for(const checkpoint of [0,3,11]) {
 ...
 // Reintroduce only the old load-time reshuffle: the reshuffle now also consumes the live stream, as CE requires.
...
fs.writeFileSync('ai_docs/reports/u-02b-evidence/continuation.json',JSON.stringify(results,null,2)+'\n');
