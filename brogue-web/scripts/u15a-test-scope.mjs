...
const planned = ['src/engine/Core/Game.ts', 'src/engine/Map/Grid.ts', 'src/engine/Generator/BlueprintEngine.ts'];
...
const patterns = {
    required: /crystalizeFromPlayer|impregnableCells|isImpregnable|IMPREGNABLE/,
    expanded: /shatter|SHATTER|freeCaptive|MONST_ATTACKABLE_THRU_WALLS|MONST_TURRET|readItem|toSnapshot|loadSnapshot/,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-15a-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
