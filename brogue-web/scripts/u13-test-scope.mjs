// U13: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const planned = ['src/engine/Core/Game.ts', 'src/entities/Creature.ts', 'src/entities/Player.ts', 'src/entities/Monster.ts', 'src/engine/Combat/Combat.ts', 'src/engine/Combat/CombatFormulas.ts', 'src/engine/Items/Item.ts', 'src/engine/Items/ItemLoader.ts', 'src/engine/Core/EntitySnapshot.ts', 'src/data/monsters.json', 'src/data/weapons.json'];
...
const patterns = {
    required: /randClump|autoHit|backstab|invisible|weaknessAmount/,
    expanded: /CombatSystem|hitProbability|damageFraction|accuracyFraction|defenseFraction|clumpedRoll|damageString|clumping|playerDefense|BE_ATTACK/,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-13-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
