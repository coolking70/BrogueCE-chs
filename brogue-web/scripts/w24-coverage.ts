/** W-24 repeatable observation probe. Bundle with esbuild; run with an output path. */
...
const seeds = [424242, 777, 20260913, 31337, 2, 3, 4, 5];
const rows: any[] = [];
let attempts: any[] = [];
...
log(`W-24 sampled ${rows.length} levels -> ${process.argv[2]}`);
