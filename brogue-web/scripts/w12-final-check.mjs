...
const unchangedFiles = ['src/data/arcana.json', 'src/data/monsters.json', 'src/data/mutations.json', 'src/engine/Combat/Bolt.ts', 'src/engine/Combat/BoltCatalog.ts', 'src/engine/Movement/CreaturePlacement.ts', 'src/engine/Map/Promotion.ts', 'src/engine/Items/Item.ts', 'src/engine/Items/ItemLoader.ts', 'src/engine/Items/ArcanaInstance.ts', 'src/engine/Items/ArcanaRecharge.ts', 'src/engine/Items/ArcanaEnchantment.ts', 'src/engine/Random.ts', 'src/engine/Environment/Gas.ts', 'src/engine/Map/DungeonFeature.ts', 'src/engine/Map/TerrainCatalog.ts', 'src/entities/Creature.ts', 'src/entities/Monster.ts', 'src/entities/Player.ts', 'src/test/fixtures/generation_baseline.json'];
...
const boundary = {
    effectBoundary,
    methodScope: changedMethods.every(k => ['zapBoltFromPlayer', 'applyBoltEffect', 'applyMonsterBoltHit', 'pickUpItemAfterDisplacement', 'placeCreature'].includes(k)) && addedMethods.every(k => ['finishBlink', 'beckonCreature'].includes(k)),
    protectedTerrainFunctions: Object.fromEntries(Object.keys(oldTerrainFunctions).map(k => [k, oldTerrainFunctions[k] === newTerrainFunctions[k]])),
    head: execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(), changedMethods, addedMethods,
    protectedMethods: Object.fromEntries(['castMonsterBolt', 'teleportCreature', 'freeCaptive', 'applyDirectBoltDamage', 'generateDepth', 'handlePlayerAction', 'teleportPlayerRandom', 'playerFalls', 'monstersFall', 'playerTurnEnded', 'processStaggerHit', 'removeDeadMonsters', 'tickArcanaResources', 'applyBasicBoltEffect', 'applyStatusToMonster', 'applyTimedStatus', 'resolveBurningDamage', 'applyInstantExplosionAt', 'triggerDeathFeatures', 'objectiveTimeBlock'].map(k => [k, oldMethods[k] !== undefined && oldMethods[k] === newMethods[k]])),
...
if (!boundary.methodScope || Object.values(effectBoundary).some(cases => Object.values(cases).some(v => !v))
    || Object.values(boundary.protectedTerrainFunctions).some(v => !v)
    || Object.values(boundary.protectedMethods).some(v => !v)
    || Object.values(boundary.unchangedFiles).some(v => !v.equal) || boundary.playerDamageHasPhysicalAttack) throw Error('W-12 boundary mismatch');
execFileSync(process.execPath, ['scripts/w12-test-scope.mjs']);
const scope = JSON.parse(fs.readFileSync(`${dir}/closure.json`, 'utf8'));
const before = manifest();
fs.writeFileSync(`${dir}/sha256-before.txt`, before);
const commands = [
    ['build', 'npm', ['run', 'build']],
    ['regression', process.execPath, ['node_modules/vitest/vitest.mjs', 'run', ...scope.all.filter(p => !p.endsWith('/generation_baseline.test.ts')), '--maxWorkers=10', '--reporter=default', '--reporter=json', `--outputFile.json=${dir}/final-regression.json`]],
