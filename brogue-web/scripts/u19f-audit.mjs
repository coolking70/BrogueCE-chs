...
const seeds=['src/engine/Generator/BlueprintEngine.ts','src/engine/Generator/MachineView.ts','src/engine/Generator/Architect.ts','src/engine/Map/AutoGenerator.ts','src/engine/Core/Game.ts','src/data/blueprints.json','src/engine/Lighting/FOV.ts','src/engine/Map/DungeonFeature.ts','src/engine/Map/TerrainCatalog.ts','src/engine/Map/WallDoorFinish.ts','src/engine/Map/Grid.ts','src/engine/Map/DungeonFeatureCatalog.ts','src/engine/UI/TerrainAppearanceCatalog.ts','src/engine/Map/Promotion.ts','src/engine/Combat/Bolt.ts'];
const pattern=/ELECTRIC_CRYSTAL|TURRET_LEVER|WORM_TUNNEL|NET_TRAP|ALARM_TRAP|DEWAR_|DEEP_WATER_ALGAE|FUNGUS_FOREST|finishWalls|promoteOnPlayerBump|COMMUTATION|RESURRECTION|SACRIFICE|ALTAR_CAGE|purgatory|commuteFloorItems|resurrectAlly|canReceiveAdoptedItem|isThrowingTutorialReward|HAS_ITEM|HAS_MONSTER|HAS_PLAYER|HAS_STAIRS|hasPendingOccupant|fillVestibuleInterior|fillAreaInterior|mapMachineInterior|cellIsFeatureCandidate|randomMatchingLocation|runAutogenerators|MachineEntityRuntime|createMachineRuntime|spawnHordeAtFeature|spawnHordeAt|finalizeBlueprintMonster|abortItemsAndMonsters|AMULET|populateLevel|pendingItems|pendingMonsters|buildAMachine|blueprintQualifies|readFileSync|readFile/;
