// Run from brogue-web: node scripts/w7-test-scope.mjs
...
const seeds=['src/engine/Items/Item.ts','src/engine/Core/Game.ts','src/engine/Items/ArcanaEnchantment.ts','src/components/InventoryOverlay.vue','src/components/GameCanvas.vue','src/data/consumables.json','src/locales/zh_CN.json'];
...
const patterns={U:/InputManager|engine\/Input|InventoryOverlay|GameCanvas|handlePlayerAction/,Q:/useArcanaItem|spawnStaff|spawnWand|tickArcanaResources|rechargeStaffsAndCharms|enchantEquippedItem|magicCharDiscoverySuffix|arcanaFlavor/,G:/ItemLoader|arcana\.json|terrainFingerprint|generateDepth|chooseKind/,enchant:/enchant/i,storage:/readFileSync|loadSnapshot|saveSnapshot|toSnapshot|serializeItem|deserializeItem|maxCharges|charges|enchantment/,observation:/createHeadlessGame|startNewGame|Architect|generate\(|populate|spawn|seed|distribution/};
...
fs.writeFileSync('ai_docs/reports/w-7-evidence/closure.json',JSON.stringify({seeds,patterns:Object.fromEntries(Object.entries(patterns).map(([k,v])=>[k,v.source])),R,S,all},null,2)+'\n');
fs.writeFileSync('/tmp/w7-tests.txt',all.filter(f=>!f.endsWith('/generation_baseline.test.ts')).join('\n')+'\n');
