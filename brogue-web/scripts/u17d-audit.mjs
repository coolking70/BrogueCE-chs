...
const out = 'ai_docs/reports/u-17d-evidence';
...
const files = [...walk('src'), ...walk('scripts')].filter(p => /\.(ts|tsx|js|mjs|cjs|vue|json)$/.test(p) && !p.includes('/u17d-'));
const seeds = ['engine/Items/Item','engine/Core/EntitySnapshot','engine/Map/LakeSystem','engine/Map/Grid','engine/Map/TerrainCatalog','engine/Map/DungeonFeatureCatalog','engine/UI/TerrainAppearanceCatalog','engine/Map/DungeonFeature','engine/Map/Promotion','engine/Map/Scent','engine/Generator/Architect','engine/Generator/BlueprintEngine','engine/Map/AutoGenerator','engine/Core/Game','entities/Creature','engine/Status/statusConfig','engine/UI/Appearance'].map(p => `src/${p}.ts`).concat(['src/components/GameCanvas.vue','src/data/blueprints.json','src/locales/zh_CN.json','src/test/fixtures/u21c-ce-terrain.json','src/test/fixtures/u17d-ce-catalog.json']);
const imports = new Map();
const sourceHits = [];
const legacyAccesses = [];
const pattern = /METHANE_VENT|PILOT_LIGHT|PARALYSIS_VENT|POISON_GAS_VENT|GAS_TRAP_POISON|FLAMETHROWER|ARMOR_IMMOLATION|pressurePlate|TrapDepressions|fireType|promoteChance|Gas|PRESSURE_PLATE|TRAP_DOOR|WALL_LEVER|FLOOR_TRIGGER_REPEATING|discoverTerrain|discoverSecret|searchForSecrets|spawnDungeonFeature|fillSpawnMap|DungeonFeature|DFF_|DF_MISSING_TILES|T_OBSTRUCTS_ITEMS|T_AUTO_DESCENT|swapItem|refreshSideEffects|evacuat|aggravat|obstruction|subsequentDF|flavorMessage|description|readFileSync|readFile/;
