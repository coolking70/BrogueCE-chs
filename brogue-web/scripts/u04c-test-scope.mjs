// U04c: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const planned = ['src/engine/Core/Game.ts', 'src/engine/Map/Grid.ts', 'src/engine/Map/Scent.ts', 'src/entities/Monster.ts', 'src/App.vue', 'src/engine/Core/EntitySnapshot.ts', 'src/engine/Generator/BlueprintEngine.ts', 'src/engine/Movement/CreaturePlacement.ts', 'src/engine/Items/ItemSpawnHeatMap.ts', 'src/engine/Map/MachineCells.ts'];
...
const patterns = {
    machine: /machineCells|machineNumber|mr\.cells|NO_INTERIOR_FLAG|heatMap|HeatMap|IS_IN_MACHINE|teleportCandidates|keyMatchesLocation|BUILD_ANYWHERE/, 
    required: /levels|meteredItems|foodSpawned|goldGenerated|spawnFuse|monsterSpawnFuse|pendingFallenByDepth|leaderId/,
    expanded: /scent|awaySince|currentLevelDepth|monstersFall|loadSnapshot|toSnapshot|serializeMonster|flavor|Flavor|machineCells|run\.|nextEntityId|currentTick|i18n_scan|source/,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-04c-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
