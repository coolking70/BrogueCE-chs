...
const dir='ai_docs/reports/w-25-evidence',base=JSON.parse(fs.readFileSync(`${dir}/baseline-before.json`,'utf8'));
...
const ordered=actual.filter(s=>old.some((o:any)=>o.id===s.id)).map(s=>old.find((o:any)=>o.id===s.id));
function sample(pool: typeof actual){
 ...
 for(const seed of base.seeds){
  ...
  ItemLoader.spawnStaff=(id,x,y)=>{const before=rng.randomNumbersGenerated,item=spawn.call(ItemLoader,id,x,y);trace.push({depth,id,before,after:rng.randomNumbersGenerated,e:item?.enchantment});return item;};
...
try{
 const control=sample(old),orderOnly=sample(ordered),current=sample(actual);
 const controlDiff=changes(base.levels,control.levels),orderDiff=changes(control.levels,orderOnly.levels),additionDiff=changes(orderOnly.levels,current.levels),totalDiff=changes(base.levels,current.levels);
 const result={controlDiff,orderDiff,additionDiff,totalDiff,control,orderOnly,current};
