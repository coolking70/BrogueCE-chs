// U06: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const planned = ['src/engine/Core/Game.ts', 'src/entities/Monster.ts', 'src/engine/Combat/Bolt.ts', 'src/engine/Combat/BoltTrajectory.ts', 'src/engine/Combat/StaffDamage.ts'];
...
const patterns = {
    required: /applyMonsterBoltHit|BE_DAMAGE|FIRE|SPARK|DRAGONFIRE|inflictDamage/,
    expanded: /castMonsterBolt|staffDamage|rollStaffDamage|BE_ATTACK|POISON_DART|WHIP|shielded|immune_fire|negateCreatureMagic|triggerDeathFeatures|finishTurnEpilogue/,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-06-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
