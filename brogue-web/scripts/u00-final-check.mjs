...
const files=[...walk('src'),...walk('public'),...walk('scripts'),...fs.readdirSync('.').filter(f=>/^(package.*json|tsconfig.*json|vite.config.ts|index.html)$/.test(f))].sort();
...
const gates=[
 ['build','npm',['run','build']],
 ['regression','npx',['vitest','run',...all.filter(f=>!f.endsWith('generation_baseline.test.ts')),'--maxWorkers=2','--reporter=default','--reporter=json',`--outputFile.json=${dir}/final-tests.json`]],
 ['drift','npm',['run','test:drift','--','--maxWorkers=1','--reporter=default','--reporter=json',`--outputFile.json=${dir}/final-drift.json`]],
 ['fresh-generation','node',['scripts/u00-generation-check.mjs']],
];
const outcomes=[];
