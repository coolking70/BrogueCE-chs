...
const boundary = {
    ...
    methodScope: changedMethods.every(k => ['zapBoltFromPlayer', 'applyBoltResult', 'applyBoltEffect', 'rebuildWaypoints', 'toSnapshot', 'loadSnapshot'].includes(k)) && addedMethods.every(k => ['tunnelAt'].includes(k)),
    ...
    protectedMethods: Object.fromEntries(['castMonsterBolt', 'teleportCreature', 'crystalizeFromPlayer', 'finishBlink', 'beckonCreature', 'placeCreature', 'freeCaptive', 'applyDirectBoltDamage', 'generateDepth', 'handlePlayerAction', 'teleportPlayerRandom', 'playerFalls', 'monstersFall', 'playerTurnEnded', 'processStaggerHit', 'removeDeadMonsters', 'tickArcanaResources', 'applyBasicBoltEffect', 'applyStatusToMonster', 'applyTimedStatus', 'resolveBurningDamage', 'applyInstantExplosionAt', 'triggerDeathFeatures', 'objectiveTimeBlock'].map(k => [k, oldMethods[k] !== undefined && oldMethods[k] === newMethods[k]])),
...
if (!boundary.waypointAlgorithmUnchanged || !boundary.methodScope || Object.values(effectBoundary).some(cases => Object.values(cases).some(v => !v))
    ...
    || Object.values(boundary.unchangedFiles).some(v => !v.equal) || boundary.playerDamageHasPhysicalAttack) throw Error('W-13 boundary mismatch');
execFileSync(process.execPath, ['scripts/w13-test-scope.mjs']);
