...
execFileSync(process.execPath,['scripts/u19f-audit.mjs'],{stdio:'inherit'});
fs.writeFileSync(`${out}/failures-final.ndjson`,'');
const before=hash();save('frozen-inputs-before',before);const gates=[];
for(const [name,cmd,args] of [['build','npm',['run','build']],['negative',process.execPath,['scripts/u19f-negatives.mjs']],['regression',process.execPath,['node_modules/vitest/vitest.mjs','run','--exclude','**/generation_baseline.test.ts','--maxWorkers=4','--reporter=default','--reporter=json','--reporter=./scripts/u19f-failure-reporter.mjs',`--outputFile=${out}/regression-final.json`]],['drift','npm',['run','test:drift']],['diff-check','git',['diff','--check']]]){
