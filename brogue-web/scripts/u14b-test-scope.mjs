// U14b: AST reverse imports plus whole-repository semantic/source-reader inventory.
...
const patterns = {
    required: /STUCK|DONNING|ENRAGED|LIFESPAN|stuck|donning|enraged|lifespan|maxStatus/,
    expanded: /T_ENTANGLES|breakEntanglingTerrain|MA_AVOID_CORRIDORS|moralAttack|playerDefense|multiplicity|spectral|guardian|moveEntranced|negateCreatureStatusEffects/,
...
const hits = [];
for (const f of [...files, ...walk('scripts'), ...walk('ai_docs')].filter(f => !f.includes('/u-14b-evidence/') && /\.(ts|vue|mjs|cjs|md)$/.test(f))) {
