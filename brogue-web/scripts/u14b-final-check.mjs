...
const files=[...walk('src'),...walk('public'),...walk('scripts'),...walk('../BrogueCE-master/src'),'progress.md',`${dir}/ce-donning.c`,`${dir}/ce-donning.json`,`${dir}/closure.json`,'ai_docs/tasks/u-14b.prompt.md','ai_docs/reports/x-0-survey.report.md',...fs.readdirSync('.').filter(f=>/^(package.*json|tsconfig.*json|vite.config.ts|index.html)$/.test(f))].sort();
...
const gates=[
 ['ce-golden','node',['scripts/u14b-ce-golden.mjs']],['boundary','node',['scripts/u14b-audit.mjs']],['counterfactual','node',['scripts/u14b-counterfactual.mjs']],
 ['build','npm',['run','build']],
