// Run from brogue-web: R(actual inputs) + W-0 S(G,Q,B,U), identity/save/flavor consumers and source/fixture readers.
...
const planned = ["src/data/arcana.json", "src/engine/Items/ArcanaInstance.ts", "src/engine/Items/ItemLoader.ts", "src/engine/Combat/Bolt.ts", "src/engine/Core/Game.ts", "src/locales/zh_CN.json"];
...
for (const f of files) fs.readFileSync(f, 'utf8').split('\n').forEach((line, i) => {
    if (/wand|magicPolarity|kindPolarity|itemMagicPolarity|magicCharDiscoverySuffix|arcanaFlavor|marketValue/i.test(line)) hits.push(`${f}:${i + 1}:${line}`);
});
