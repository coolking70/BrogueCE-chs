// Final, explicit-file gates. After freeze, only output reports/evidence may change.
import fs from 'node:fs';import path from 'node:path';import crypto from 'node:crypto';import {spawnSync} from 'node:child_process';
const dir='ai_docs/reports/u-14a-evidence';
const walk=d=>fs.readdirSync(d,{withFileTypes:true}).flatMap(e=>e.isDirectory()?walk(path.join(d,e.name)):[path.join(d,e.name)]);
const hash=f=>crypto.createHash('sha256').update(fs.readFileSync(f)).digest('hex');
const files=[...walk('src'),...walk('public'),...walk('scripts'),...walk('../BrogueCE-master/src'),'progress.md',`${dir}/ce-weakness.c`,`${dir}/ce-weakness.json`,`${dir}/closure.json`,'ai_docs/tasks/u-14a.prompt.md','ai_docs/reports/x-0-survey.report.md',...fs.readdirSync('.').filter(f=>/^(package.*json|tsconfig.*json|vite.config.ts|index.html)$/.test(f))].sort();
...
const gates=[
 ['ce-golden','node',['scripts/u14a-ce-golden.mjs']],['boundary','node',['scripts/u14a-audit.mjs']],['counterfactual','node',['scripts/u14a-negative-check.mjs']],
 ['build','npm',['run','build']],
...
const after=snapshot(),changed=files.filter(f=>before[f]!==after[f]);fs.writeFileSync(`${dir}/final-input-after.json`,JSON.stringify(after,null,2)+'\n');fs.writeFileSync(`${dir}/final-check.json`,JSON.stringify({outcomes,changed,inputCount:files.length,inputManifestSha256:hash(`${dir}/final-input-after.json`),final:true},null,2)+'\n');
