...
describe('W-20 save, identity and actual submission',()=>{
 ...
 it('legacy missing clone tag does not infer clone from name/glyph, and high dormant id is reserved',()=>{const g=live(),m=mob(g);m.id=1200000;m.name='clone';g.toggleMonsterDormancy(m);const saved=JSON.parse(JSON.stringify(g.toSnapshot()));for(const s of saved.dormantMonsters)delete s.cloneState;g.loadSnapshot(saved);expect(g.dormantMonsters[0]!.isClone).toBe(false);expect(new Monster(0,0,data('rat')).id).toBeGreaterThan(1200000);});
