...
/**
 * 端到端实测（2026-09-15，10 种子 × D1-D26 = 260 层）：修复后仅存的
 * 两个坏层，均为机器阶段（BlueprintEngine）所致，与湖泊无关：
 * - seed777/D15：机器锁门（LOCKED_DOOR）落在树状走廊割点上，封死
 *   113 格子树（该层深水格数为 0，可排除任何水因）；
 * - seed999/D12：机器特征水深水（key_flood_trap）切断走廊（Architect
 *   阶段快照 181/181 全连通，机器阶段后 62/180）。
