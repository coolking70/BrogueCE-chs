...
| 轮次 | 工作树 | 分支 | 输出 |
|---|---|---|---|
| **AI-1** P1-52 气味追击诊断轮 | `…/wt-ai-1` | `round/ai-1` | `/tmp/zrun-ai-1.json` |
...
3. **AI-1**（P1-52 气味追击）跑中 → 验收 → 合并
4. 小轮（可合并为一轮）：P1-53 蓝图物品改走 chooseKind + CE_POW_FOOD 注释订正 +
   `DF_CRYSTAL_WALL` 入目录 + `AutoGenerator.ts` 两条缺口接线
   （**移动生成流，独占**）
5. 小轮：`DF_CRYSTAL_WALL` 入目录 + `AutoGenerator.ts` 两条缺口接线（**移动生成流，独占**）
