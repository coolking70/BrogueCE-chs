...
> 最后更新：2026-09-17 上午。**C-4 链全部完成**，P1-42 与 F-0 已提交。**F-1 跑中**。
...
- 测试：**66 文件 / 728 passed | 8 skipped | 5 todo，零红**，build 绿。
...
- **Phase C**：C-0 环路、C-1 房间剖面、C-2 湖泊、C-3 墙面与门、
  **C-4 链全部完成**（4a-0 四层模型 / 4a 属性表 / 4b DF 目录 / 4c promoteTile 与两趟驱动）。
  P1-42（密门发现）与 F-0（火与气体测绘）已完成。
  **F 链跑中**：F-1（火焰迁成地形，行为逐位不变）→ F-2a（火机制）→ G-1（气体）→ G-2。
...
| # | 轮次 | 状态 | 任务书 |
|---|---|---|---|
| 1 | **F-1** 火焰迁成地形（行为逐位不变） | 🟡 跑中 `wt-f-1` | `tasks/f-1.prompt.md` |
| 2 | **F-2a** 放开 CE 火焰蔓延规则（真改玩法；含 P1-44 抗火药水） | ⬜ 待写 | F-0 已给红线清单 |
| 3 | **G-1** 气体迁层 + volume 量纲 + updateVolumetricMedia（含 P1-45 幽灵气） | ⬜ 待写 | G-0 已由 F-0 代替 |
| 4 | **G-2** 24 条 GAS 层 DF 接线 | ⬜ 待写 | |
