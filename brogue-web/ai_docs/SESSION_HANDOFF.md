...
> 最后更新：2026-09-16 夜。C-4 链完成；F 链火侧走完（F-0/F-1/F-2a/F-2b）。**G-1 跑中**。
...
- 测试：**70 文件 / 772 passed 零红**，build 绿（F-2b 后；二级门禁顺延到 F-2c 之后）。
...
- **Phase C**：C-0 环路、C-1 房间剖面、C-2 湖泊、C-3 墙面与门、
  ...
  **F 链火侧已走完**：F-0 测绘 ✅ → F-1 迁层 ✅ → F-2a 蔓延/寿命/产物 ✅
  → F-2b 生物燃烧状态机 + P1-44 ✅。
  **气体侧进行中**：**G-1（迁 GAS 层 + volume 量纲 + updateVolumetricMedia）跑中**
  → G-2（24 条 GAS DF + 蒸汽源）→ G-3（气体效果阈值与比例伤害，生物侧）
  → **F-2c（爆炸 GAS_EXPLOSION，须在 G-2 之后）**。
...
| # | 轮次 | 状态 | 任务书 |
|---|---|---|---|
| 1 | **G-1** 气体迁 GAS 层 + volume 量纲 + 扩散/消散/混合（含 P1-45） | 🟡 跑中 `wt-g-1` | `tasks/g-1.prompt.md` |
| 2 | **G-2** 24 条 GAS 层 DF 接线（含 §5.3-9 蒸汽源） | ⬜ 待写 | |
