...
> 最后更新：2026-09-17 夜。F/G 链完成；C-5 坠落、B-0 勘察、B-1a 鉴定第一刀均已提交。
> **第二对并行：B-1b（鉴定持久化）+ C-6（runAutogenerators）跑中。**
...
- 测试：**76 文件 / 892 passed 零红**，build 绿（B-1a 后串行读数）。
...
| # | 轮次 | 状态 | 任务书 |
|---|---|---|---|
| 1 | **B-1b** 鉴定态持久化 + call + 戒指双槽 + 移除免费按钮 | 🟡 跑中 `wt-b-1b` | `tasks/b-1b.prompt.md` |
| 2 | **C-6** runAutogenerators（digDungeon 最后一步） | 🟡 跑中 `wt-c-6` | `tasks/c-6.prompt.md` |
| 3 | **B-2** 投掷 ∥ **B-3** 三占位卷轴（B-0 说可并行） | ⬜ 待写 | |
| 4 | **B-1c** detect magic 极性 | ⬜ 待写 | `isPolarityRevealed()` 留形已备 |
| 5 | **B-4** 生成规则对齐（**独占轮**，移动 RNG 流） | ⬜ 待写 | P1-50：金币恒 0、life 药水 10-15 只 |
| 6 | **C-7** 光照目录 | ⬜ 待写 | |
| 7 | **UI/渲染轮** | ⬜ 待写 | 已积四条：P1-47 裸键名、EMBERS/ASH/PLAIN_FIRE 渲染、燃烧无视觉、onConfirmRequest |
| — | 中小 P1：P1-39 / P1-41 / P1-43 / C-4a-1 / i18n 扫描器模板字符串盲区 | ⬜ | 可合并 |
