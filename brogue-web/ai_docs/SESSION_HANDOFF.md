...
| 轮次 | 工作树 | 分支 | 输出 |
|---|---|---|---|
| **B-4b** 物品落位与金币「落在哪/多少」 | `…/wt-b-4b` | `round/b-4b` | `/tmp/zrun-b-4b.json` |
...
2. **B-4b** 跑中 → 验收 → 合并
3. **AI 轮（P1-52）**：气味追击成功率仅约 10%，玩家侧表现为"怪物很容易跟丢"。
   实测证据在路线图 P1-52；嫌疑是气味梯度在门格附近非单调、过早落入
   「局部最大 → 放弃」路径。**这是本轮登记里唯一玩家可感知的行为缺陷**，
   优先级建议排在渲染之前。
4. 小轮：`DF_CRYSTAL_WALL` 入目录 + `AutoGenerator.ts` 两条缺口接线（**移动生成流，独占**）
