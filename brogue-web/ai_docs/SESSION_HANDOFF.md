...
**进度**：W-0 ✅ → W-1 ✅ `a72ac60` → W-2 ✅ `53e9a27` → W-3 ✅ `aa77220`
→ W-4 ✅ `dd82261` → W-5 ✅ `60a89ee` → W-6 ✅ `1219bad` → W-7 ✅ `348b869`
→ W-8 ✅ `6582cf4` → W-9 ✅ `e36d575` → W-10 ✅ `ec8b671` → W-11 ✅ `f141c64` → W-12 ✅ `d93cae4`
→ W-13 ✅ `59f462f` → W-14 ✅ `5bce487` → W-15 ✅ `d5c01a6` → W-16 ✅ `cf8d0f8` → W-17 ✅ `0e0d467`
→ W-18 ✅ `a255166` → W-19 ✅ `195dbd9` → W-20 ✅ `5212426` → W-21 ✅ `b66d0e0`
→ **W-22 ⏸ 暂缓** `59e172c` → W-23 ✅ `59575da` → W-24 ✅ `5b8b31b` → W-25 ✅ `2472cfe`
→ W-26 ✅ `59e1512`。**🏁 W 链收官（2026-09-24）**：21 种法器正常入口逐项审计通过；
9 魔杖/22 + 12 杖/110，定义 24（21 CE + 3 退池）。门禁 137 文件 / 2525 通过 / 660s。
**W 链全部未闭合项（26 项，带来源轮次）见 `ai_docs/reports/w-26.report.md` §8**——全项目勘察的起点。
📌 W-24 重捕获过基线（91 字段，单变量归因）；W-25/W-26 零漂移未重采。

### 🔭 X-0 全项目勘察（`153dbb5`）——W 链之后的权威规格
**`ai_docs/reports/x-0-survey.report.md`**：27 域覆盖表（§2）、42 项合并欠账 K01–K42（§3）、
派发单元 U00–U27 + UR1–4（§4）、Game.ts 评估（§5）。**后续任务书以它为准**，同 W-0 的地位。
- 推荐起步：**U00 开新局归零 → U01 快照丢字段 → U05a 领养品唯一性**；U02/U03 先交兼容方案
- Game.ts：**不整体拆**；UR1（编解码）在 U01 后可做，UR2–4 等触发条件（报告 §5.3）——用户决策
- 验收方抽查属实：DCOLS=79 非 64（K40）、serializeItem 漏 flags/originDepth/disposableHere（K02）、
  startNewGame 不清 stats（K39）
- ⚠️ 历史报告与现树不符的：C-7 的 64 宽、V-2b-6 的快照往返、V-1 简并 rand_range 耗骰前提
- 📌 **用户裁决（2026-09-24）：项目仍在开发阶段，旧存档兼容/迁移不用考虑。** 任务书不要求旧档兜底；
  X-0 里 U01/U02/U03/U10 等"在场：旧档恢复策略/schema 迁移"据此降级为无需在场
- 📌 **用户裁决（2026-09-25）：地图/生成大面积变化等"保留现状"类事项不是卡点，其他大部分决策点也不是。**
  X-0 所有"在场=是"单元可自主派发；仍保留"生成漂移先单变量归因再重采"、全量门禁与 CE 抽查（防 bug 而非保现状）。
  仍需先问：不可逆/对外操作、凭据、重大额度决策。
- X-0 已完成 13 轮：U00 `ead265f`、U01 `552c25b`、U05a `4edb9d9`、U15a `20d69e2`、U06 `282372e`、U13 `2197706`、
  U14a `f93e3ff`、U18a `75b9600`、U07 `8b13602`、U08 `27203b9`、U09 `3d668fe`、U10 `8279737`、U11 `6f40cfe`
  （W-22 学习收口，empowerment 口径"数值与学习均对齐"）。门禁 150 文件 / 3049 通过。
- 模型评测（Windows 192.168.0.106，E:\bench，W-21 重跑各 1 次）：astra xhigh ✅、sol medium ✅（token 38%）、
  sol xhigh ❌（留 2 红）、luna xhigh/medium ❌（540 黄金值不符）。主力仍 astra xhigh。
**机制前置七轮全部首提全绿、零补完轮。** W-23 后门禁：134 文件 / 2425 通过 / 721s。

⏸ **W-22 暂缓原因**：blink（dar blademaster/imp）与古灵藤蔓（mangrove dryad）是普通尸体可达的
可学候选，CE 抽签不看 web 可执行性；删候选/重抽/作废/"学会但无效"都偏离 CE。
empowerment 口径 = **"数值已对齐、学习未完成"**。恢复前置：怪物 blink 调度 + 藤蔓 DF 旁支 +
吸收字段存档；是否开这三轮由用户在 W-26 后决定。CE 缺陷留存：`anyoneWantABite` 第二遍复用
最后一名盟友的距离图——恢复时要忠实复刻。

📌 **待用户决定（W-26 后）**：① W-22 前置三轮是否排期 ② 全项目勘察轮 ③ Game.ts 拆分。
W-5 是唯一动生成流的一轮（91/104 层、339 字段），已重捕获基线。

📌 W-7 的一个 CE 事实值得留存：CE 附魔 STAFF 写 `enchant2 = 500/E`
（`Items.c:7863`），而充能卷轴写 `5000/E`（`:4730`）——**这个"不对称"是承重的**，
它让附魔后计时低于 W-6 归一化上界，避免把附魔刚送的充能扣回去。别"修"它。

📌 **用户裁决（2026-09-23）：W-4~W-23 继续夜跑，但 W-24~26 留到用户在场再做。**
理由：那三轮是唯一会大幅改变游戏内容分布的（改 kind 候选/频率），
且连改三轮生成池若前面归因不干净，第三轮会很难查。
⚠️ W-5 虽在夜跑范围内，但**它会移动生成流**（恢复实例充能抽签），需重捕获基线。
...
### ⛔ 验收方凭记忆/读注释写 CE 事实，一天错了三次（W 链，2026-09-23）

| 轮 | 验收方写的 | CE 实际 | 谁发现的 |
|---|---|---|---|
| W-10 | "吸毒气会获得毒状态" | 毒气是 `T_CAUSES_DAMAGE` 直接扣血；**毒地衣**（`T_CAUSES_POISON`）才挂毒 | 执行方 |
| W-16 | "刀刃有寿命、到期消失" | `Globals.c:1137` 刀刃 1 HP、**无寿命**，靠一击即死/消魔/…而消失 | 执行方 |
| W-16 提交信息 | "刀刃不下楼，**玩家换层即死**" | `Time.c:2561-2563` 的绑定领袖死亡判定**明确排除 `MONSTER_ALLY`** ⇒ 刀刃是盟友，不走这条 | 执行方（W-17 复核时） |

⚠️ **第三条最值得记**：那条是验收方**查过 CE 之后**写的——读的是 `Rogue.h:2156` 的注释
"will die if the leader dies or becomes separated from the leader"，据此推断了行为，
**但没读实际执行路径**。**注释写的是意图，代码里有例外（`!= MONSTER_ALLY`）。**

⇒ 规矩：
1. 任务书里验收方写的 CE 陈述**一律标"线索，请自行核实"**（这三次都因此没造成损失）
2. 验收方自己下 CE 结论时，**要读到执行路径的代码，不能停在注释或头文件**
3. **`cf8d0f8`（W-16）提交信息中"玩家换层即死"一句作废**，以本节为准
