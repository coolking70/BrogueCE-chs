...
| 轮次 | 工作树 | 分支 | 输出 |
|---|---|---|---|
| **T-1** 尾账合并轮（独占生成流） | `…/wt-t-1` | `round/t-1` | `/tmp/zrun-t-1.json` |
| **R-1** 渲染纯重构（与 T-1 并行，文件不相交） | `…/wt-r-1` | `round/r-1` | `/tmp/zrun-r-1.json` |
...
4. **T-1** 尾账合并轮跑中（P1-53 加权抽取 + DF_CRYSTAL_WALL 接线 + 两处订正）
5. **UI-1**（六条渲染欠账）—— **必须等 R-1 落地后才有落点**
6. 小轮：`DF_CRYSTAL_WALL` 入目录 + `AutoGenerator.ts` 两条缺口接线（**移动生成流，独占**）
