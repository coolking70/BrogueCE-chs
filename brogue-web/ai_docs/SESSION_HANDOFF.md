...
17. **V-2b-4 祭坛族轮**（下一轮，任务书待写）：解锁 7 条。
    ⚠️ **待用户拍板**：web `ALTAR` 带自创的「取物塌陷」语义
    （`Game.ts:3232-3245`，从祭坛取物→同组祭坛塌成 CHARRED_FLOOR 并销毁其上物品），
    与 CE 的 10 种祭坛正面冲突。是让 CE 祭坛接管、还是保留 web 玩法。
18. ~~原 V-2b-3 条目~~：
