...
| 轮次 | 工作树 | 分支 | 输出 |
|---|---|---|---|
| **C-8** `generateTerrain` 连通性漏洞（诊断+修复） | `…/wt-c-8` | `round/c-8` | `/tmp/zrun-c-8.json` |
...
6. ⏸ **V-1a 已完成但暂缓合并**（分支 `round/v-1a` @ `ed16454`，产物完好）——
   它已达成 CE 量级（附魔卷轴 25.9→**14.9**、life 药水 18.7→**6.3**），
   但**暴露出 `generateTerrain` 的连通性漏洞**：`seed20260916`/D21 是卡死局
   （上行楼梯只可达 23 格，够不到下行楼梯）。四条闸门同时报红
   （`c_3` T12 / `p1_26` / `p1_29` / `p1_33`）。
   **不合并的理由**：要合并就得把坏层塞进 `KNOWN_MACHINE_STAGE_BAD_LEVELS` 白名单，
   那是放宽守卫（该测试自己写着「理想为空集」），且等于明知故犯地发布一张卡死层。
7. **C-8** 连通性诊断修复 跑中 → 绿了之后**回头合并 V-1a**
   → 再 V-1b（引擎轮，**高风险**）→ V-2（数据全量还原）
