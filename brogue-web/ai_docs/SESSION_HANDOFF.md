...
> 最后更新：2026-09-17 夜。**F/G 链全部完成**（火与气体已全量对齐 CE）。
> **首次跨链并行：C-5（坠落）+ B-0（Phase B 勘察）同时跑中。**
...
- 测试：**74 文件 / 848 passed 零红**，build 绿（F-2c 后二级串行读数）。
...
| # | 轮次 | 状态 | 任务书 |
|---|---|---|---|
| 1 | **C-5** 坠落子系统 —— 解禁 CHASM 与桥梁（吸收 P1-22） | 🟡 跑中 `wt-c-5` | `tasks/c-5.prompt.md` |
| 2 | **B-0** Phase B 勘察（纯测量，零代码改动） | 🟡 跑中 `wt-b-0` | `tasks/b-0.prompt.md` |
| 3 | **B-1…B-n** 鉴定系统 / 投掷武器 / 占位物品 | ⬜ 待写 | 输入=B-0 的对照表 |
| 4 | **C-6** runAutogenerators ｜ **C-7** 光照目录 | ⬜ 待写 | |
| 5 | **UI/渲染轮** | ⬜ 待写 | 已积三条：P1-47 裸键名、EMBERS/ASH/PLAIN_FIRE 渲染、燃烧无视觉 |
| — | 中小 P1：P1-39 深水可游 / P1-41 成员铺开改路径距离 / P1-43 蓝图选址 / C-4a-1 | ⬜ | 可合并 |
