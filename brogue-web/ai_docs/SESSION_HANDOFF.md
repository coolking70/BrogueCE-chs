...
> 最后更新：2026-09-17 凌晨。C-2 / P1-37 / C-3 均已提交，C-4a-0 跑中。
...
- 测试：**61 文件 / 627 passed | 8 skipped | 5 todo，零红**，build 绿。
- **Phase D 全部完成**（P4-1 ~ P4-10）。
- **Phase C**：C-0 环路、C-1 房间剖面、C-2 湖泊、C-3 墙面与门 已完成；
  **C-4a-0（四层地形模型迁移）跑中**。
...
| # | 轮次 | 状态 | 任务书 |
|---|---|---|---|
| 1 | **C-4a-0** 四层地形模型迁移（行为逐位不变） | 🟡 跑中 `wt-c-4a-0` | `tasks/c-4a-0.prompt.md` |
| 2 | **C-4a** 地形属性表 + 统一通行判据（吸收 P1-38/40） | ⬜ 待写 | 输入=C-4a-0 的干跑测量表 |
| 3 | **C-4b** DF 目录 + spawnDungeonFeature | ⬜ 待写 | 大工程 |
| 4 | **C-4c** promoteTile + 两趟驱动 + 接线机器 | ⬜ 待写 | 大工程 |
