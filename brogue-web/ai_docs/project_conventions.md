...
## ⚠️ 门禁跑法更正（2026-09-18 实测）：**不要再加 `--fileParallelism=false`**

下面那条「单 worker 跨文件泄漏」的**处置建议是错的** —— 它开出的药方
（强制串行）正是病因本身。实测对比：

| | 串行 `--fileParallelism=false` | 并行（vitest 默认） |
|---|---|---|
| 耗时 | **1432s（24 分钟）** | **411s（6 分 51 秒）** |
| CPU | 单核 | **886%（约 9/10 核）** |
| 结果 | 90 文件 / 1161 绿 | **90 文件 / 1161 绿，零红** |

**快 3.5 倍，且一条都没红。**

**机理**：`--fileParallelism=false` 让全部文件**共用一个 worker、共享模块注册表**，
`rng` 与 `ItemLoader` 的静态态就这么串了过去。
而并行时 vitest 给每个文件独立 worker 且默认 `isolate: true`，**隔离反而更强**。

**佐证**：2026-09-18 修的 `b_1a`「诅咒绝不进名字」顺序依赖
（`generation_baseline` 排在它之前跑就翻红）**正是串行造成的产物** ——
并行下不会出现这种跨文件顺序依赖。

### 新口径

- **全量门禁一律 `npx vitest run`（不加 `--fileParallelism=false`，不带文件参数）。**
- 下面那条旧规矩里「全量字母序门禁是唯一最终口径」**仍然成立**，
  只是"最终口径"改为**并行全量**。
- 仍然**不要**用自定义文件组合的结果下结论 —— 那个理由（组合不代表全量）没变。

### 附：耗时分布（并行前的串行实测，用于挑优化目标）

前 15 个文件吃掉 **74%** 的时间（1064s / 1432s），其余 75 个文件合计仅 368s。
最贵的几个：`c_0_add_loops` 136.7s、`b_4a_item_generation` 115.7s、
**`armor_model_effect` 107.4s（只有 2 个用例，53.7s/例）**、
`c_1_room_profile` 99.8s、`c_8_connectivity` 81.3s、`p1_29` 71.9s、
`monster_stats_effect` 42.8s（2 例）。
并行之后总时长已降到 7 分钟，**这些不再是瓶颈，暂不需要砍样本**——
砍样本会削弱守卫，现在没有理由付这个代价。

---

## vitest 单 worker 复用下的模块级单例跨文件泄漏（B-4b 实测；**处置已被上一节更正**）
