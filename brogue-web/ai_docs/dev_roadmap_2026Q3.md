...
## P1-52（2026-09-18 B-4a 验收时由验收方实测发现）

**追踪怪的气味追击成功率只有约 10%，此前被一条挑 seed 的测试掩盖。**

场景（`p4_8_scent_map` T5）：一条笔直走廊，玩家在 (54,15) 原地等待并持续留味，
一只 HUNTING 的 rat 从 (46,15) 出发，中间隔一扇门；鼠只需走 7 格、有 10 个回合。
实测成功率：main 上 12 seed 中 1 次、B-4a 上 40 seed 中 5 次 —— **约 10%**。

已排除的解释：
- **不是 B-4a 的回归**：两边成功率统计一致，且 B-4a 是纯物品生成轮、AI 零改动；
- **不是"追踪态下走下坡"**：新补的逐步上坡断言在两边全部 seed 上都绿；
- **不止是 CE 的 3% 丢目标**（`Monsters.c:1674-1680`）：10 回合累计约 26%，
  远不足以解释约 90% 的失败率。

**待查**：最可能的嫌疑是气味梯度在门格附近非单调，使鼠过早落入
「局部最大 → 放弃气味」路径①（`Monster.ts` 约 1412-1425，对应 CE
`Monsters.c:3475-3484`）。若属实，这是 web 与 CE 的真实偏差，
会让**所有**隔视线的气味追击提前失效——玩家侧的表现是"怪物很容易跟丢"。

归 **AI 轮**（独立轮次，不要塞进生成轮）。在查清之前，
T5 的结局断言**不构成"追踪行为正确"的证据**。

## P1-47（2026-09-17 F-2c 验收时由执行方在范围外发现）
