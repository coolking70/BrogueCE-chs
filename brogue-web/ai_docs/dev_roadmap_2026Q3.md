...
## P1-46（2026-09-17 P1-42 验收时发现，**比 P1-42 的键位冲突大得多**）
...
**这不是 P1-42 一轮能决定的事，需要用户裁决键位方案**，三条路：
- **A｜回到 CE 的纯 vi 键**：还原度最高，但改变现有操作习惯；
- **B｜保留 WASD，CE 命令改用空闲键**：不破坏习惯，但每个命令都要单独选键，
  且与 CE 文档/攻略对不上；空闲键（避开 CE 已有语义）有 `f` `m` `o` `p` `q` `v`；
- **C｜双套键位可切换**：工作量最大，但两边都不得罪。

**在裁决之前，每一轮移植 CE 命令都会重新撞一次。**
