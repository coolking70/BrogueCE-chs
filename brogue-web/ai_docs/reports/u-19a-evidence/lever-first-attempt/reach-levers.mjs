...
const g=window.activeGame;g.startNewGame({seed:19,mode:'test'});const s=machineScene(g,ce,[36,38].includes(ce)?2:1);observeFromOrigin(g,s,false);const target=s.snapshots[0].placements[0],actions=[],origin={...g.player.loc};
...
g.updateVision();g.update();g.onRenderRequested();return {ce,origin,target,actions,searches,final:{...g.player.loc},adjacent:adjacent(g.player.loc),revealed:g.grid.getCell(target.x,target.y).layers[0]===T.WALL_LEVER,visible:g.grid.getCell(target.x,target.y).isVisible,text:JSON.parse(window.render_game_to_text())};
